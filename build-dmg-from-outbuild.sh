...
# Puku Editor - Production DMG Build (from out-build)
# Uses the production-compiled out-build directory created by gulp compilation
# Skips marketplace extension downloads - uses only built-in extensions
...
# Check if out-build exists (from previous gulp compilation)
if [ ! -d "$VSCODE_DIR/out-build" ]; then
    echo "❌ out-build directory not found!"
    echo "Run the production compilation first:"
    echo "  cd src/vscode"
    echo "  source ~/.nvm/nvm.sh && nvm use 22.20.0"
    echo "  export NODE_OPTIONS='--max-old-space-size=8192'"
    echo "  npx gulp compile-build-with-mangling"
    exit 1
fi
...
echo "📦 Bundling production-compiled VS Code..."

# Copy production-compiled output from out-build to out
# (VS Code runtime expects the out/ directory)
echo "  - Copying minified code from out-build..."
cp -R "$VSCODE_DIR/out-build" "$APP_RESOURCES/app/out"

# Copy node_modules (all dependencies)
...
# Copy built-in extensions (skip marketplace downloads)
...
# Update app metadata with Puku branding
echo ""
...
echo ""
echo "Built with:"
echo "  • Production-compiled code from out-build (minified, 5.8MB saved)"
echo "  • VS Code built-in extensions only (no marketplace downloads)"
echo "  • Puku Editor extension with dependencies"
echo ""
echo "To install:"
