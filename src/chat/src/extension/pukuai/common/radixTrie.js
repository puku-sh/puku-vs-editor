...
class LRURadixTrie {
        ...
        if (remainingKey.length > 0) {
            ...
            for (const [edge, child] of node.children) {
                if (edge.startsWith(remainingKey)) {
                    ...
                    const intermediate = new LRURadixNode();
            ...
            if (remainingKey.length > 0) {
                ...
                const newNode = new LRURadixNode();
    ...
    findAll(key) {
        return this.findClosestNode(key)
            .stack.map(({ node, remainingKey }) => node.value !== undefined ? { remainingKey, value: node.value } : undefined)
    ...
    findClosestNode(key) {
        let hasNext = true;
        let node = this.root;
        const stack = [{ node, remainingKey: key }];
...
exports.LRURadixTrie = LRURadixTrie;
...
//# sourceMappingURL=radixTrie.js.map