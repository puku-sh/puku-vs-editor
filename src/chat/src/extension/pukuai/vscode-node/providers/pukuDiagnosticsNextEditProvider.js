...
exports.PukuDiagnosticsNextEditProvider = void 0;
/*---------------------------------------------------------------------------------------------
...
const pukuDiagnosticsCache_1 = require("../../node/pukuDiagnosticsCache");
/**
 ...
 */
let PukuDiagnosticsNextEditProvider = class PukuDiagnosticsNextEditProvider extends lifecycle_1.Disposable {
    constructor(_authService, _configService, _indexingService, _logService) {
        super();
        this._authService = _authService;
        this._configService = _configService;
        this._indexingService = _indexingService;
        this._logService = _logService;
        this.ID = 'puku-diagnostics';
        this._enabled = true;
        this._cache = new pukuDiagnosticsCache_1.PukuDiagnosticsCache();
        this._requestId = 0;
        ...
        this._register(vscode.workspace.onDidChangeTextDocument(e => this._onDocumentChange(e)));
     ...
     */
    async runUntilNextEdit(docId, context, delayMs, token) {
        const reqId = ++this._requestId;
        const document = docId.document;
        const position = docId.position;
     ...
     */
    async _computeDiagnosticFix(reqId, document, position, token) {
        // Early exits
        if (!this._enabled) {
        ...
        const result = {
            type: 'diagnostics',
            fix: {
                range: new vscode.Range(new vscode.Position(fix.range.start.line, fix.range.start.character), new vscode.Position(fix.range.end.line, fix.range.end.character)),
                newText: fix.newText,
                label: fix.label
            },
            requestId: reqId
     ...
     */
    async _generateDiagnosticFix(reqId, diagnostic, document, token, apiKey) {
        try {
            ...
            const searchResults = await this._indexingService.search(query, 5, // top 5 results
            document.languageId);
     ...
     */
    _getDiagnosticsNearCursor(document, position) {
        const allDiagnostics = vscode.languages.getDiagnostics(document.uri);
        const maxDistance = vscode.workspace.getConfiguration('puku.diagnostics').get('maxDistance', 2);
        // Only errors and warnings within maxDistance lines
        return allDiagnostics
            .filter(d => d.severity <= vscode.DiagnosticSeverity.Warning &&
            Math.abs(d.range.start.line - position.line) <= maxDistance)
            .sort((a, b) => Math.abs(a.range.start.line - position.line) -
            Math.abs(b.range.start.line - position.line));
    }
    /**
     * Extract imports from document
     */
    _extractImports(document) {
...
exports.PukuDiagnosticsNextEditProvider = PukuDiagnosticsNextEditProvider;
exports.PukuDiagnosticsNextEditProvider = PukuDiagnosticsNextEditProvider = __decorate([
    __param(0, pukuAuth_1.IPukuAuthService),
    __param(1, pukuConfig_1.IPukuConfigService),
    __param(2, pukuIndexingService_1.IPukuIndexingService),
    __param(3, logService_1.ILogService)
], PukuDiagnosticsNextEditProvider);
//# sourceMappingURL=pukuDiagnosticsNextEditProvider.js.map