...
suite('CompletionsCache', function () {
	...
	suite('findAll', function () {
		...
		test('handles word-by-word acceptance', function () {
			...
			assert.strictEqual(results[0][0], '.log("Hello, World!");\n}');

			// User accepts '.log'
			results = cache.findAll('function hello() {\n    console.log', '');
			assert.strictEqual(results.length, 1);
			assert.strictEqual(results[0][0], '("Hello, World!");\n}');
		});

		test('supports multiple completions for same prefix (Feature #64)', function () {
			cache.append('const x = ', '', ['42', '100', '999']);

			const results = cache.findAll('const x = ', '');
			// Should return one array with all 3 completions
			assert.strictEqual(results.length, 1);
			assert.strictEqual(results[0].length, 3);
			assert.ok(results[0].includes('42'));
			assert.ok(results[0].includes('100'));
			assert.ok(results[0].includes('999'));
		});

		test('handles empty completion', function () {
			cache.append('const x = ', '', ['']);
			const results = cache.findAll('const x = ', '');
			// Empty completions are filtered out (not useful)
			assert.deepStrictEqual(results, []);
		});
	});
	...
	suite('append', function () {
		...
		test('stores multiple completions in single call (Feature #64)', function () {
			cache.append('const x = ', '', ['42', '100', '999']);
		...
		test('appends to existing prefix with same suffix', function () {
			...
			const results = cache.findAll('const x = ', '');
			// Two separate append calls = 2 arrays
	...
	suite('LRU eviction', function () {
		test('evicts least recently used entries when cache is full', function () {
			// CompletionsCache uses LRURadixTrie with maxSize=500
			// We'll add 501 unique entries to trigger eviction
			for (let i = 0; i < 501; i++) {
			...
			// Last entry (x500) should still be there
			assert.deepStrictEqual(cache.findAll('const x500 = ', ''), [['500']]);
		});
	...
	suite('real-world scenarios', function () {
		test('simulates typing and accepting a Go function', function () {
			...
			assert.strictEqual(results[0][0], '.Println("Hello, World!")');
		...
		test('simulates Python function with context', function () {
			...
			assert.strictEqual(results[0][0], ' n <= 1:\n        return n\n    a, b = 0, 1\n    for _ in range(n):\n        a, b = b, a + b');
		});
		...
		test('simulates JavaScript import and function', function () {
			...
			assert.strictEqual(results[0][0], ', filter, reduce');
		});

		test('simulates cycling through multiple completions (Feature #64)', function () {
			const prefix = 'function calculateTotal(';
			const suffix = '';

			// Store 3 completions from API
			cache.append(prefix, suffix, [
				'items: Item[]): number { return items.reduce((sum, item) => sum + item.price, 0); }',
				'prices: number[]): number { return prices.reduce((sum, p) => sum + p, 0); }',
				'cart: Cart): number { return cart.items.reduce((sum, item) => sum + item.price * item.quantity, 0); }'
			]);

			// User can retrieve all 3 completions for cycling
			const results = cache.findAll(prefix, suffix);
			assert.strictEqual(results.length, 1);
			assert.strictEqual(results[0].length, 3);
			assert.ok(results[0][0].includes('Item[]'));
			assert.ok(results[0][1].includes('number[]'));
			assert.ok(results[0][2].includes('Cart'));
		});
	});
});
