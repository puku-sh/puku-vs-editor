...
(0, vitest_1.suite)('FIM with Import Context - E2E Tests (api.puku.sh)', () => {
    ...
    async function callPukuFimAPI(prompt, suffix, openFiles) {
    ...
    (0, vitest_1.suite)('TypeScript with import context', () => {
        (0, vitest_1.test)('completion uses imported utility function', async () => {
            // Create utility file
            const utilsContent = `
...
function greetUser(name: string, birthYear: number) {
	...
	return `;
            ...
            // Extract imports
            const imports = await pukuImportExtractor_1.pukuImportExtractor.extractImports(mainContent, 'typescript');
        ...
        (0, vitest_1.test)('completion without import context vs with import context', async () => {
...
class UserController {
	...
	async handleGetUser(userId: number) {
		...
		return {
            ...
            const resultWith = await callPukuFimAPI(mainContent, '\n\t\t};\n\t}\n}', [{ filepath: typesUri.fsPath, content: typesContent }]);
    ...
    (0, vitest_1.suite)('Python with import context', () => {
        (0, vitest_1.test)('completion uses imported helper module', async () => {
...
def process_payment(amount: float):
	...
	formatted = `;
            const imports = await pukuImportExtractor_1.pukuImportExtractor.extractImports(mainContent, 'python');
            console.log('Extracted Python imports:', imports);
            // Call API with import context
            const result = await callPukuFimAPI(mainContent, '\n\treturn formatted', [{ filepath: helperUri.fsPath, content: helperContent }]);
            assert.ok(result.choices.length > 0, 'Should return Python completion');
            console.log('Python completion:', result.choices[0]?.text);
        });
    });
    (0, vitest_1.suite)('Go with import context', () => {
        (0, vitest_1.test)('completion uses imported package', async () => {
...
func processData(input string) string {
    ...
    (0, vitest_1.suite)('Performance with import context', () => {
        (0, vitest_1.test)('completion latency with 3 imported files', async () => {
...
const result = `;
        ...
        (0, vitest_1.test)('comparison: no context vs semantic context vs import context', async () => {
...
class UserService {
	...
	async getUser(id: number) {
		return await this.client.`;
            ...
            const result2 = await callPukuFimAPI(mainContent, '\n\t}\n}', [{ filepath: apiUri.fsPath, content: apiContent }]);
    ...
    (0, vitest_1.suite)('Error handling', () => {
        (0, vitest_1.test)('handles API errors gracefully', async () => {
            ...
            try {
                ...
                console.log('Network error (expected):', err.message);
                assert.ok(true);
            }
        });
        (0, vitest_1.test)('handles large context gracefully', async () => {
            ...
            const result = await callPukuFimAPI(mainContent, ';', [{ filepath: largeUri.fsPath, content: largeContent }]);
            assert.ok(result.choices.length > 0, 'Should handle large context');
            console.log('Handled large context successfully');
        });
    });
});
//# sourceMappingURL=pukuFimWithImportContext.e2e.spec.js.map