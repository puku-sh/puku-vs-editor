...
exports.ensureNodePtyShim = ensureNodePtyShim;
const fs_1 = require("fs");
const path = __importStar(require("path"));
let shimCreated = undefined;
 ...
 */
async function ensureNodePtyShim(extensionPath, vscodeAppRoot, logService) {
    if (shimCreated) {
        return shimCreated;
    }
...
async function _ensureNodePtyShim(extensionPath, vscodeAppRoot, logService) {
    ...
    try {
        await fs_1.promises.mkdir(nodePtyDir, { recursive: true });
        const entries = await fs_1.promises.readdir(vscodeNodePtyPath);
...
async function copyNodePtyWithRetries(sourceDir, destDir, entries, logService) {
    const primaryBinary = entries.find(entry => entry.endsWith('.node'));
    for (let attempt = 1; attempt <= MAX_COPY_ATTEMPTS; attempt++) {
        try {
            await fs_1.promises.cp(sourceDir, destDir, {
                recursive: true,
                dereference: true,
                force: true,
...
async function shouldCopyEntry(srcPath, logService) {
    try {
...
async function waitForMaterializedShim(destDir, primaryBinary, logService) {
...
async function isShimMaterialized(destDir, primaryBinary) {
    if (primaryBinary) {
        const binaryStat = await fs_1.promises.stat(path.join(destDir, primaryBinary)).catch(() => undefined);
        if (binaryStat && binaryStat.isFile() && binaryStat.size > 0) {
            return true;
        }
    }
    const entries = await fs_1.promises.readdir(destDir).catch(() => []);
    for (const entry of entries) {
        const stat = await fs_1.promises.stat(path.join(destDir, entry)).catch(() => undefined);
...
//# sourceMappingURL=nodePtyShim.js.map