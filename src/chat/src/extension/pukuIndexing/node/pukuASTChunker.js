...
exports.pukuASTChunker = exports.PukuASTChunker = void 0;
...
class PukuASTChunker {
     ...
     */
    async chunkContent(content, languageId) {
     ...
     */
    isASTSupported(languageId) {
        return (0, treeSitterLanguages_1.getWasmLanguage)(languageId) !== undefined;
    }
    /**
     * AST-based chunking using Tree-sitter
     */
    async _chunkWithAST(content, language) {
        const structure = await structure_1.structureComputer.getStructure(language, content);
        if (!structure) {
            return [];
        }
        const chunks = [];
     ...
     */
    _extractChunksFromNode(node, content, lines, chunks, parentType) {
     ...
     */
    _fillGaps(astChunks, content, lines) {
        if (astChunks.length === 0) {
            return this._chunkByLines(content);
        }
        const result = [];
     ...
     */
    _extractSymbolName(text, chunkType) {
        const patterns = {
            function: [
                /function\s+(\w+)/,
                /const\s+(\w+)\s*=/,
...
exports.PukuASTChunker = PukuASTChunker;
/**
 * Singleton instance
 */
exports.pukuASTChunker = new PukuASTChunker();
//# sourceMappingURL=pukuASTChunker.js.map