...
suite('Tokenizers can be loaded', function () {
    for (const tokenizer of Object.values(tokenization_1.TokenizerName)) {
        test(`Tokenizer ${tokenizer} can be loaded`, function () {
            (0, tokenization_1.getTokenizer)(tokenizer);
        });
    }
});
// test suite for MockTokenizer
suite('MockTokenizer', function () {
    const tokenizer = (0, tokenization_1.getTokenizer)(tokenization_1.TokenizerName.mock);
...
suite('Tokenizer Test Suite - cl100k', function () {
    const tokenizer = (0, tokenization_1.getTokenizer)(tokenization_1.TokenizerName.cl100k);
    ...
    test('tokenizeStrings', function () {
        ...
        assert.strictEqual(tokens_s.slice(0, half).join(''), tokenizer.detokenize(tokens.slice(0, half)), 'tokenizeStrings slice should represent the corresponding slice with tokenize');
    });
    test('takeLastTokens invariant of starting position', function () {
        const suffix = tokenizer.takeLastTokens(source, 25);
        assert.strictEqual(suffix.text, `"To the import {imp.module} as {imp.as_name}, we add the following comment: {description}")\n        return description`);
...
suite('Tokenizer Test Suite - o200k', function () {
    const tokenizer = (0, tokenization_1.getTokenizer)(tokenization_1.TokenizerName.o200k);
    test('empty string', function () {
        const str = '';
        assert.deepStrictEqual(tokenizer.tokenize(str), []);
        assert.strictEqual(tokenizer.detokenize(tokenizer.tokenize(str)), str);
...
suite('ApproximateTokenizer', function () {
    const cl100kTokenizer = new tokenization_1.ApproximateTokenizer(tokenization_1.TokenizerName.cl100k, 'python');
    const o200kTokenizer = new tokenization_1.ApproximateTokenizer(tokenization_1.TokenizerName.o200k, 'python');
    const defaultTokenizer = new tokenization_1.ApproximateTokenizer(); // o200k, no language;
