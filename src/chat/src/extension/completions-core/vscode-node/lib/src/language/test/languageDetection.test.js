...
suite('language detection', function () {
    test('reuse languages for untitled documents', function () {
        assert.deepStrictEqual(languageDetection_1.languageDetection.detectLanguage({ uri: 'untitled:///abc', languageId: 'typescript' }), new languageDetection_1.Language('typescript', true, ''));
    });
    test('normalizes "c" to "cpp" for untitled documents', function () {
        assert.deepStrictEqual(languageDetection_1.languageDetection.detectLanguage({ uri: 'untitled:///abc', languageId: 'c' }), new languageDetection_1.Language('cpp', true, ''));
    });
    test('reuse languages for notebook documents', function () {
        assert.deepStrictEqual(languageDetection_1.languageDetection.detectLanguage({ uri: 'vscode-notebook-cell:/abc', languageId: 'typescript' }).languageId, 'typescript');
    });
    const toDetectByExtension = [
        ['.ts', 'typescript'],
        ['.js', 'javascript'],
        ['.jsx', 'javascriptreact'],
        ['.tsx', 'typescriptreact'],
    ...
    function assertExtension(uri, expectedExtension) {
        const doc = (0, textDocument_1.createTextDocument)(uri, 'clientProvidedLanguageId', 1, 'test content');
        const language = languageDetection_1.languageDetection.detectLanguage(doc);
        assert.deepStrictEqual(language.fileExtension, expectedExtension);
    }
    function assertLanguageId(uri, expectedLanguageId) {
        const doc = (0, textDocument_1.createTextDocument)(uri, 'clientProvidedLanguageId', 1, 'test content');
        const language = languageDetection_1.languageDetection.detectLanguage(doc);
    ...
    test('detected languages for ambiguous options will be re-detected', function () {
        assert.deepStrictEqual(detect('testfile.c', languageDetection_1.languageDetection).languageId, 'cpp');
        assert.deepStrictEqual(detect('testfile.h', languageDetection_1.languageDetection).languageId, 'cpp');
        assert.deepStrictEqual(detect('testfile.cpp', languageDetection_1.languageDetection).languageId, 'cpp');
        assert.deepStrictEqual(detect('testfile.h', languageDetection_1.languageDetection).languageId, 'cpp');
    });
    function detect(filename, languageDetection) {
        return languageDetection.detectLanguage((0, textDocument_1.createTextDocument)(`file:///${filename}`, 'clientProvidedLanguageId', 1, 'test content'));
    }
});
