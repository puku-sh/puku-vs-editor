 ...
 */
function querySnapshot(snapshot, path) {
    const segments = path
        .trim()
        .split('.')
        .map(s => s.trim());
...
function validateNodeName(name, current, segment, path) {
    if (name !== '*' && name !== current.name) {
        throw new Error(`Name mismatch at segment '${segment}'. Expected '${current.name}' but got '${name}'. Path: ${path}`);
    }
}
function validateNodeChildrenLength(index, children, segment, path) {
    if (typeof index === 'number' && index >= children.length) {
        throw new Error(`Index out of bounds at segment '${segment}'. Maximum index is ${children.length - 1}. Path: ${path}`);
    }
}
