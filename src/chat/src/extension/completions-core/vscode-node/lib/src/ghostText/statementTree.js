...
exports.StatementTree = exports.StatementNode = void 0;
...
class StatementNode {
    ...
    dump(prefix1 = '', prefix2 = '') {
        ...
        this.children.forEach(child => {
            result.push(child.dump(`${prefix2}+- `, child.nextSibling === undefined ? `${prefix2}   ` : `${prefix2}|  `));
        });
        return result.join('\n');
    }
    dumpPath(prefix1 = '', prefix2 = '', forChild = false) {
        if (this.parent) {
...
class StatementTree {
    static isSupported(languageId) {
        return (JSStatementTree.languageIds.has(languageId) ||
    ...
    static create(languageId, text, startOffset, endOffset) {
    ...
    async parse() {
        if (!this.tree) {
            this.tree = await (0, parse_1.parseTreeSitter)(this.languageId, this.text);
        }
        return this.tree;
    }
    getStatementQuery(tree) {
        return this.getQuery(tree.getLanguage(), this.getStatementQueryText());
    }
    getQuery(language, queryText) {
...
class JSStatementNode extends StatementNode {
    ...
    isSingleLineIfStatement() {
        ...
        if (this.children.length === 1 && this.children[0].node.type !== 'statement_block') {
            return true;
        }
...
class JSStatementTree extends StatementTree {
    static { this.languageIds = new Set(['javascript', 'javascriptreact', 'jsx']); }
    createNode(node) {
...
class TSStatementTree extends StatementTree {
    static { this.languageIds = new Set(['typescript', 'typescriptreact']); }
    createNode(node) {
        return new JSStatementNode(node);
    }
    getStatementQueryText() {
...
class PyStatementTree extends StatementTree {
    static { this.languageIds = new Set(['python']); }
    createNode(node) {
...
class GoStatementTree extends StatementTree {
    static { this.languageIds = new Set(['go']); }
    createNode(node) {
...
class PhpStatementTree extends StatementTree {
    static { this.languageIds = new Set(['php']); }
    createNode(node) {
...
class RubyStatementNode extends StatementNode {
    static { this.compoundTypeNames = new Set(['if', 'case', 'while', 'until', 'for', 'begin', 'module', 'class', 'method']); }
    get isCompoundStatementType() {
...
class RubyStatementTree extends StatementTree {
    static { this.languageIds = new Set(['ruby']); }
    createNode(node) {
...
class JavaStatementNode extends StatementNode {
    ...
    isSingleLineIfStatement() {
        ...
        // Exclude if statements with braces so that block position is correct:
        // can have a single statement without braces
        if (this.children.length === 1 && this.children[0].node.type !== 'block') {
            return true;
        }
        return false;
    }
...
class JavaStatementTree extends StatementTree {
    ...
    static { this.languageIds = new Set(['java']); }
    createNode(node) {
...
class CSharpStatementTree extends StatementTree {
    static { this.languageIds = new Set(['csharp']); }
    createNode(node) {
...
class CStatementNode extends StatementNode {
    ...
    childrenFinished() {
        if (this.isSingleLineDeclarationStatement() || this.isSingleLineConceptDefinition()) {
            this.collapse();
        }
    }
    isSingleLineDeclarationStatement() {
...
class CStatementTree extends StatementTree {
    static { this.languageIds = new Set(['c', 'cpp']); }
    createNode(node) {
...
//# sourceMappingURL=statementTree.js.map