...
exports.getAllRecentEditsByTimestamp = getAllRecentEditsByTimestamp;
exports.findChangeSpan = findChangeSpan;
exports.getDiff = getDiff;
exports.unifiedDiff = unifiedDiff;
exports.findReplaceDiff = findReplaceDiff;
exports.editsOverlap = editsOverlap;
exports.updateEdits = updateEdits;
exports.buildIncomingEdit = buildIncomingEdit;
exports.trimOldFilesFromState = trimOldFilesFromState;
exports.recentEditsReducer = recentEditsReducer;
exports.summarizeEdit = summarizeEdit;
/**
 * Flatten all edits from a RecentEditMap into a single array,
 * sorted by timestamp (oldest first).
 */
function getAllRecentEditsByTimestamp(map) {
 ...
 */
function findChangeSpan(prevLines, newLines) {
    let start = 0;
 ...
 */
function getDiff(file, prevLines, newLines, start, endPrev, endNew, context) {
 ...
 */
function unifiedDiff(hunk, removeDeletedLines = false, insertionsBeforeDeletions = false, appendNoReplyMarker = false) {
    const out = [];
 ...
 */
function aidersDiff(hunk, removeDeletedLines = false) {
 ...
 */
function findReplaceDiff(hunk, removeDeletedLines = false) {
    const { before, removed, added, after } = hunk;
    ...
    if (removedWithWarning.length === 0) {
        ...
        res.push(`--- and deleted ${removedWithWarning.length} line${removedWithWarning.length === 1 ? '' : 's'} to make: ---`);
    }
    else {
 ...
 */
function editsOverlap(incoming, last, editMergeLineDistance) {
 ...
 */
function updateEdits(originalContent, existing, incoming, currentFileLines, config) {
    let edits = [...existing];
...
function buildIncomingEdit(file, prevLines, nextLines, span, config) {
 ...
 */
function trimOldFilesFromState(state, maxFiles) {
    ...
    const modifiedFilesInOrder = Object.entries(state)
        ...
        .sort(([aFile, a], [bFile, b]) => a.edits[a.edits.length - 1].timestamp - b.edits[b.edits.length - 1].timestamp);
 ...
 */
function recentEditsReducer(state = {}, file, newContents, config) {
    ...
    const { originalContent: updatedOriginal, edits: updatedEdits } = updateEdits(prev.originalContent, prev.edits, incoming, newLines, config);
 ...
 */
function summarizeEdit(edit, config) {
    const oldNonEmptyLines = edit.diff.removed.filter(x => x.trim().length > 0);
    const newNonEmptyLines = edit.diff.added.filter(x => x.trim().length > 0);
    let result;
    if (config.removeDeletedLines && newNonEmptyLines.length === 0) {
        ...
        result = unifiedDiff(edit.diff, config.removeDeletedLines, config.insertionsBeforeDeletions, config.appendNoReplyMarker);
    }
...
//# sourceMappingURL=recentEditsReducer.js.map