...
const x = textDocumentManager_1.TextDocumentManager;
console.log(x);
suite('VerboseBlockTrimmer', function () {
    test('.getCompletionTrimOffset() returns undefined when it is under the line limit', async function () {
        await testCompletionTrimming((0, ts_dedent_1.default) `
    ...
    async function testCompletionTrimming(textWithCompletion) {
        await testCompletionTrimmingWithTrimmer(textWithCompletion, blockTrimmer_1.VerboseBlockTrimmer);
    }
});
suite('TerseBlockTrimmer', function () {
    ...
    async function testCompletionTrimming(textWithCompletion, languageId = 'typescript') {
        await testCompletionTrimmingWithTrimmer(textWithCompletion, blockTrimmer_1.TerseBlockTrimmer, languageId);
    }
});
async function testCompletionTrimmingWithTrimmer(textWithCompletion, blockTrimmerType, languageId = 'typescript') {
    ...
    assert_1.default.strictEqual(actualOffset, expectedOffset, (0, ts_dedent_1.default) `
...
suite('getBlockPositionType()', function () {
    test('empty document returns NonBlock', async function () {
        await testPositionType(blockTrimmer_1.BlockPositionType.NonBlock, 'âš');
    });
    test('on a simple expression returns NonBlock', async function () {
        await testPositionType(blockTrimmer_1.BlockPositionType.NonBlock, 'âšconst x = 1;');
    });
    test('with an empty block returns EmptyBlock', async function () {
        await testPositionType(blockTrimmer_1.BlockPositionType.EmptyBlock, 'while (true) { âš }');
        await testPositionType(blockTrimmer_1.BlockPositionType.EmptyBlock, 'function example() { âš }');
    });
    test('at the end of a non-empty block returns BlockEnd', async function () {
        await testPositionType(blockTrimmer_1.BlockPositionType.BlockEnd, 'while (true) { x += 1; âš }');
    });
    test('mid-statement at the end of a non-empty block returns BlockEnd', async function () {
        await testPositionType(blockTrimmer_1.BlockPositionType.BlockEnd, 'while (true) { x += 1âš; }');
    });
    test('between statements within a block returns MidBlock', async function () {
        await testPositionType(blockTrimmer_1.BlockPositionType.MidBlock, 'while (true) { last = x; âš x += 1; }');
    });
    test('on a statement before the last within a block returns MidBlock', async function () {
        await testPositionType(blockTrimmer_1.BlockPositionType.MidBlock, 'while (true) { last = xâš; x += 1; }');
    });
    test('on a multi-line simple statement within a block before the last line returns MidBlock', async function () {
        await testPositionType(blockTrimmer_1.BlockPositionType.MidBlock, (0, ts_dedent_1.default) `
			if (true) {
				someFunction(
					arg1,
					arg2,
    ...
    test('on a multi-line simple statement within a block on the last line returns BlockEnd', async function () {
        await testPositionType(blockTrimmer_1.BlockPositionType.BlockEnd, (0, ts_dedent_1.default) `
			if (true) {
				someFunction(
					arg1,
					arg2,
    ...
    test('inside an empty block of a single-line if statement in JS returns EmptyBlock', async function () {
        await testPositionType(blockTrimmer_1.BlockPositionType.EmptyBlock, 'if (true) { âš }');
    });
    ...
    test('supports Go', async function () {
        await testPositionType(blockTrimmer_1.BlockPositionType.EmptyBlock, (0, ts_dedent_1.default) `
				package main
    ...
    async function testPositionType(expectedType, textWithCursor, languageId = 'typescript') {
        const cursorMarker = 'âš';
        ...
        const doc = (0, textDocument_1.createTextDocument)('file:///test.ts', languageId, 0, prefix + suffix);
        const pos = doc.positionAt(cursorPos);
        const actualType = await (0, blockTrimmer_1.getBlockPositionType)(doc, pos);
        assert_1.default.strictEqual(actualType, expectedType);
    }
});
//# sourceMappingURL=blockTrimmer.test.js.map