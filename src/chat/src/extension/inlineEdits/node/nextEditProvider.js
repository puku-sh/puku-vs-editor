...
const jumpToCursorPosition_1 = require("../common/jumpToCursorPosition");
...
let NextEditProvider = class NextEditProvider extends lifecycle_1.Disposable {
    ...
    async _getNextEditCanThrow(docId, context, triggerTime, shouldExpandEditWindow, parentTracer, logContext, cancellationToken, telemetryBuilder) {
        ...
        if (cachedEdit) {
            ...
            const providerRequestStartDateTime = (this._configService.getExperimentBasedConfig(configurationService_1.ConfigKey.Internal.InlineEditsDebounceUseCoreRequestTime, this._expService)
        ...
        if (error instanceof statelessNextEditProvider_1.NoNextEditReason.FetchFailure || error instanceof statelessNextEditProvider_1.NoNextEditReason.Unexpected) {
        ...
        else if (error instanceof statelessNextEditProvider_1.NoNextEditReason.NoSuggestions && error.nextCursorPosition !== undefined) {
            ...
            if (currentSelection) {
                ...
                const commandJumpToEditRange = {
                    command: jumpToCursorPosition_1.jumpToPositionCommandId,
                    title: "Jump to next edit",
                    arguments: [error.nextCursorPosition]
                };
                const edit = stringEdit_1.StringReplacement.replace(new offsetRange_1.OffsetRange(0, 0), ''); // should be no-op edit
                return new nextEditResult_1.NextEditResult(logContext.requestId, req, { edit, displayLocation, documentBeforeEdits: documentAtInvocationTime, action: commandJumpToEditRange });
            }
        }
        ...
        const currentSelection = selections.at(0);
        const transformer = documentAtInvocationTime.getTransformer();
        const currentCursorPosition = currentSelection ? transformer.getRange(currentSelection) : undefined;
        ...
        if (!showLabel || !currentCursorPosition || /* is close enough to not show label */ currentCursorPosition.startLineNumber - 2 <= editPosition.startLineNumber && editPosition.endLineNumber <= currentCursorPosition.endLineNumber + 5) { // TODO@ulugbekna
            ...
            nextEditResult = new nextEditResult_1.NextEditResult(logContext.requestId, req, { edit, showRangePreference, documentBeforeEdits: currentDocument, targetDocumentId });
        }
        else {
            ...
            const label = ['.ts', '.tsx', '.js', '.jsx'].includes(docId.extension) && this._configService.getExperimentBasedConfig(configurationService_1.ConfigKey.AdvancedExperimentalExperiments.InlineEditsNextCursorPredictionDisplayLine, this._expService)
            ...
            nextEditResult = new nextEditResult_1.NextEditResult(logContext.requestId, req, {
                edit, //: noopEdit,
                showRangePreference,
