...
exports.healReplaceStringParams = healReplaceStringParams;
exports.correctOldStringMismatch = correctOldStringMismatch;
exports.correctNewString = correctNewString;
exports.correctNewStringEscaping = correctNewStringEscaping;
exports.correctStringEscaping = correctStringEscaping;
exports._unescapeStringForGeminiBug = _unescapeStringForGeminiBug;
...
const commonTypes_js_1 = require("../../../platform/chat/common/commonTypes.js");
const markdown_js_1 = require("../../../util/common/markdown.js");
const strings_js_1 = require("../../../util/vs/base/common/strings.js");
const editFileToolUtils_js_1 = require("./editFileToolUtils.js");
function matchAndCount(currentContent, oldString, eol) {
    const r = (0, editFileToolUtils_js_1.findAndReplaceOne)(currentContent, oldString, '<none>', eol);
 ...
 */
async function healReplaceStringParams(model, currentContent, originalParams, // This is the EditToolParams from edit.ts, without \'corrected\'
eol, healEndpoint, token) {
    let finalNewString = originalParams.newString;
    const unescapeStringForGeminiBug = model?.family.includes('gemini') ? _unescapeStringForGeminiBug : (s) => s;
    const newStringPotentiallyEscaped = unescapeStringForGeminiBug(originalParams.newString) !==
        originalParams.newString;
    const expectedReplacements = originalParams.expected_replacements ?? 1;
    let finalOldString = originalParams.oldString;
    let occurrences = (0, strings_js_1.count)(currentContent, finalOldString);
    if (occurrences === expectedReplacements) {
        if (newStringPotentiallyEscaped) {
            finalNewString = await correctNewStringEscaping(healEndpoint, finalOldString, originalParams.newString, token);
        }
    }
        ...
        const unescapedOldStringAttempt = unescapeStringForGeminiBug(originalParams.oldString);
        occurrences = matchAndCount(currentContent, unescapedOldStringAttempt, eol);
        if (occurrences === expectedReplacements) {
            finalOldString = unescapedOldStringAttempt;
            if (newStringPotentiallyEscaped) {
                finalNewString = await correctNewString(healEndpoint, originalParams.oldString, // original old
            ...
            const llmCorrectedOldString = await correctOldStringMismatch(healEndpoint, currentContent, unescapedOldStringAttempt, token);
            ...
            if (llmOldOccurrences === expectedReplacements) {
                ...
                if (newStringPotentiallyEscaped) {
                    const baseNewStringForLLMCorrection = unescapeStringForGeminiBug(originalParams.newString);
                    finalNewString = await correctNewString(healEndpoint, originalParams.oldString, // original old
    ...
    const result = {
        ...
        occurrences: (0, strings_js_1.count)(currentContent, finalOldString), // Recalculate occurrences with the final oldString
    };
    return result;
}
// Define the expected JSON schema for the LLM response for oldString correction
const oldString_CORRECTION_SCHEMA = {
    type: 'object',
    properties: {
...
async function correctOldStringMismatch(healEndpoint, fileContent, problematicSnippet, token) {
    const prompt = `
...
const newString_CORRECTION_SCHEMA = {
    type: 'object',
    properties: {
        corrected_newString: {
            type: 'string',
            description: 'The original_newString adjusted to be a suitable replacement for the corrected_oldString, while maintaining the original intent of the change.',
        },
    },
 ...
 */
async function correctNewString(endpoint, originalOldString, correctedOldString, originalNewString, token) {
...
const CORRECT_newString_ESCAPING_SCHEMA = {
    type: 'object',
    properties: {
        corrected_newString_escaping: {
            type: 'string',
            description: 'The newString with corrected escaping, ensuring it is a proper replacement for the oldString, especially considering potential over-escaping issues from previous LLM generations.',
        },
    },
    required: ['corrected_newString_escaping'],
};
async function correctNewStringEscaping(geminiClient, oldString, potentiallyProblematicNewString, token) {
    const prompt = `
...
async function getJsonResponse(endpoint, prompt, schema, example, token) {
...
`.trim();
    const contents = [
        ...
        { role: prompt_tsx_1.Raw.ChatRole.System, content: [{ type: prompt_tsx_1.Raw.ChatCompletionContentPartKind.Text, text: 'You are an expert at analyzing files and patterns.' }] },
        { role: prompt_tsx_1.Raw.ChatRole.User, content: [{ type: prompt_tsx_1.Raw.ChatCompletionContentPartKind.Text, text: prompt }] },
    ];
    const result = await endpoint.makeChatRequest2({
        debugName: 'healStringReplace',
        messages: contents,
        finishedCb: undefined,
        location: commonTypes_js_1.ChatLocation.Other,
        enableRetryOnFilter: true,
    }, token);
    if (result.type !== commonTypes_js_1.ChatFetchResponseType.Success) {
        return undefined;
    }
    for (const block of (0, markdown_js_1.extractCodeBlocks)(result.value)) {
        try {
...
async function correctStringEscaping(potentiallyProblematicString, endpoint, token) {
    const prompt = `
...
function trimPairIfPossible(target, trimIfTargetTrims, currentContent, expectedReplacements) {
    ...
    if (target.length !== trimmedTargetString.length) {
        const trimmedTargetOccurrences = (0, strings_js_1.count)(currentContent, trimmedTargetString);
 ...
 */
function _unescapeStringForGeminiBug(inputString) {
    // Regex explanation:
...
//# sourceMappingURL=editFileHealing.js.map