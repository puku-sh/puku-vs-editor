...
let ChatEndpoint = ChatEndpoint_1 = class ChatEndpoint {
    ...
    get urlOrRequestMetadata() {
        ...
        return this.modelMetadata.urlOrRequestMetadata ??
            (this.useResponsesApi ? { type: pukuRequestTypes_1.RequestType.ChatResponses } : { type: pukuRequestTypes_1.RequestType.ChatCompletions });
    }
    get useResponsesApi() {
    ...
    async acceptChatPolicy() {
        ...
        try {
            const response = await (0, networking_1.postRequest)(this._fetcherService, this._telemetryService, this._capiClientService, { type: pukuRequestTypes_1.RequestType.ModelPolicy, modelId: this.model }, (await this._authService.getCopilotToken()).token, await (0, crypto_1.createRequestHMAC)(process.env.HMAC_SECRET), 'chat-policy', (0, uuid_1.generateUuid)(), {
                state: 'enabled'
            });
