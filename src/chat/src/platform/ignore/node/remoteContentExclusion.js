...
exports.RemoteContentExclusion = void 0;
 ...
 */
class RemoteContentExclusion {
    constructor(_gitService, _logService, _authService, _capiClientService, _fileSystemService, _workspaceService) {
        ...
        this._contentExclusionCache = new Map();
        ...
        this._ignoreGlobResultCache = new map_1.ResourceMap();
        // Map of the hash of file contents to the result of the regex check
        this._ignoreRegexResultCache = new Map();
        this._lastRuleFetch = 0;
        this._disposables = [];
        ...
        this._fileReadLimiter = new async_1.Limiter(10);
        this._disposables.push(this._fileReadLimiter);
    }
    async isIgnored(file, token = cancellation_1.CancellationToken.None) {
        ...
        if (this._contentExclusionFetchPromise) {
            await (0, async_1.raceCancellationError)(this._contentExclusionFetchPromise, token);
        }
        // Open the repository to get the repo associated with the file and the fetch urls
        const repo = await (0, async_1.raceCancellationError)(this._gitService.getRepositoryFetchUrls(file), token);
        ...
        if (this.shouldFetchContentExclusionRules(repoMetadata) || (Date.now() - this._lastRuleFetch > 30 * 60 * 1000)) {
            ...
            await (0, async_1.raceCancellationError)(this.makeContentExclusionRequest(), token);
        }
        ...
        for (const { patterns } of this._contentExclusionCache.values()) {
            for (const rule of patterns) {
                const matchesPattern = (0, minimatch_1.minimatch)(fileName, rule, minimatchConfig) || (0, minimatch_1.minimatch)(file.path, rule, minimatchConfig);
                if (matchesPattern) {
            ...
            if (ifAnyMatch.length > 0 || ifNoneMatch.length > 0) {
                if (!fileContents) {
                    try {
                        ...
                        const fileContentOrBuffer = await this._fileReadLimiter.queue(() => (0, fileSystemServiceImpl_1.readFileFromTextBufferOrFS)(this._fileSystemService, this._workspaceService, file, 1024));
                        fileContents = typeof fileContentOrBuffer === 'string' ? fileContentOrBuffer : new TextDecoder().decode(fileContentOrBuffer);
                        fileContentHash = await (0, crypto_1.createSha256Hash)(fileContents);
     ...
     */
    get isRegexContextExclusionsEnabled() {
        return [...this._contentExclusionCache.values()].some(({ ifAnyMatch, ifNoneMatch }) => ifAnyMatch.length > 0 || ifNoneMatch.length > 0);
    }
    /**
    ...
    async asMinimatchPatterns() {
        await this._contentExclusionFetchPromise;
        const patterns = Array.from(this._contentExclusionCache.values()).flatMap(({ patterns }) => patterns);
        return patterns;
    }
    dispose() {
    ...
    shouldFetchContentExclusionRules(repoInfo) {
        if (!repoInfo) {
            return false;
        }
     ...
     */
    async makeContentExclusionRequest() {
        if (this._contentExclusionFetchPromise) {
            await this._contentExclusionFetchPromise;
        }
        try {
    ...
    async _contentExclusionRequest() {
        ...
        const updateRulesForRepos = async (reposToFetch) => {
            const response = await capiClientService.makeRequest({
                headers: {
                    'Authorization': `token ${ghToken}`
                },
            }, { type: pukuRequestTypes_1.RequestType.ContentExclusion, repos: reposToFetch });
            if (!response.ok) {
            ...
            for (let j = 0; j < data.length; j++) {
                ...
                const ifAnyMatch = (0, arrays_1.coalesce)(data[j].rules.map(rule => rule.ifAnyMatch).flat()).map(pattern => stringToRegex(pattern));
                const ifNoneMatch = (0, arrays_1.coalesce)(data[j].rules.map(rule => rule.ifNoneMatch).flat()).map(pattern => stringToRegex(pattern));
    ...
    getRepositoryInfo(repo) {
        ...
        const fetchUrls = (0, arrays_1.coalesce)(repo.remoteFetchUrls.map(url => {
            if (!url) {
                return undefined;
            }
            // This can throw when the URL is something like a local file path which is a valid git remote
            try {
                return (0, gitService_1.normalizeFetchUrl)(url);
            }
            catch {
                return undefined;
            }
...
exports.RemoteContentExclusion = RemoteContentExclusion;
/**
...
//# sourceMappingURL=remoteContentExclusion.js.map