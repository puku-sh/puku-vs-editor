...
exports.PromptBody = exports.PromptHeader = exports.GithubPromptHeaderAttributes = exports.PromptHeaderAttributes = exports.ParsedPromptFile = exports.PromptFileParser = exports.PROMPT_NAME_REGEXP = void 0;
/*---------------------------------------------------------------------------------------------
...
const yaml_1 = require("../../../../../base/common/yaml");
const range_1 = require("../../../../../editor/common/core/range");
exports.PROMPT_NAME_REGEXP = /^[\p{L}\d_\-\.]+$/u;
class PromptFileParser {
    constructor() {
    }
    parse(uri, content) {
        const linesWithEOL = (0, strings_1.splitLinesIncludeSeparators)(content);
        ...
        if (linesWithEOL[0].match(/^---[\s\r\n]*$/)) {
            ...
            const range = new range_1.Range(2, 1, headerEndLine + 1, 1);
        ...
        if (bodyStartLine < linesWithEOL.length) {
            ...
            const range = new range_1.Range(bodyStartLine + 1, 1, linesWithEOL.length + 1, 1);
            body = new PromptBody(range, linesWithEOL, uri);
        }
        return new ParsedPromptFile(uri, header, body);
    }
}
exports.PromptFileParser = PromptFileParser;
...
exports.ParsedPromptFile = ParsedPromptFile;
...
})(PromptHeaderAttributes || (exports.PromptHeaderAttributes = PromptHeaderAttributes = {}));
var GithubPromptHeaderAttributes;
(function (GithubPromptHeaderAttributes) {
    GithubPromptHeaderAttributes.mcpServers = 'mcp-servers';
})(GithubPromptHeaderAttributes || (exports.GithubPromptHeaderAttributes = GithubPromptHeaderAttributes = {}));
class PromptHeader {
    ...
    get _parsedHeader() {
        if (this._parsed === undefined) {
            ...
            const node = (0, yaml_1.parse)(lines, yamlErrors);
            const attributes = [];
    ...
    asRange({ start, end }) {
        return new range_1.Range(this.range.startLineNumber + start.line, start.character + 1, this.range.startLineNumber + end.line, end.character + 1);
    }
    asValue(node) {
        switch (node.type) {
            case 'string':
    ...
    get name() {
        ...
        if (name && exports.PROMPT_NAME_REGEXP.test(name)) {
            return name;
        }
        return undefined;
    }
...
class PromptBody {
    ...
    getParsedBody() {
        if (this._parsed === undefined) {
            ...
            const bodyOffset = iterator_1.Iterable.reduce(iterator_1.Iterable.slice(this.linesWithEOL, 0, this.range.startLineNumber - 1), (len, line) => line.length + len, 0);
            for (let i = this.range.startLineNumber - 1, lineStartOffset = bodyOffset; i < this.range.endLineNumber - 1; i++) {
                ...
                for (const match of linkMatch) {
                    ...
                    const range = new range_1.Range(i + 1, linkStartOffset + 1, i + 1, linkEndOffset + 1);
                    fileReferences.push({ content: match[2], range, isMarkdownLink: true });
                    markdownLinkRanges.push(new range_1.Range(i + 1, match.index + 1, i + 1, match.index + match[0].length + 1));
                }
                ...
                for (const match of matches) {
                    ...
                    const fullRange = new range_1.Range(i + 1, match.index + 1, i + 1, match.index + fullMatch.length + 1);
                    if (markdownLinkRanges.some(mdRange => range_1.Range.areIntersectingOrTouching(mdRange, fullRange))) {
                        continue;
                    }
                    ...
                    const range = new range_1.Range(i + 1, startOffset + 1, i + 1, endOffset + 1);
            ...
            this._parsed = { fileReferences: fileReferences.sort((a, b) => range_1.Range.compareRangesUsingStarts(a.range, b.range)), variableReferences, bodyOffset };
        }
        return this._parsed;
    }
    getContent() {
    ...
    resolveFilePath(path) {
        try {
            if (path.startsWith('/')) {
                ...
                const dirName = (0, resources_1.dirname)(this.uri);
                return (0, resources_1.joinPath)(dirName, path);
            }
        }
        catch {
            return undefined;
