...
class AutorunObserver {
    ...
    constructor(_debugNameData, _runFn, _changeTracker, debugLocation) {
        ...
        (0, logging_1.getLogger)()?.handleAutorunCreated(this, debugLocation);
        this._run();
        (0, deps_1.trackDisposable)(this);
    }
    dispose() {
        if (this._disposed) {
            return;
        ...
        (0, logging_1.getLogger)()?.handleAutorunDisposed(this);
        (0, deps_1.markAsDisposed)(this);
    }
    _run() {
        ...
        try {
            if (!this._disposed) {
                (0, logging_1.getLogger)()?.handleAutorunStarted(this);
                ...
                try {
                    ...
                    (0, deps_1.onBugIndicatingError)(e);
                }
                finally {
                    this._isRunning = false;
                    if (delayedStore !== undefined) {
    ...
    endUpdate(_observable) {
        ...
        (0, deps_1.assertFn)(() => this._updateCount >= 0);
    }
    ...
    handleChange(observable, change) {
        if (this._isDependency(observable)) {
            (0, logging_1.getLogger)()?.handleAutorunDependencyChanged(this, observable, change);
            try {
    ...
    _ensureNoRunning() {
        if (!this._isRunning) {
            throw new deps_1.BugIndicatingError('The reader object cannot be used outside its compute function!');
        }
    }
    ...
    get store() {
        this._ensureNoRunning();
        if (this._disposed) {
            throw new deps_1.BugIndicatingError('Cannot access store after dispose');
        }
        if (this._store === undefined) {
            this._store = new deps_1.DisposableStore();
        }
        return this._store;
    }
    get delayedStore() {
        ...
        if (this._delayedStore === undefined) {
            this._delayedStore = new deps_1.DisposableStore();
        }
