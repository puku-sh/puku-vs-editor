...
export const reportFeedbackCommandId = 'puku.debug.inlineEdit.reportFeedback';
...
export class InlineEditDebugComponent extends Disposable {
	...
	constructor(
		...
		this._register(commands.registerCommand(pickProviderId, async (args: unknown) => {
			...
			const pick = await window.showWarningMessage(`Inline edits provider set to ${selectedProvider}. Reloading will undo this change. Set "puku.${ConfigKey.Internal.InlineEditsProviderId.id}": "${selectedProvider}" in your settings file to make the change persistent.`, 'Open settings (JSON)');
			if (!pick) { return; }

			await commands.executeCommand('workbench.action.openSettingsJson', { revealSetting: { key: `puku.${ConfigKey.Internal.InlineEditsProviderId.id}`, edit: true } });
		}));
	}
