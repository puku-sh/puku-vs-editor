...
async function copyWasmFiles(): Promise<void> {
	...
	try {
		...
		// Copy tree-sitter.wasm for AST chunking
		const treeSitterSource = path.join(__dirname, './node_modules/web-tree-sitter/tree-sitter.wasm');
		const treeSitterDest = path.join(destDir, 'tree-sitter.wasm');
		await copyFile(treeSitterSource, treeSitterDest);
		console.log('Copied tree-sitter.wasm');

		// Copy language-specific tree-sitter wasm files
		const wasmSourceDir = path.join(__dirname, './node_modules/@vscode/tree-sitter-wasm/wasm');
		const wasmFiles = await glob('tree-sitter-*.wasm', { cwd: wasmSourceDir, absolute: false });
