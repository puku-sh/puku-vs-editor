...
class CodeMain {
    ...
    async startup() {
        ...
        try {
            ...
            await instantiationService.invokeFunction(async (accessor) => {
                ...
                bufferLogger.logger = loggerService.createLogger('main', { name: localize(161, null) });
                // Lifecycle
    ...
    async claimInstance(logService, environmentMainService, lifecycleMainService, instantiationService, productService, retry) {
        ...
        try {
            ...
            try {
                ...
                if (!retry || isWindows || error.code !== 'ECONNREFUSED') {
                    if (error.code === 'EPERM') {
                        this.showStartupWarningDialog(localize(162, null, productService.nameShort), localize(163, null), productService);
                    }
                    throw error;
                }
            ...
            if (!environmentMainService.args.wait && !environmentMainService.args.status) {
                startupWarningDialogHandle = setTimeout(() => {
                    this.showStartupWarningDialog(localize(164, null, productService.nameShort), localize(165, null), productService);
                }, 10000);
            }
        ...
        if (environmentMainService.args.status) {
            console.log(localize(166, null, productService.nameShort));
    ...
    handleStartupDataDirError(environmentMainService, productService, error) {
        if (error.code === 'EACCES' || error.code === 'EPERM') {
            ...
            this.showStartupWarningDialog(localize(167, null), localize(168, null, toErrorMessage(error), directories.join('\n')), productService);
        }
    }
