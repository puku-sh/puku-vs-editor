...
let CodeApplication = class CodeApplication extends Disposable {
    ...
    async shouldBlockOpenable(openable, windowsMainService, dialogMainService) {
        ...
        if (isWorkspaceToOpen(openable)) {
            ...
            message = localize(153, null, openableUri.scheme === Schemas.file ? getPathLabel(openableUri, { os: OS, tildify: this.environmentMainService }) : openableUri.toString(true), this.productService.nameShort);
        }
        else if (isFolderToOpen(openable)) {
            openableUri = openable.folderUri;
            message = localize(154, null, openableUri.scheme === Schemas.file ? getPathLabel(openableUri, { os: OS, tildify: this.environmentMainService }) : openableUri.toString(true), this.productService.nameShort);
        }
        else {
            openableUri = openable.fileUri;
            message = localize(155, null, openableUri.scheme === Schemas.file ? getPathLabel(openableUri, { os: OS, tildify: this.environmentMainService }) : openableUri.toString(true), this.productService.nameShort);
        }
        ...
        const { response, checkboxChecked } = await dialogMainService.showMessageBox({
            ...
            message,
            detail: localize(158, null),
            checkboxLabel: openableUri.scheme === Schemas.file ? localize(159, null) : localize(160, null),
            cancelId: 1
        });
        if (response !== 0) {
