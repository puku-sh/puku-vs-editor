...
class RunCommands extends Action2 {
    constructor() {
        super({
            id: 'runCommands',
            title: nls.localize2(6821, "Run Commands"),
            f1: false,
            metadata: {
                description: nls.localize(6817, null),
                args: [
                    {
                        name: 'args',
                        schema: {
                            ...
                            properties: {
                                commands: {
                                    type: 'array',
                                    description: nls.localize(6818, null),
                                    items: {
                                        anyOf: [
                                            {
    ...
    async run(accessor, args) {
        ...
        if (!this._isCommandArgs(args)) {
            notificationService.error(nls.localize(6819, null));
            return;
        }
        if (args.commands.length === 0) {
            notificationService.warn(nls.localize(6820, null));
            return;
        }
