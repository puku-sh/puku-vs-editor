...
let TerminalInstance = class TerminalInstance extends Disposable {
    ...
    static getXtermConstructor(keybindingService, contextKeyService) {
        ...
        xtermConstructor = Promises.withAsyncBody(async (resolve) => {
            ...
            // Localize strings
            Terminal.strings.promptLabel = nls.localize(12644, null);
            Terminal.strings.tooMuchOutput = keybinding ? nls.localize(12645, null, keybinding.getLabel()) : nls.localize(12646, null);
            resolve(Terminal);
        });
        return xtermConstructor;
    }
            ...
            this._register(xterm.raw.onBell(() => {
                if (this._configurationService.getValue("terminal.integrated.enableBell" /* TerminalSettingId.EnableBell */) || this._configurationService.getValue("terminal.integrated.enableVisualBell" /* TerminalSettingId.EnableVisualBell */)) {
                    this.statusList.add({
                        ...
                        icon: Codicon.bell,
                        tooltip: nls.localize(12647, null)
        ...
        xterm.raw.attachCustomKeyEventHandler((event) => {
            ...
            if (resolveResult.kind === 2 /* ResultKind.KbFound */ && resolveResult.commandId && this._skipTerminalCommands.some(k => k === resolveResult.commandId) && !this._terminalConfigurationService.config.sendKeybindingsToShell) {
                ...
                if (this._storageService.getBoolean(SHOW_TERMINAL_CONFIG_PROMPT_KEY, -1 /* StorageScope.APPLICATION */, true) &&
                    ...
                    this._notificationService.prompt(Severity.Info, nls.localize(12648, null, this._productService.nameLong), [
                        {
                            label: nls.localize(12649, null),
                            run: () => {
    ...
    _createProcessManager() {
        ...
        this._register(processManager.onPtyDisconnect(() => {
            ...
            this.statusList.add({
                ...
                tooltip: nls.localize(12650, null)
            });
        }));
    ...
    async _createProcess() {
        ...
        if (activeWorkspaceRootUri) {
            ...
            if (!trusted) {
                this._onProcessExit({ message: nls.localize(12651, null) });
            }
        }
            ...
            this._onProcessExit({
                message: nls.localize(12652, null, this._cwd, this._userHome)
            });
        }
    ...
    async _trust() {
        return (await this._workspaceTrustRequestService.requestWorkspaceTrust({
            message: nls.localize(12656, null)
        })) === true;
    }
    ...
    _setAriaLabel(xterm, terminalId, title) {
        ...
        if (xterm && xterm.textarea) {
            if (title && title.length > 0) {
                labelParts.push(nls.localize(12657, null, terminalId, title));
            }
            else {
                labelParts.push(nls.localize(12658, null, terminalId));
            }
            const screenReaderOptimized = this._accessibilityService.isScreenReaderOptimized();
            if (!screenReaderOptimized) {
                labelParts.push(nls.localize(12659, null));
            }
            ...
            if (this._configurationService.getValue("accessibility.verbosity.terminal" /* AccessibilityVerbositySettingId.Terminal */) && accessibilityHelpKeybinding) {
                labelParts.push(nls.localize(12660, null, accessibilityHelpKeybinding));
            }
    ...
    _onEnvironmentVariableInfoChanged(info) {
        if (info.requiresAction) {
            this.xterm?.raw.textarea?.setAttribute('aria-label', nls.localize(12663, null, this._instanceId));
        }
    ...
    async changeColor(color, skipQuickPick) {
        ...
        quickPick.placeholder = nls.localize(12664, null);
        quickPick.show();
    ...
    switch (typeof exitCodeOrError) {
        case 'number': {
            ...
            if (processState === 4 /* ProcessState.KilledDuringLaunch */) {
                if (commandLine) {
                    message = nls.localize(12665, null, commandLine, code);
                }
                else {
                    message = nls.localize(12666, null, code);
                }
            }
            else {
                if (commandLine) {
                    message = nls.localize(12667, null, commandLine, code);
                }
                else {
                    message = nls.localize(12668, null, code);
                }
            }
            break;
        }
        case 'object': {
            ...
            message = nls.localize(12669, null, innerMessage);
            break;
        }
    }
    return { code, message };
