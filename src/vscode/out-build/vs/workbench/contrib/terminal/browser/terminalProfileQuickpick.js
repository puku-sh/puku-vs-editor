...
let TerminalProfileQuickpick = class TerminalProfileQuickpick {
    ...
    async _createAndShow(type) {
        ...
        let keyMods;
        const options = {
            placeHolder: type === 'createInstance' ? nls.localize(12712, null) : nls.localize(12713, null),
        ...
        if (configProfiles.length > 0) {
            quickPickItems.push({ type: 'separator', label: nls.localize(12716, null) });
        ...
        quickPickItems.push({ type: 'separator', label: nls.localize(12717, null) });
        ...
        if (autoDetectedProfiles.length > 0) {
            quickPickItems.push({ type: 'separator', label: nls.localize(12718, null) });
    ...
    async _isProfileSafe(profile) {
        ...
        return await new Promise(r => {
            ...
            const handle = this._notificationService.prompt(Severity.Warning, nls.localize(12719, null, `"${unsafePaths.join(',')}"`), [{
                    label: nls.localize(12720, null),
                    run: () => r(true)
                }, {
                    label: nls.localize(12721, null),
                    run: () => r(false)
                }]);
    ...
    _createProfileQuickPickItem(profile) {
        const buttons = [{
                ...
                tooltip: nls.localize(12722, null)
            }];
