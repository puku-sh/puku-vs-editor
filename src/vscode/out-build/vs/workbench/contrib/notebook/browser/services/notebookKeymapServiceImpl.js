...
let NotebookKeymapService = class NotebookKeymapService extends Disposable {
    ...
    promptForDisablingOtherKeymaps(newKeymap, oldKeymaps) {
        ...
        this.notificationService.prompt(Severity.Info, localize(10573, null, distinct(oldKeymaps.map(k => k.local.manifest.displayName)).map(name => `'${name}'`).join(', ')), [{
                label: localize(10574, null),
                run: () => onPrompt(true)
            }, {
                label: localize(10575, null),
                run: () => onPrompt(false)
            }]);
    }
};
