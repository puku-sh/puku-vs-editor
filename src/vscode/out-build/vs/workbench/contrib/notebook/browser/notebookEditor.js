    ...
    async setInput(input, options, context, token, noRetry) {
        try {
            ...
            if (model === null) {
                ...
                if (!knownProvider) {
                    throw new Error(localize(10473, null, input.viewType));
                }
                ...
                throw createEditorOpenError(new Error(localize(10474, null, input.viewType)), [
                    toAction({
                        id: 'workbench.notebook.action.installOrEnableMissing', label: extensionInfo
                            ? localize(10475, null, input.viewType)
                            : localize(10476, null, input.viewType),
                        run: async () => {
                    ...
                    toAction({
                        id: 'workbench.notebook.action.openAsText', label: localize(10477, null), run: async () => {
            ...
            if (e.fileOperationResult === 7 /* FileOperationResult.FILE_TOO_LARGE */) {
                ...
                if (e instanceof TooLargeFileOperationError) {
                    message = localize(10478, null, ByteSize.formatSize(e.size));
                }
                else {
                    message = localize(10479, null);
                }
            ...
            const error = createEditorOpenError(e instanceof Error ? e : new Error((e ? e.message : '')), [
                toAction({
                    id: 'workbench.notebook.action.openInTextEditor', label: localize(10480, null), run: async () => {
