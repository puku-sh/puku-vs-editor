...
registerAction2(class DeleteCellAction extends NotebookCellAction {
    ...
    async runWithContext(accessor, context) {
        ...
        if (runState === NotebookCellExecutionState.Executing && configService.getValue(NotebookSetting.confirmDeleteRunningCell)) {
            ...
            const primaryButton = localize(10242, null);
            confirmation = await dialogService.confirm({
                type: 'question',
                message: localize(10243, null),
                primaryButton: primaryButton,
                checkbox: {
                    label: localize(10244, null)
                }
            });
        }
        else {
...
registerAction2(class ChangeCellLanguageAction extends NotebookCellAction {
    constructor() {
        super({
            ...
            metadata: {
                description: localize(10248, null),
                args: [
                    {
                        name: 'range',
    ...
    async showLanguagePicker(accessor, context) {
        ...
        providerLanguages.forEach(languageId => {
            ...
            if (context.cell.cellKind === CellKind.Markup ? (languageId === 'markdown') : (languageId === context.cell.language)) {
                description = localize(10249, null, languageId);
            }
            else {
                description = localize(10250, null, languageId);
            }
        ...
        const autoDetectMode = {
            label: localize(10251, null)
        };
        const picks = [
            autoDetectMode,
            { type: 'separator', label: localize(10252, null) },
            ...topItems,
            { type: 'separator' },
            ...mainItems
        ];
        const selection = await quickInputService.pick(picks, { placeHolder: localize(10253, null) });
...
registerAction2(class DetectCellLanguageAction extends NotebookCellAction {
    constructor() {
        super({
            ...
            title: localize2(10261, "Accept Detected Language for Cell"),
            f1: true,
    ...
    async runWithContext(accessor, context) {
        ...
        if (detection) {
            ...
            notificationService.warn(localize(10254, null));
        }
    }
});
...
registerAction2(class SelectNotebookIndentation extends NotebookAction {
    constructor() {
        super({
            ...
            title: localize2(10262, 'Select Indentation'),
            f1: true,
    ...
    async showNotebookIndentationPicker(accessor, context) {
        ...
        if (!activeNotebook || activeNotebook.isDisposed) {
            return quickInputService.pick([{ label: localize(10255, null) }]);
        }
        if (activeNotebook.isReadOnly) {
            return quickInputService.pick([{ label: localize(10256, null) }]);
        }
        const picks = [
        ...
        picks.splice(3, 0, { type: 'separator', label: localize(10257, null) });
        picks.unshift({ type: 'separator', label: localize(10258, null) });
        const action = await quickInputService.pick(picks, { placeHolder: localize(10259, null), matchOnDetail: true });
        if (!action) {
            return;
        }
        action.run();
