...
let AccessibleView = class AccessibleView extends Disposable {
    ...
    configureKeybindings(unassigned) {
        ...
        quickPick.title = localize(4791, null);
        quickPick.placeholder = localize(4792, null);
        quickPick.show();
    ...
    _convertTokensToSymbols(tokens, symbols) {
        ...
        for (const token of tokens) {
            ...
            if (label) {
                symbols.push({ markdownToParse: label, label: localize(4793, null, token.type, label), ariaLabel: localize(4794, null, token.type, label), firstListItem });
    ...
    disableHint() {
        ...
        alert(localize(4795, null, this._currentProvider.verbositySettingKey));
    }
    ...
    _render(provider, container, showAccessibleViewHelp, updatedContent) {
        ...
        this._getTextModel(URI.from({ path: `accessible-view-${provider.id}`, scheme: Schemas.accessibleView, fragment: this._currentContent })).then((model) => {
            ...
            if (verbose && !showAccessibleViewHelp && hasActions) {
                actionsHint = provider.options.position ? localize(4796, null) : localize(4797, null);
            }
            let ariaLabel = provider.options.type === "help" /* AccessibleViewType.Help */ ? localize(4798, null) : localize(4799, null);
            this._title.textContent = ariaLabel;
            if (actionsHint && provider.options.type === "view" /* AccessibleViewType.View */) {
                ariaLabel = localize(4800, null, actionsHint);
            }
            else if (actionsHint) {
                ariaLabel = localize(4801, null, actionsHint);
            }
    ...
    _updateToolbar(providedActions, type) {
        this._toolbar.setAriaLabel(type === "help" /* AccessibleViewType.Help */ ? localize(4802, null) : localize(4803, null));
    ...
    _accessibleViewHelpDialogContent(providerHasSymbols) {
        ...
        const toolbarHint = localize(4804, null);
        const chatHints = this._getChatHints();
        let hint = localize(4805, null);
        if (navigationHint) {
            hint += ' - ' + navigationHint + '\n';
        }
        if (goToSymbolHint) {
    ...
    _getChatHints() {
        ...
        return [localize(4806, null, '<keybinding:workbench.action.chat.insertCodeBlock>'),
            localize(4807, null, '<keybinding:workbench.action.chat.insertIntoNewFile>'),
            localize(4808, null, '<keybinding:workbench.action.chat.runInTerminal>')].join('\n');
    }
    _navigationHint() {
        return localize(4809, null, `<keybinding:${"editor.action.accessibleViewNext" /* AccessibilityCommandId.ShowNext */}`, `<keybinding:${"editor.action.accessibleViewPrevious" /* AccessibilityCommandId.ShowPrevious */}>`);
    }
    _disableVerbosityHint(provider) {
        if (provider.options.type === "help" /* AccessibleViewType.Help */ && this._verbosityEnabled()) {
            return localize(4810, null, `<keybinding:${"editor.action.accessibleViewDisableHint" /* AccessibilityCommandId.DisableVerbosityHint */}>`);
        }
        return '';
    }
    _goToSymbolHint(providerHasSymbols) {
        ...
        return localize(4811, null, `<keybinding:${"editor.action.accessibleViewGoToSymbol" /* AccessibilityCommandId.GoToSymbol */}>`);
    }
    _configureUnassignedKbHint() {
        ...
        return localize(4812, null, keybindingToConfigureQuickPick);
    }
    _configureAssignedKbHint() {
        ...
        return localize(4813, null, keybindingToConfigureQuickPick);
    }
    ...
    _exitDialogHint(provider) {
        return this._verbosityEnabled() && !provider.options.position ? localize(4814, null) : '';
    }
    _readMoreHint(provider) {
        return provider.options.readMoreUrl ? localize(4815, null, `<keybinding:${"editor.action.accessibilityHelpOpenHelpLink" /* AccessibilityCommandId.AccessibilityHelpOpenHelpLink */}>`) : '';
    }
};
...
let AccessibleViewService = class AccessibleViewService extends Disposable {
    ...
    getOpenAriaHint(verbositySettingKey) {
        ...
        let hint = null;
        if (keybinding) {
            hint = localize(4816, null, keybinding);
        }
        else {
            hint = localize(4817, null);
        }
        return hint;
    }
    disableHint() {
...
let AccessibleViewSymbolQuickPick = class AccessibleViewSymbolQuickPick {
    ...
    show(provider) {
        ...
        const quickPick = disposables.add(this._quickInputService.createQuickPick());
        quickPick.placeholder = localize(4818, null);
        quickPick.title = localize(4819, null);
        const picks = [];
