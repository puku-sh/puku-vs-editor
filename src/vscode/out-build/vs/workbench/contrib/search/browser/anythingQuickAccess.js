...
let AnythingQuickAccessProvider = class AnythingQuickAccessProvider extends PickerQuickAccessProvider {
    ...
    doGetPicks(filter, options, disposables, token) {
        ...
        if (this.pickState.isQuickNavigating) {
            if (picks.length > 0) {
                picks.push({ type: 'separator', label: localize(11679, null) });
            }
            ...
            if (historyEditorPicks.length !== 0) {
                picks.push({ type: 'separator', label: localize(11680, null) });
        ...
        return {
            ...
            additionalPicks: (async () => {
                ...
                return additionalPicks.length > 0 ? [
                    { type: 'separator', label: this.configuration.includeSymbols ? localize(11681, null) : localize(11682, null) },
                    ...additionalPicks
                ] : [];
            })(),
    ...
    getHelpPicks(query, token, runOptions) {
        ...
        const providers = this.lazyRegistry.value.getQuickAccessProviders()
            ...
            return {
                ...
                ariaLabel: localize(11683, null, label, helpEntry.description),
                accept: () => {
    ...
    createAnythingPick(resourceOrEditor, configuration) {
        ...
        return {
            resource,
            label,
            ariaLabel: isDirty ? localize(11688, null, labelAndDescription) : labelAndDescription,
            description,
