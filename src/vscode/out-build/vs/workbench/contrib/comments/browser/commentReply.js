...
let CommentReply = class CommentReply extends Disposable {
    ...
    setCommentEditorDecorations() {
        ...
        const placeholder = hasExistingComments
            ? (this._commentOptions?.placeHolder || nls.localize(6843, null))
            : (this._commentOptions?.placeHolder || nls.localize(6844, null));
    ...
    createReplyButton(commentEditor, commentForm) {
        ...
        this._register(this.hoverService.setupManagedHover(getDefaultHoverDelegate('mouse'), this._reviewThreadReplyButton, this._commentOptions?.prompt || nls.localize(6845, null)));
        this._reviewThreadReplyButton.textContent = this._commentOptions?.prompt || nls.localize(6846, null);
