...
export const COMMENTS_VIEW_TITLE = nls.localize2(6918, "Comments");
...
let CommentNodeRenderer = class CommentNodeRenderer {
    ...
    getCountString(commentCount) {
        if (commentCount > 2) {
            return nls.localize(6909, null, commentCount - 1);
        }
        else if (commentCount === 2) {
            return nls.localize(6910, null);
        }
        else {
            return nls.localize(6911, null);
        }
    }
    getRenderedComment(commentBody) {
        ...
        for (let i = 0; i < images.length; i++) {
            ...
            textDescription.textContent = image.alt ? nls.localize(6912, null, image.alt) : nls.localize(6913, null);
    ...
    renderElement(node, index, templateData) {
        ...
        if (node.element.threadRelevance === CommentThreadApplicability.Outdated) {
            ...
            templateData.threadMetadata.relevance.innerText = nls.localize(6914, null);
        ...
        if (node.element.range) {
            if (node.element.range.startLineNumber === node.element.range.endLineNumber) {
                templateData.threadMetadata.range.textContent = nls.localize(6915, null, node.element.range.startLineNumber);
            }
            else {
                templateData.threadMetadata.range.textContent = nls.localize(6916, null, node.element.range.startLineNumber, node.element.range.endLineNumber);
            }
        }
        ...
        templateData.repliesMetadata.lastReplyDetail.textContent = nls.localize(6917, null, lastComment.userName);
