...
let CommentsPanel = class CommentsPanel extends FilterViewPane {
    ...
    constructor(options, instantiationService, viewDescriptorService, editorService, configurationService, contextKeyService, contextMenuService, keybindingService, openerService, themeService, commentService, hoverService, uriIdentityService, storageService, pathService) {
        ...
        super({
            ...options,
            filterOptions: {
                placeholder: nls.localize(6919, null),
                ariaLabel: nls.localize(6920, null),
    ...
    updateFilter() {
        ...
        this.filterWidget.updateBadge(total === filtered || total === 0 ? undefined : nls.localize(6921, null, filtered, total));
    ...
    makeCommentLocationLabel(file, range) {
        ...
        if (!range) {
            return nls.localize(6922, null, fileLabel);
        }
        if (range.startLineNumber === range.endLineNumber) {
            return nls.localize(6923, null, range.startLineNumber, range.startColumn, fileLabel);
        }
        else {
            return nls.localize(6924, null, range.startLineNumber, range.endLineNumber, fileLabel);
        }
    }
    ...
    getScreenReaderInfoForNode(element, forAriaLabel) {
        ...
        if (forAriaLabel && this.configurationService.getValue("accessibility.verbosity.comments" /* AccessibilityVerbositySettingId.Comments */)) {
            ...
            accessibleViewHint = kbLabel ? nls.localize(6925, null, kbLabel) : nls.localize(6926, null);
        }
        ...
        if (element.threadRelevance === CommentThreadApplicability.Outdated) {
            return nls.localize(6927, null, labelInfo.userName, labelInfo.bodyLabel, labelInfo.locationLabel, labelInfo.replyCountLabel, relevantLines) + replies + accessibleViewHint;
        }
        else {
            return nls.localize(6928, null, labelInfo.userName, labelInfo.bodyLabel, labelInfo.locationLabel, labelInfo.replyCountLabel, relevantLines) + replies + accessibleViewHint;
        }
    }
    getRepliesAsString(node, forAriaLabel) {
        ...
        return '\n' + node.replies.map(reply => nls.localize(6929, null, reply.comment.userName, (typeof reply.comment.body === 'string') ? reply.comment.body : reply.comment.body.value)).join('\n');
    }
    getReplyCountAsString(node, forAriaLabel) {
        return node.replies.length && !forAriaLabel ? nls.localize(6930, null, node.replies.length) : '';
    }
    createTree() {
            ...
            accessibilityProvider: {
                getAriaLabel: (element) => {
                    if (element instanceof CommentsModel) {
                        return nls.localize(6931, null);
                    }
                    if (element instanceof ResourceWithCommentThreads) {
                        return nls.localize(6932, null, basename(element.resource), element.resource.fsPath);
                    }
