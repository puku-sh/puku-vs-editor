...
let TerminalLinkManager = class TerminalLinkManager extends DisposableStore {
            ...
            activate: async (event, text) => {
                ...
                if (terminalConfigurationService.config.allowedLinkSchemes.indexOf(scheme) === -1) {
                    const userAllowed = await new Promise((resolve) => {
                        notificationService.prompt(Severity.Warning, nls.localize(13291, null, scheme), [
                            {
                                label: nls.localize(13292, null, scheme),
                                run: () => {
    ...
    _getLinkHoverString(uri, label) {
        ...
        if (editorConf.multiCursorModifier === 'ctrlCmd') {
            if (isMacintosh) {
                clickLabel = nls.localize(13293, null);
            }
            else {
                clickLabel = nls.localize(13294, null);
            }
        }
        else {
            if (isMacintosh) {
                clickLabel = nls.localize(13295, null);
            }
            else {
                clickLabel = nls.localize(13296, null);
            }
        }
        let fallbackLabel = nls.localize(13297, null);
        try {
            if (this._tunnelService.canTunnel(URI.parse(uri))) {
                fallbackLabel = nls.localize(13298, null);
            }
        }
        catch {
