...
let TerminalLinkQuickpick = class TerminalLinkQuickpick extends DisposableStore {
    ...
    async show(instance, links) {
        ...
        const picks = [];
        if (webPicks) {
            picks.push({ type: 'separator', label: localize(13300, null) });
            picks.push(...webPicks);
        }
        if (filePicks) {
            picks.push({ type: 'separator', label: localize(13301, null) });
            picks.push(...filePicks);
        }
        if (folderPicks) {
            picks.push({ type: 'separator', label: localize(13302, null) });
            picks.push(...folderPicks);
        }
        if (wordPicks) {
            picks.push({ type: 'separator', label: localize(13303, null) });
        ...
        pick.items = picks;
        pick.placeholder = localize(13304, null);
        ...
        if (!usingAllLinks) {
            disposables.add(Event.once(pick.onDidChangeValue)(async () => {
                ...
                const picks = [];
                if (webPicks) {
                    picks.push({ type: 'separator', label: localize(13305, null) });
                    picks.push(...webPicks);
                }
                if (filePicks) {
                    picks.push({ type: 'separator', label: localize(13306, null) });
                    picks.push(...filePicks);
                }
                if (folderPicks) {
                    picks.push({ type: 'separator', label: localize(13307, null) });
                    picks.push(...folderPicks);
                }
                if (wordPicks) {
                    picks.push({ type: 'separator', label: localize(13308, null) });
                    picks.push(...wordPicks);
                }
                pick.items = picks;
            }));
