...
let KeybindingsEditor = class KeybindingsEditor extends EditorPane {
    ...
    constructor(group, telemetryService, themeService, keybindingsService, contextMenuService, keybindingEditingService, contextKeyService, notificationService, clipboardService, instantiationService, editorService, storageService, configurationService, accessibilityService) {
        ...
        this.recordKeysAction = this._register(new Action(KEYBINDINGS_EDITOR_COMMAND_RECORD_SEARCH_KEYS, localize(10739, null), ThemeIcon.asClassName(keybindingsRecordKeysIcon)));
        this.recordKeysAction.checked = false;
        this.sortByPrecedenceAction = this._register(new Action(KEYBINDINGS_EDITOR_COMMAND_SORTBY_PRECEDENCE, localize(10740, null), ThemeIcon.asClassName(keybindingsSortIcon)));
    ...
    createHeader(parent) {
        ...
        const fullTextSearchPlaceholder = localize(10741, null);
        const keybindingsSearchPlaceholder = localize(10742, null);
        const clearInputAction = this._register(new Action(KEYBINDINGS_EDITOR_COMMAND_CLEAR_SEARCH_RESULTS, localize(10743, null), ThemeIcon.asClassName(preferencesClearInputIcon), false, async () => this.clearSearchResults()));
    ...
    createRecordingBadge(container) {
        ...
        recordingBadge.textContent = localize(10744, null);
    ...
    renderKeybindingsEntries(reset, preserveFocus) {
        if (this.keybindingsEditorModel) {
            ...
            this.accessibilityService.alert(localize(10749, null, keybindingsEntries.length));
    ...
    getAriaLabel(keybindingsEntries) {
        if (this.sortByPrecedenceAction.checked) {
            return localize(10750, null, keybindingsEntries.length);
        }
        else {
            return localize(10751, null, keybindingsEntries.length);
        }
    }
    ...
    createDefineKeybindingAction(keybindingItemEntry) {
        return {
            label: keybindingItemEntry.keybindingItem.keybinding ? localize(10752, null) : localize(10753, null),
            enabled: true,
    ...
    onKeybindingEditingError(error) {
        this.notificationService.error(typeof error === 'string' ? error : localize(10762, null, `${error}`));
    }
};
...
let ActionsColumnRenderer = class ActionsColumnRenderer {
    ...
    createEditAction(keybindingItemEntry) {
        ...
        return {
            ...
            enabled: true,
            id: 'editKeybinding',
            tooltip: keybinding ? localize(10763, null, `(${keybinding.getLabel()})`) : localize(10764, null),
    ...
    createAddAction(keybindingItemEntry) {
        ...
        return {
            ...
            enabled: true,
            id: 'addKeybinding',
            tooltip: keybinding ? localize(10765, null, `(${keybinding.getLabel()})`) : localize(10766, null),
            run: () => this.keybindingsEditor.defineKeybinding(keybindingItemEntry, false)
        };
    }
...
let CommandColumnRenderer = class CommandColumnRenderer {
    ...
    renderElement(keybindingItemEntry, index, templateData) {
        ...
        const title = keybindingItem.commandLabel ? localize(10767, null, keybindingItem.commandLabel, keybindingItem.command) : keybindingItem.command;
...
let SourceColumnRenderer = class SourceColumnRenderer {
    ...
    renderElement(keybindingItemEntry, index, templateData) {
        ...
        if (isString(keybindingItemEntry.keybindingItem.source)) {
            ...
            templateData.sourceColumnHover.update(localize(10768, null, extensionLabel));
...
class AccessibilityProvider {
    ...
    getWidgetAriaLabel() {
        return localize(10769, null);
    }
    getAriaLabel({ keybindingItem }) {
        const ariaLabel = [
            keybindingItem.commandLabel ? keybindingItem.commandLabel : keybindingItem.command,
            keybindingItem.keybinding?.getAriaLabel() || localize(10770, null),
            keybindingItem.when ? keybindingItem.when : localize(10771, null),
        ...
        if (this.configurationService.getValue("accessibility.verbosity.keybindingsEditor" /* AccessibilityVerbositySettingId.KeybindingsEditor */)) {
            const kbEditorAriaLabel = localize(10772, null);
