...
const $ = DOM.$;
const searchBoxLabel = localize(10890, null);
const SEARCH_TOC_BEHAVIOR_KEY = 'workbench.settings.settingsSearchTocBehavior';
const SHOW_AI_RESULTS_ENABLED_LABEL = localize(10891, null);
const SHOW_AI_RESULTS_DISABLED_LABEL = localize(10892, null);
...
let SettingsEditor2 = class SettingsEditor2 extends EditorPane {
    ...
    createHeader(parent) {
        ...
        const clearInputAction = this._register(new Action(SETTINGS_EDITOR_COMMAND_CLEAR_SEARCH_RESULTS, localize(10893, null), ThemeIcon.asClassName(preferencesClearInputIcon), false, async () => this.clearSearchResults()));
        ...
        const filterAction = this._register(new Action(SETTINGS_EDITOR_COMMAND_SUGGEST_FILTERS, localize(10894, null), ThemeIcon.asClassName(preferencesFilterIcon)));
    ...
    toggleAiSearch() {
        if (this.searchInputActionBar && this.showAiResultsAction && this.searchInputActionBar.hasAction(this.showAiResultsAction)) {
            if (!this.showAiResultsAction.enabled) {
                aria.status(localize(10895, null));
            }
    ...
    createBody(parent) {
        ...
        this.noResultsMessage.innerText = localize(10896, null);
        this.clearFilterLinkContainer = $('span.clear-search-filters');
        this.clearFilterLinkContainer.textContent = ' - ';
        const clearFilterLink = DOM.append(this.clearFilterLinkContainer, $('a.pointer.prominent', { tabindex: 0 }, localize(10897, null)));
    ...
    createTOC(container) {
        ...
        this.tocTree = this._register(this.instantiationService.createInstance(TOCTree, DOM.append(container, $('.settings-toc-wrapper', {
            'role': 'navigation',
            'aria-label': localize(10898, null),
        })), this.viewState));
        this.tocTreeDisposed = false;
    ...
    renderResultCountMessages(showAiResultsMessage) {
        ...
        if (!this.searchResultModel) {
            ...
            if (showAiResultsMessage) {
                switch (count) {
                    case 0:
                        resultString = localize(10900, null);
                        break;
                    case 1:
                        resultString = localize(10901, null);
                        break;
                    default: resultString = localize(10902, null, count);
                }
            }
            else {
                switch (count) {
                    case 0:
                        resultString = localize(10903, null);
                        break;
                    case 1:
                        resultString = localize(10904, null);
                        break;
                    default: resultString = localize(10905, null, count);
                }
            }
...
let SyncControls = class SyncControls extends Disposable {
    constructor(window, container, commandService, userDataSyncService, userDataSyncEnablementService, telemetryService) {
        ...
        this.turnOnSyncButton.label = localize(10906, null);
