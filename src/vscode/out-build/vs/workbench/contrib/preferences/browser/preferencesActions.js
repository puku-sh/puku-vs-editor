...
let ConfigureLanguageBasedSettingsAction = class ConfigureLanguageBasedSettingsAction extends Action {
    ...
    static { this.LABEL = nls.localize2(10841, "Configure Language Specific Settings..."); }
    ...
    async run() {
        ...
        const picks = languages.map(({ languageName, languageId }) => {
            const description = nls.localize(10839, null, languageId);
        ...
        await this.quickInputService.pick(picks, { placeHolder: nls.localize(10840, null) })
            .then(pick => {
            if (pick) {
