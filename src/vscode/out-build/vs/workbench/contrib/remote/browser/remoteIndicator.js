...
let RemoteStatusIndicator = class RemoteStatusIndicator extends Disposable {
    ...
    registerActions() {
        const category = nls.localize2(11259, "Remote");
        // Show Remote Menu
        const that = this;
        this._register(registerAction2(class extends Action2 {
            constructor() {
                super({
                    ...
                    category,
                    title: nls.localize2(11260, "Show Remote Menu"),
                    f1: true,
                    keybinding: {
            ...
            this._register(registerAction2(class extends Action2 {
                constructor() {
                    super({
                        ...
                        category,
                        title: nls.localize2(11261, "Close Remote Connection"),
                        f1: true,
            ...
            this._register(registerAction2(class extends Action2 {
                constructor() {
                    super({
                        ...
                        category,
                        title: nls.localize2(11262, "Install Remote Development Extensions"),
                        f1: true
                    });
    ...
    updateRemoteStatusIndicator() {
        ...
        if (this.remoteAuthority) {
            ...
            switch (this.connectionState) {
                case 'initializing':
                    this.renderRemoteStatusIndicator(nls.localize(11238, null), nls.localize(11239, null), undefined, true /* progress */);
                    break;
                case 'reconnecting':
                    this.renderRemoteStatusIndicator(`${nls.localize(11240, null, truncate(hostLabel, RemoteStatusIndicator_1.REMOTE_STATUS_LABEL_MAX_LENGTH))}`, undefined, undefined, true /* progress */);
                    break;
                case 'disconnected':
                    this.renderRemoteStatusIndicator(`$(alert) ${nls.localize(11241, null, truncate(hostLabel, RemoteStatusIndicator_1.REMOTE_STATUS_LABEL_MAX_LENGTH))}`);
                    break;
                default: {
                    ...
                    if (hostNameTooltip) {
                        ...
                        tooltip.appendText(nls.localize(11242, null, hostLabel));
                    }
        ...
        if (this.virtualWorkspaceLocation) {
            ...
            if (workspaceLabel) {
                ...
                if (hostNameTooltip) {
                    ...
                    tooltip.appendText(nls.localize(11243, null, workspaceLabel));
                }
                if (!isWeb || this.remoteAuthority) {
                    tooltip.appendMarkdown('\n\n');
                    tooltip.appendMarkdown(nls.localize(11244, null, `command:${LIST_WORKSPACE_UNSUPPORTED_EXTENSIONS_COMMAND_ID}`));
                }
        ...
        this.renderRemoteStatusIndicator(RemoteStatusIndicator_1.DEFAULT_REMOTE_STATUS_LABEL, nls.localize(11245, null));
        return;
    }
    ...
    withNetworkStatus(initialText, initialTooltip, showProgress) {
        ...
        switch (this.networkState) {
            case 'offline': {
                const offlineMessage = nls.localize(11247, null);
                text = textWithAlert();
            ...
            case 'high-latency':
                text = textWithAlert();
                tooltip = this.appendTooltipLine(tooltip, nls.localize(11248, null, remoteConnectionLatencyMeasurer.latency?.current?.toFixed(2), remoteConnectionLatencyMeasurer.latency?.average?.toFixed(2)));
                break;
        }
        ...
        try {
            ...
            if (!this.lifecycleService.willShutdown) {
                const { confirmed } = await this.dialogService.confirm({
                    type: Severity.Error,
                    message: nls.localize(11249, null, remoteLabel),
                    detail: error && !isCancellationError(error) ? toErrorMessage(error) : undefined,
                    primaryButton: nls.localize(11250, null)
                });
                if (confirmed) {
    ...
    showRemoteMenu() {
                ...
                for (const metadata of this.remoteExtensionMetadata) {
                    if (!metadata.installed && metadata.isPlatformCompatible) {
                        ...
                        const buttons = [{
                                ...
                                tooltip: nls.localize(11251, null)
                            }];
                        notInstalledItems.push({ type: 'item', id: metadata.id, label: label, buttons: buttons });
                    }
                }
                items.push({
                    type: 'separator', label: nls.localize(11252, null)
                });
            ...
            if (RemoteStatusIndicator_1.SHOW_CLOSE_REMOTE_COMMAND_ID) {
                if (this.remoteAuthority) {
                    items.push({
                        type: 'item',
                        id: RemoteStatusIndicator_1.CLOSE_REMOTE_COMMAND_ID,
                        label: nls.localize(11253, null)
                    });
                    if (this.connectionState === 'disconnected') {
                        items.push({
                            type: 'item',
                            id: ReloadWindowAction.ID,
                            label: nls.localize(11254, null)
                        });
                    }
                }
                else if (this.virtualWorkspaceLocation) {
        ...
        quickPick.placeholder = nls.localize(11256, null);
        ...
        disposables.add(Event.once(quickPick.onDidAccept)((async (_) => {
            ...
            if (selectedItems.length === 1) {
                ...
                if (remoteExtension) {
                    quickPick.items = [];
                    quickPick.busy = true;
                    quickPick.placeholder = nls.localize(11257, null);
                    try {
...
Registry.as(ConfigurationExtensions.Configuration)
    ...
    properties: {
        'workbench.remoteIndicator.showExtensionRecommendations': {
            type: 'boolean',
            markdownDescription: nls.localize(11258, null),
            default: true
        },
    }
});
