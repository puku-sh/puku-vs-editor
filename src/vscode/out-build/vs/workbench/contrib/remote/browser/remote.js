...
class HelpModel extends Disposable {
    ...
    updateItems() {
        ...
        if (documentation.length) {
            ...
            const documentationHelpItem = this.items?.find(item => item.icon === icons.documentationIcon) ?? new HelpItem(icons.documentationIcon, nls.localize(11182, null), helpItemValues, this.quickInputService, this.environmentService, this.openerService, this.remoteExplorerService, this.workspaceContextService);
        ...
        if (issues.length) {
            ...
            const reviewIssuesHelpItem = this.items?.find(item => item.icon === icons.reviewIssuesIcon) ?? new HelpItem(icons.reviewIssuesIcon, nls.localize(11183, null), helpItemValues, this.quickInputService, this.environmentService, this.openerService, this.remoteExplorerService, this.workspaceContextService);
...
class HelpItemBase {
    ...
    async handleClick() {
        ...
        if (this.values.length > 1) {
            ...
            if (actions.length) {
                const action = await this.quickInputService.pick(actions, { placeHolder: nls.localize(11185, null) });
                if (action) {
...
let HelpPanel = class HelpPanel extends ViewPane {
    ...
    static { this.TITLE = nls.localize2(11196, "Help and feedback"); }
    ...
    renderBody(container) {
        ...
        this.tree = this.instantiationService.createInstance((WorkbenchAsyncDataTree), 'RemoteHelp', treeContainer, new HelpTreeVirtualDelegate(), [new HelpTreeRenderer()], new HelpDataSource(), {
            accessibilityProvider: {
                ...
                getWidgetAriaLabel: () => nls.localize(11186, null)
            }
        });
...
let RemoteViewPaneContainer = class RemoteViewPaneContainer extends FilterViewPaneContainer {
    ...
    getTitle() {
        const title = nls.localize(11187, null);
        return title;
    }
};
...
Registry.as(Extensions.ViewContainersRegistry).registerViewContainer({
    id: VIEWLET_ID,
    title: nls.localize2(11197, "Remote Explorer"),
...
class ReconnectionTimer {
    ...
    _render() {
        ...
        if (remainingTime === 1) {
            this._parent.report(nls.localize(11188, null, remainingTime));
        }
        else {
            this._parent.report(nls.localize(11189, null, remainingTime));
        }
    }
}
/**
    ...
    constructor(remoteAgentService, progressService, dialogService, commandService, quickInputService, logService, environmentService, telemetryService) {
        ...
        if (connection) {
            ...
            const reconnectButton = {
                label: nls.localize(11190, null),
                callback: () => {
                    reconnectWaitEvent?.skipWait();
                }
            };
            const reloadButton = {
                label: nls.localize(11191, null),
                callback: () => {
                ...
                switch (e.type) {
                    case 0 /* PersistentConnectionEventType.ConnectionLost */:
                        ...
                        if (visibleProgress || e.millisSinceLastIncomingData > DISCONNECT_PROMPT_TIME) {
                            ...
                            visibleProgress.report(nls.localize(11192, null));
                        }
                        break;
                    ...
                    case 2 /* PersistentConnectionEventType.ReconnectionRunning */:
                        ...
                        if (visibleProgress || e.millisSinceLastIncomingData > DISCONNECT_PROMPT_TIME) {
                            ...
                            visibleProgress.report(nls.localize(11193, null));
                    ...
                    case 3 /* PersistentConnectionEventType.ReconnectionPermanentFailure */:
                        ...
                        if (e.handled) {
                            ...
                            dialogService.confirm({
                                type: Severity.Error,
                                message: nls.localize(11194, null),
                                primaryButton: nls.localize(11195, null)
                            }).then(result => {
                                if (result.confirmed) {
                                    commandService.executeCommand(ReloadWindowAction.ID);
                                }
