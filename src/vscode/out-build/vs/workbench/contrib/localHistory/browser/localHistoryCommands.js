...
const LOCAL_HISTORY_CATEGORY = localize2(9328, 'Local History');
const CTX_LOCAL_HISTORY_ENABLED = ContextKeyExpr.has('config.workbench.localHistory.enabled');
//#region Compare with File
export const COMPARE_WITH_FILE_LABEL = localize2(9329, 'Compare with File');
...
registerAction2(class extends Action2 {
    constructor() {
        super({
            ...
            title: localize2(9330, 'Compare with Previous'),
            menu: {
                id: MenuId.TimelineItemContext,
                group: '1_compare',
                order: 2,
...
registerAction2(class extends Action2 {
    constructor() {
        super({
            ...
            title: localize2(9331, 'Select for Compare'),
            menu: {
                id: MenuId.TimelineItemContext,
...
registerAction2(class extends Action2 {
    constructor() {
        super({
            ...
            title: localize2(9332, 'Compare with Selected'),
            menu: {
                id: MenuId.TimelineItemContext,
                group: '2_compare_with',
                order: 1,
...
registerAction2(class extends Action2 {
    constructor() {
        super({
            ...
            title: localize2(9333, 'Show Contents'),
            menu: {
                id: MenuId.TimelineItemContext,
                group: '3_contents',
                order: 1,
...
const RESTORE_CONTENTS_LABEL = localize2(9334, 'Restore Contents');
registerAction2(class extends Action2 {
    constructor() {
        super({
...
const restoreSaveSource = SaveSourceRegistry.registerSource('localHistoryRestore.source', localize(9308, null));
async function restore(accessor, item) {
    ...
    if (entry) {
        ...
        const { confirmed } = await dialogService.confirm({
            type: 'warning',
            message: localize(9309, null, basename(entry.workingCopy.resource)),
            detail: localize(9310, null),
            primaryButton: localize(9311, null)
        });
        if (!confirmed) {
            return;
        }
        ...
        try {
            ...
            await dialogService.error(localize(9312, null, basename(entry.workingCopy.resource)), toErrorMessage(error));
            return;
        }
...
registerAction2(class extends Action2 {
    constructor() {
        super({
            ...
            title: localize2(9335, 'Find Entry to Restore'),
            f1: true,
    ...
    async run(accessor) {
        ...
        resourcePicker.placeholder = localize(9313, null);
        ...
        entryPicker.placeholder = localize(9314, null);
...
MenuRegistry.appendMenuItem(MenuId.TimelineTitle, { command: { id: 'workbench.action.localHistory.restoreViaPicker', title: localize2(9336, 'Local History: Find Entry to Restore...') }, group: 'submenu', order: 1, when: CTX_LOCAL_HISTORY_ENABLED });
//#endregion
//#region Rename
registerAction2(class extends Action2 {
    constructor() {
    ...
    async run(accessor, item) {
        ...
        if (entry) {
            ...
            inputBox.title = localize(9315, null);
            inputBox.ignoreFocusOut = true;
            inputBox.placeholder = localize(9316, null);
...
registerAction2(class extends Action2 {
    ...
    async run(accessor, item) {
        ...
        if (entry) {
            ...
            const { confirmed } = await dialogService.confirm({
                type: 'warning',
                message: localize(9317, null, entry.workingCopy.name, toLocalHistoryEntryDateLabel(entry.timestamp)),
                detail: localize(9318, null),
                primaryButton: localize(9319, null),
            });
            if (!confirmed) {
                return;
            }
...
registerAction2(class extends Action2 {
    constructor() {
        super({
            ...
            title: localize2(9339, 'Delete All'),
            f1: true,
            category: LOCAL_HISTORY_CATEGORY,
            precondition: CTX_LOCAL_HISTORY_ENABLED
        });
    ...
    async run(accessor) {
        ...
        const { confirmed } = await dialogService.confirm({
            type: 'warning',
            message: localize(9320, null),
            detail: localize(9321, null),
            primaryButton: localize(9322, null),
        });
        if (!confirmed) {
            return;
        }
...
registerAction2(class extends Action2 {
    constructor() {
        super({
            ...
            title: localize2(9340, 'Create Entry'),
            f1: true,
            category: LOCAL_HISTORY_CATEGORY,
    ...
    async run(accessor) {
        ...
        const disposables = new DisposableStore();
        const inputBox = disposables.add(quickInputService.createInputBox());
        inputBox.title = localize(9323, null);
        inputBox.ignoreFocusOut = true;
        inputBox.placeholder = localize(9324, null, labelService.getUriBasenameLabel(resource));
        inputBox.show();
...
async function openEntry(entry, editorService, options) {
    ...
    await editorService.openEditor({
        resource,
        label: localize(9325, null, entry.workingCopy.name, SaveSourceRegistry.getSourceLabel(entry.source), toLocalHistoryEntryDateLabel(entry.timestamp)),
        options
    });
}
...
export function toDiffEditorArguments(arg1, arg2, options) {
    ...
    if (URI.isUri(arg2)) {
        ...
        label = localize(9326, null, arg1.workingCopy.name, SaveSourceRegistry.getSourceLabel(arg1.source), toLocalHistoryEntryDateLabel(arg1.timestamp), arg1.workingCopy.name);
    }
    // Compare with another entry
        ...
        label = localize(9327, null, arg1.workingCopy.name, SaveSourceRegistry.getSourceLabel(arg1.source), toLocalHistoryEntryDateLabel(arg1.timestamp), modified.workingCopy.name, SaveSourceRegistry.getSourceLabel(modified.source), toLocalHistoryEntryDateLabel(modified.timestamp));
    }
    return [
        originalResource,
        modifiedResource,
