    ...
    inputSchema: {
        ...
        properties: {
            urls: {
                ...
                description: localize(6642, null)
            }
        },
        required: ['urls']
    }
...
let FetchWebPageTool = class FetchWebPageTool {
    ...
    async invoke(invocation, _countTokens, _progress, token) {
        ...
        if (!allValidUris.length && invalidUris.size === 0) {
            return {
                content: [{ kind: 'text', value: localize(6643, null) }]
            };
        }
            ...
            try {
                ...
                if (imageMimeType) {
                    ...
                    if (detected.seemsBinary) {
                        ...
                        fileContents.push(localize(6644, null));
                    }
                    else {
    ...
    async prepareToolInvocation(context, token) {
        ...
        const pastTenseMessage = invalid.length
            ? invalid.length > 1
                ...
                ? new MarkdownString(localize(6645, null, urlsNeedingConfirmation.size, invalid.map(url => `- ${url}`).join('\n')))
                // If there is only one invalid URL, show it
                : new MarkdownString(localize(6646, null, invalid[0]))
            // No invalid URLs
            : new MarkdownString();
        const invocationMessage = new MarkdownString();
        if (urlsNeedingConfirmation.size > 1) {
            pastTenseMessage.appendMarkdown(localize(6647, null, urlsNeedingConfirmation.size));
            invocationMessage.appendMarkdown(localize(6648, null, urlsNeedingConfirmation.size));
        }
            ...
            if (url.length > 400 || validFileUris.length === 1) {
                pastTenseMessage.appendMarkdown(localize(6649, null, url));
                ...
                invocationMessage.appendMarkdown(localize(6650, null, url));
                ...
                pastTenseMessage.appendMarkdown(localize(6651, null, url));
                invocationMessage.appendMarkdown(localize(6652, null, url));
            }
        }
        ...
        result.confirmationMessages = {
            ...
            disclaimer: new MarkdownString('$(info) ' + localize(6655, null), { supportThemeIcons: true })
        };
        return result;
    }
    _parseUris(urls) {
