...
let ChatWidget = class ChatWidget extends Disposable {
    ...
    render(parent) {
        ...
        scrollDownButton.setTitle(localize(6130, null));
    ...
    renderWelcomeViewContentIfNeeded() {
        ...
        if (!numItems) {
            ...
            if (this.chatEntitlementService.anonymous && !this.chatEntitlementService.sentiment.installed) {
                additionalMessage = new MarkdownString(localize(6131, null, defaultChat.provider.default.name, defaultChat.provider.default.name, defaultChat.termsStatementUrl, defaultChat.privacyStatementUrl), { isTrusted: true });
            }
            else {
    ...
    async renderWelcomeHistorySection() {
        try {
            ...
            if (!this.historyList) {
                ...
                this.historyList = this._register(this.instantiationService.createInstance((WorkbenchList), 'ChatHistoryList', this.historyListContainer, delegate, [renderer], {
                    ...
                    accessibilityProvider: {
                        ...
                        getWidgetAriaLabel: () => localize(6132, null)
                    }
                }));
            ...
            previousChatsLink.textContent = localize(6133, null);
            previousChatsLink.setAttribute('role', 'button');
            previousChatsLink.setAttribute('tabindex', '0');
            previousChatsLink.setAttribute('aria-label', localize(6134, null));
            // Add hover tooltip for the link at the end of the list
            const hoverContent = localize(6135, null);
    ...
    _getGenerateInstructionsMessage() {
        ...
        if (this._instructionFilesExist === true) {
            ...
            return new MarkdownString(localize(6136, null, `command:${generateInstructionsCommand}`), { isTrusted: { enabledCommands: [generateInstructionsCommand] } });
        }
    ...
    getWelcomeViewContent(additionalMessage) {
        if (this.isLockedToCodingAgent) {
            ...
            const message = providerMessage
                ...
                : (this._lockedAgent?.prefix === '@copilot '
                    ? new MarkdownString(localize(6137, null, this._lockedAgent.prefix, 'https://aka.ms/coding-agent-docs') + this.chatDisclaimer, { isTrusted: true })
                    : new MarkdownString(localize(6138, null, this._lockedAgent?.prefix) + this.chatDisclaimer));
            return {
                title: providerTitle ?? localize(6139, null, this._lockedAgent?.prefix),
                message,
        ...
        if (this.input.currentModeKind === ChatModeKind.Ask) {
            title = localize(6140, null);
        }
        else if (this.input.currentModeKind === ChatModeKind.Edit) {
            title = localize(6141, null);
        }
        else {
            title = localize(6142, null);
        }
        return {
            title,
    ...
    getPromptFileSuggestions() {
        ...
        if (!this.chatEntitlementService.sentiment.installed) {
            ...
            if (isEmpty) {
                return [
                    {
                        icon: Codicon.vscode,
                        label: localize(6143, null),
                        prompt: localize(6144, null),
                    },
                    {
                        icon: Codicon.newFolder,
                        label: localize(6145, null),
                        prompt: localize(6146, null),
                    }
                ];
            }
            else {
                return [
                    {
                        ...
                        label: localize(6147, null),
                        prompt: localize(6148, null),
                    },
                    {
                        icon: Codicon.gear,
                        label: localize(6149, null),
                        prompt: localize(6150, null),
                    }
                ];
            }
        }
        ...
        for (const { promptName } of topPrompts) {
            const description = this.promptDescriptionsCache.get(promptName);
            const commandLabel = localize(6151, null, promptName);
    ...
    setModel(model) {
        ...
        if (this._lockedAgent) {
            ...
            if (!placeholder) {
                placeholder = localize(6152, null, this._lockedAgent.id);
            }
