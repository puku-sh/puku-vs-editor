...
class ManageLanguageModelAuthenticationAction extends Action2 {
    ...
    constructor() {
        super({
            ...
            title: localize2(5202, 'Manage Language Model Access...'),
    ...
    async run(accessor) {
        ...
        if (extensionAuth.size === 0) {
            dialogService.prompt({
                type: 'info',
                message: localize(5192, null),
                detail: localize(5193, null)
            });
            return;
        }
        const items = [];
        ...
        for (const [ownerId, allowedExtensions] of extensionAuth) {
            ...
            items.push({
                type: 'separator',
                id: ownerId,
                label: localize(5194, null, extension.displayName || extension.name),
                buttons: [{
                        iconClass: ThemeIcon.asClassName(Codicon.info),
                        tooltip: localize(5195, null),
                    }]
            });
            ...
            if (allowedExtensions.length > 0) {
                ...
                items.push({
                    label: localize(5198, null),
                    description: localize(5199, null, ownerId),
                    pickable: false
                });
            }
        }
        ...
        const result = await quickInputService.pick(items, {
            ...
            title: localize(5200, null),
            placeHolder: localize(5201, null),
        });
        if (!result) {
            return;
        }
