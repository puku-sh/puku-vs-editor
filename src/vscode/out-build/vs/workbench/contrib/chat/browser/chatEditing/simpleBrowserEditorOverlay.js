...
let SimpleBrowserOverlayWidget = class SimpleBrowserOverlayWidget {
    constructor(_editor, _container, _hostService, _chatWidgetService, fileService, environmentService, logService, configurationService, _preferencesService, _browserElementsService, contextMenuService) {
        ...
        const startSelectionMessage = localize(5751, null);
        ...
        actions.push(toAction({
            ...
            run: async () => {
                ...
                // stop selection
                message.textContent = localize(5756, null);
                finishedSelecting();
            }
        }));
        const startButton = this._showStore.add(new ButtonWithDropdown(mainContent, {
            ...
            title: localize(5757, null),
            supportIcons: true,
            ...defaultButtonStyles
        }));
        startButton.primaryButton.label = localize(5758, null);
        startButton.element.classList.add('element-selection-start');
        const cancelButton = this._showStore.add(new Button(mainContent, { ...defaultButtonStyles, supportIcons: true, title: localize(5759, null) }));
        cancelButton.element.className = 'element-selection-cancel hidden';
        const cancelButtonLabel = localize(5760, null);
        cancelButton.label = cancelButtonLabel;
        const configure = this._showStore.add(new Button(mainContent, { supportIcons: true, title: localize(5761, null) }));
        configure.icon = Codicon.gear;
        const collapseOverlay = this._showStore.add(new Button(mainContent, { supportIcons: true, title: localize(5762, null) }));
        collapseOverlay.icon = Codicon.chevronRight;
        const nextSelection = this._showStore.add(new Button(mainContent, { supportIcons: true, title: localize(5763, null) }));
        ...
        const expandOverlay = this._showStore.add(new Button(expandContainer, { supportIcons: true, title: localize(5764, null) }));
        ...
        const startElementSelection = async () => {
            ...
            this._editor.focus();
            // start selection
            message.textContent = localize(5765, null);
            this.hideElement(startButton.element);
            this.showElement(cancelButton.element);
            await this.addElementToChat(cts);
            // stop selection
            message.textContent = localize(5766, null);
            finishedSelecting();
        };
        ...
        this._showStore.add(addDisposableListener(cancelButton.element, 'click', () => {
            cts.cancel();
            message.textContent = localize(5767, null);
            finishedSelecting();
        }));
...
let SimpleBrowserOverlayController = class SimpleBrowserOverlayController {
    constructor(container, group, instaService, configurationService, _browserElementsService) {
        ...
        const show = async () => {
            ...
            connectingWebviewElement.textContent = localize(5768, null);
            ...
            if (activeBrowserType) {
                try {
                    ...
                    connectingWebviewElement.textContent = localize(5769, null);
                    return;
                }
            }
