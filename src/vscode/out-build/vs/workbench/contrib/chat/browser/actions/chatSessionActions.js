            ...
            await chatSessionsService.setEditableSession(context.session.resource, {
                validationMessage: (value) => {
                    if (!value || value.trim().length === 0) {
                        return { content: localize(5227, null), severity: Severity.Error };
                    }
                    if (value.length > 100) {
                        return { content: localize(5228, null), severity: Severity.Error };
                    }
                    return null;
                },
                placeholder: localize(5229, null),
                startingValue: label,
                onFinish: async (value, success) => {
                    if (success && value && value.trim() !== label) {
                        try {
                            ...
                            logService.error(localize(5230, null, (error instanceof Error ? error.message : String(error))));
                        }
                    }
    ...
    async run(accessor, context) {
        ...
        try {
            ...
            const result = await dialogService.confirm({
                message: localize(5232, null),
                detail: localize(5233, null),
                primaryButton: localize(5234, null),
                type: 'warning'
            });
            if (result.confirmed) {
...
export class ChatSessionsGettingStartedAction extends Action2 {
    ...
    constructor() {
        super({
            ...
            title: nls.localize2(5250, "Getting Started with Chat Sessions"),
        ...
        const quickPickItems = recommendations.map((recommendation) => {
            ...
            return {
                ...
                detail: extensionInstalled
                    ? nls.localize(5240, null, recommendation.extensionName)
                    : nls.localize(5241, null, recommendation.extensionName),
                extensionId: recommendation.extensionId,
                disabled: extensionInstalled,
            };
        });
        const selected = await quickInputService.pick(quickPickItems, {
            title: nls.localize(5242, null),
            placeHolder: nls.localize(5243, null),
            canPickMany: true,
        });
        if (!selected) {
            return;
...
MenuRegistry.appendMenuItem(MenuId.ViewTitle, {
    command: {
        ...
        title: nls.localize2(5251, "New Chat Editor"),
        icon: Codicon.plus
    },
    group: 'navigation',
    order: 1,
...
//# sourceMappingURL=chatSessionActions.js.map