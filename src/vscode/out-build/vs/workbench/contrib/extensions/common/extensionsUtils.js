...
let KeymapExtensions = class KeymapExtensions extends Disposable {
    ...
    promptForDisablingOtherKeymaps(newKeymap, oldKeymaps) {
        ...
        this.notificationService.prompt(Severity.Info, localize(8542, null, oldKeymaps.map(k => `'${k.local.manifest.displayName}'`).join(', ')), [{
                label: localize(8543, null),
                run: () => onPrompt(true)
            }, {
                label: localize(8544, null),
                run: () => onPrompt(false)
            }]);
    }
};
