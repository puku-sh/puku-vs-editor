...
let ExtensionRecommendationNotificationService = class ExtensionRecommendationNotificationService extends Disposable {
    ...
    async promptImportantExtensionsInstallNotification(extensionRecommendations) {
        ...
        return this.promptRecommendationsNotification({ ...extensionRecommendations, extensions }, {
            ...
            onDidNeverShowRecommendedExtensionsAgain: (extensions) => {
                ...
                this.notificationService.prompt(Severity.Info, localize(7965, null), [{
                        label: localize(7966, null),
                        run: () => this.setIgnoreRecommendationsConfig(true)
                    }, {
                        label: localize(7967, null),
    ...
    async promptWorkspaceRecommendations(recommendations) {
        ...
        await this.promptRecommendationsNotification({ extensions: recommendations, source: 2 /* RecommendationSource.WORKSPACE */, name: localize(7968, null) }, {
    ...
    async promptRecommendationsNotification({ extensions: extensionIds, source, name, searchValue }, recommendationsNotificationActions) {
        ...
        if (extensions.length === 1) {
            extensionsMessage = localize(7969, null, extensions[0].displayName, extensions[0].publisherDisplayName);
        }
        else {
            ...
            if (publishers.length > 2) {
                extensionsMessage = localize(7970, null, publishers[0], publishers[1]);
            }
            else if (publishers.length === 2) {
                extensionsMessage = localize(7971, null, publishers[0], publishers[1]);
            }
            else {
                extensionsMessage = localize(7972, null, publishers[0]);
            }
        }
        let message = localize(7973, null, extensionsMessage, name);
        if (source === 3 /* RecommendationSource.EXE */) {
            message = localize(7974, null, name, extensionsMessage);
        }
        if (!searchValue) {
        ...
        const donotShowAgainLabel = source === 2 /* RecommendationSource.WORKSPACE */ ? localize(7975, null)
            : extensions.length > 1 ? localize(7976, null) : localize(7977, null);
        ...
        return createCancelablePromise(async (token) => {
            ...
            choices.push({
                ...
                menu: this.userDataSyncEnablementService.isEnabled() && this.userDataSyncEnablementService.isResourceEnabled("extensions" /* SyncResource.Extensions */) ? [{
                        label: localize(7979, null),
                        run: () => installExtensions(true)
                    }] : undefined,
            });
            choices.push(...[{
                    label: localize(7980, null),
                    run: async () => {
