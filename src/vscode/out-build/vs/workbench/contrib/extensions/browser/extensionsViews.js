...
let ExtensionsListView = class ExtensionsListView extends AbstractExtensionsListView {
    ...
    filterExtensionsByFeature(local, query) {
        ...
        try {
            ...
            return {
                ...
                description: localize(8412, null, feature.label)
            };
        }
        finally {
            renderer?.dispose();
    ...
    async queryGallery(query, options, token) {
        ...
        try {
            ...
            if (localExtensions.length) {
                ...
                return { model: new PagedModel(localExtensions), disposables: new DisposableStore(), message: { text: localize(8413, null, message.text), severity: message.severity } };
            }
            throw error;
        }
    }
        ...
        if (this.bodyTemplate) {
            ...
            if (this.isBodyVisible()) {
                if (message) {
                    ...
                    this.bodyTemplate.messageBox.textContent = localize(8414, null);
                }
    ...
    getMessage(error) {
        if (this.isOfflineError(error)) {
            return { text: localize(8415, null), severity: Severity.Warning };
        }
        else {
            return { text: localize(8416, null, getErrorMessage(error)), severity: Severity.Error };
        }
    }
