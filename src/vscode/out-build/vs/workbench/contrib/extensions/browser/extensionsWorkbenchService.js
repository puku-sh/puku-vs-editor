        ...
        if (this.gallery) {
            ...
            if (!cache) {
                if (this.gallery.assets.manifest) {
                    ...
                    this.logService.error(nls.localize(8450, null), this.identifier.id);
                }
            }
            return cache;
        }
...
let ExtensionsWorkbenchService = class ExtensionsWorkbenchService extends Disposable {
    ...
    async updateAutoUpdateForAllExtensions(isAutoUpdateEnabled) {
        ...
        const result = await this.dialogService.confirm({
            title: nls.localize(8451, null),
            message: isAutoUpdateEnabled
                ? nls.localize(8452, null)
                : nls.localize(8453, null),
            detail: nls.localize(8454, null),
        });
        if (!result.confirmed) {
            return;
        }
    ...
    computeExtensionsNotifications() {
        ...
        if (disallowedExtensions.length) {
            computedNotificiations.push({
                message: this.configurationService.inspect(AllowedExtensionsConfigKey).policy
                    ? nls.localize(8456, null)
                    : nls.localize(8457, null),
        ...
        if (invalidExtensions.length) {
            if (invalidExtensions.some(e => e.local && e.local.manifest.engines?.vscode &&
                ...
                computedNotificiations.push({
                    message: nls.localize(8458, null),
                    severity: Severity.Warning,
                ...
                computedNotificiations.push({
                    message: nls.localize(8459, null),
                    severity: Severity.Warning,
                    extensions: invalidExtensions,
        ...
        if (deprecatedExtensions.length) {
            computedNotificiations.push({
                message: nls.localize(8460, null),
                severity: Severity.Warning,
    ...
    async updateRunningExtensions(message = nls.localize(8461, null), auto = false) {
        const toAdd = [];
        const toRemove = [];
            ...
            if (await this.extensionService.stopExtensionHosts(message, auto)) {
                ...
                if (auto) {
                    this.notificationService.notify({
                        ...
                        message: nls.localize(8462, null),
    ...
    getRuntimeState(extension) {
        ...
        const reloadActionLabel = reloadAction === "reloadWindow" /* ExtensionRuntimeActionType.ReloadWindow */ ? nls.localize(8463, null) : nls.localize(8464, null);
        if (isUninstalled) {
            ...
            if (!canRemoveRunningExtension && isSameExtensionRunning && !runningExtension.isUnderDevelopment) {
                return { action: reloadAction, reason: nls.localize(8465, null, reloadActionLabel) };
            }
            return undefined;
        }
        if (extension.local) {
            ...
            if (runningExtension) {
                if (isEnabled) {
                    ...
                    if (isSameExtensionRunning) {
                        ...
                        if (!runningExtension.isUnderDevelopment && (extension.version !== runningExtension.version || extension.local.targetPlatform !== runningExtension.targetPlatform)) {
                            ...
                            if (productUpdateVersion
                                ...
                                if (state.type === "available for download" /* StateType.AvailableForDownload */) {
                                    return { action: "downloadUpdate" /* ExtensionRuntimeActionType.DownloadUpdate */, reason: nls.localize(8466, null, this.productService.nameLong) };
                                }
                                if (state.type === "downloaded" /* StateType.Downloaded */) {
                                    return { action: "applyUpdate" /* ExtensionRuntimeActionType.ApplyUpdate */, reason: nls.localize(8467, null, this.productService.nameLong) };
                                }
                                if (state.type === "ready" /* StateType.Ready */) {
                                    return { action: "quitAndInstall" /* ExtensionRuntimeActionType.QuitAndInstall */, reason: nls.localize(8468, null, this.productService.nameLong) };
                                }
                                return undefined;
                            }
                            return { action: reloadAction, reason: nls.localize(8469, null, reloadActionLabel) };
                        }
                        if (this.extensionsServers.length > 1) {
                            ...
                            if (extensionInOtherServer) {
                                ...
                                if (runningExtensionServer === this.extensionManagementServerService.remoteExtensionManagementServer && this.extensionManifestPropertiesService.prefersExecuteOnUI(extension.local.manifest) && extensionInOtherServer.server === this.extensionManagementServerService.localExtensionManagementServer) {
                                    return { action: reloadAction, reason: nls.localize(8470, null, reloadActionLabel) };
                                }
                                ...
                                if (runningExtensionServer === this.extensionManagementServerService.localExtensionManagementServer && this.extensionManifestPropertiesService.prefersExecuteOnWorkspace(extension.local.manifest) && extensionInOtherServer.server === this.extensionManagementServerService.remoteExtensionManagementServer) {
                                    return { action: reloadAction, reason: nls.localize(8471, null, reloadActionLabel, this.extensionManagementServerService.remoteExtensionManagementServer?.label) };
                                }
                            }
                        }
                    }
                        ...
                        if (extension.server === this.extensionManagementServerService.localExtensionManagementServer && runningExtensionServer === this.extensionManagementServerService.remoteExtensionManagementServer) {
                            ...
                            if (this.extensionManifestPropertiesService.prefersExecuteOnUI(extension.local.manifest)) {
                                return { action: reloadAction, reason: nls.localize(8472, null, reloadActionLabel) };
                            }
                        }
                        if (extension.server === this.extensionManagementServerService.remoteExtensionManagementServer && runningExtensionServer === this.extensionManagementServerService.localExtensionManagementServer) {
                            ...
                            if (this.extensionManifestPropertiesService.prefersExecuteOnWorkspace(extension.local.manifest)) {
                                return { action: reloadAction, reason: nls.localize(8473, null, reloadActionLabel) };
                            }
                        }
                    }
                    return undefined;
                    ...
                    if (isSameExtensionRunning) {
                        return { action: reloadAction, reason: nls.localize(8474, null, reloadActionLabel) };
                    }
                }
                return undefined;
            }
            // Extension is not running
                ...
                if (isEnabled && !this.extensionService.canAddExtension(toExtensionDescription(extension.local))) {
                    return { action: reloadAction, reason: nls.localize(8475, null, reloadActionLabel) };
                }
                ...
                if (otherServer && extension.enablementState === 1 /* EnablementState.DisabledByExtensionKind */) {
                    ...
                    if (extensionInOtherServer && extensionInOtherServer.local && this.extensionEnablementService.isEnabled(extensionInOtherServer.local)) {
                        return { action: reloadAction, reason: nls.localize(8476, null, reloadActionLabel) };
                    }
                }
            }
        }
    ...
    async downloadVSIX(extensionId, versionKind) {
        ...
        if (!galleryExtension) {
            throw new Error(nls.localize(8477, null, extensionId));
        }
        ...
        for (const targetPlatform of version?.targetPlatforms ?? galleryExtension.allTargetPlatforms) {
            if (targetPlatform !== "unknown" /* TargetPlatform.UNKNOWN */ && targetPlatform !== "universal" /* TargetPlatform.UNIVERSAL */) {
                options.push({
                    label: targetPlatform === "undefined" /* TargetPlatform.UNDEFINED */ ? nls.localize(8478, null) : TargetPlatformToString(targetPlatform),
                    id: targetPlatform
                });
            }
        }
        if (options.length > 1) {
            const message = nls.localize(8479, null);
        ...
        const result = await this.fileDialogService.showOpenDialog({
            title: nls.localize(8480, null),
            canSelectFiles: false,
            canSelectFolders: true,
            canSelectMany: false,
            openLabel: nls.localize(8481, null),
        });
        if (!result?.[0]) {
            return;
        }
        this.progressService.withProgress({ location: 15 /* ProgressLocation.Notification */ }, async (progress) => {
            try {
                progress.report({ message: nls.localize(8482, null) });
                ...
                this.notificationService.info(nls.localize(8483, null));
            }
            catch (error) {
                this.notificationService.error(nls.localize(8484, null, getErrorMessage(error)));
            }
        });
    }
    async pickVersionToDownload(extensionId) {
        ...
        if (!allVersions.length) {
            await this.dialogService.info(nls.localize(8485, null));
            return;
        }
        const picks = allVersions.map((v, i) => {
            return {
                id: v.version,
                label: v.version,
                description: `${fromNow(new Date(Date.parse(v.date)), true)}${v.isPreReleaseVersion ? ` (${nls.localize(8486, null)})` : ''}`,
                ariaLabel: `${v.isPreReleaseVersion ? 'Pre-Release version' : 'Release version'} ${v.version}`,
                data: v,
            };
        });
        const pick = await this.quickInputService.pick(picks, {
            placeHolder: nls.localize(8487, null),
            matchOnDetail: true
        });
        return pick?.data;
    }
    ...
    async shouldRequireConsentToUpdate(extension) {
        ...
        return nls.localize(8489, null, extension.displayName);
    }
    ...
    async canInstall(extension) {
        if (!(extension instanceof Extension)) {
            return new MarkdownString().appendText(nls.localize(8490, null));
        }
        if (extension.isMalicious) {
            return new MarkdownString().appendText(nls.localize(8491, null));
        }
        if (extension.deprecationInfo?.disallowInstall) {
            return new MarkdownString().appendText(nls.localize(8492, null));
        }
        if (extension.gallery) {
            if (!extension.gallery.isSigned && shouldRequireRepositorySignatureFor(extension.private, await this.extensionGalleryManifestService.getExtensionGalleryManifest())) {
                return new MarkdownString().appendText(nls.localize(8493, null));
            }
            ...
            return localResult ?? remoteResult ?? webResult ?? new MarkdownString().appendText(nls.localize(8494, null, extension.displayName ?? extension.identifier.id));
        }
        if (extension.resourceExtension && await this.extensionManagementService.canInstall(extension.resourceExtension) === true) {
            return true;
        }
        return new MarkdownString().appendText(nls.localize(8495, null, extension.displayName ?? extension.identifier.id));
    }
    async install(arg, installOptions = {}, progressLocation) {
        ...
        if (arg instanceof URI) {
            ...
            if (extension?.isMalicious) {
                throw new Error(nls.localize(8496, null));
            }
            if (gallery) {
            ...
            if (!servers || servers.length) {
                if (!installable) {
                    if (!gallery) {
                        ...
                        const reportIssueMessage = reportIssueUri ? nls.localize(8497, null, reportIssueUri.toString()) : '';
                        if (installOptions.version) {
                            const message = nls.localize(8498, null, id, installOptions.version);
                            ...
                            const message = nls.localize(8499, null, id);
                            throw new ExtensionManagementError(reportIssueMessage ? `${message} ${reportIssueMessage}` : message, "NotFound" /* ExtensionManagementErrorCode.NotFound */);
                        }
                    }
                    installable = gallery;
            ...
            if (installOptions.justification) {
                ...
                buttons.push({
                    label: isString(installOptions.justification) || !installOptions.justification.action
                        ? nls.localize(8500, null)
                        : nls.localize(8501, null, installOptions.justification.action), run: () => true
                });
                if (!extension) {
                    buttons.push({ label: nls.localize(8502, null), run: () => { this.open(extension); return false; } });
                }
                const result = await this.dialogService.prompt({
                    title: nls.localize(8503, null),
                    message: extension ? nls.localize(8504, null, extension.displayName, extension.publisherDisplayName) : nls.localize(8505, null),
                    detail: isString(installOptions.justification) ? installOptions.justification : installOptions.justification.reason,
                    cancelButton: true,
                    buttons,
                    checkbox: syncCheck ? {
                        label: nls.localize(8506, null),
                        checked: true,
                    } : undefined,
                });
                if (!result.result) {
        ...
        if (!extension) {
            throw new Error(nls.localize(8507, null));
        }
        if (installOptions.enable) {
            if (extension.enablementState === 11 /* EnablementState.DisabledWorkspace */ || extension.enablementState === 10 /* EnablementState.DisabledGlobally */) {
                if (installOptions.justification) {
                    const result = await this.dialogService.confirm({
                        title: nls.localize(8508, null),
                        message: nls.localize(8509, null, extension.displayName),
                        detail: isString(installOptions.justification) ? installOptions.justification : installOptions.justification.reason,
                        primaryButton: isString(installOptions.justification) ? nls.localize(8510, null) : nls.localize(8511, null, installOptions.justification.action),
                    });
                    if (!result.confirmed) {
                        throw new CancellationError();
                    }
    ...
    async installInServer(extension, server, installOptions) {
        await this.doInstall(extension, async () => {
            ...
            if (!isTargetPlatformCompatible(local.targetPlatform, [local.targetPlatform], targetPlatform)) {
                throw new Error(nls.localize(8512, null, extension.identifier.id));
            }
    ...
    async uninstall(e) {
        ...
        if (extension.local.isApplicationScoped && this.userDataProfilesService.profiles.length > 1) {
            const { confirmed } = await this.dialogService.confirm({
                title: nls.localize(8513, null),
                type: Severity.Info,
                message: nls.localize(8514, null, extension.displayName),
                primaryButton: nls.localize(8515, null)
            });
            if (!confirmed) {
                throw new CancellationError();
            }
    ...
    getErrorMessageForUninstallingAnExtensionWithDependents(extension, dependents) {
        if (dependents.length === 1) {
            return nls.localize(8519, null, extension.displayName, dependents[0].manifest.displayName);
        }
        if (dependents.length === 2) {
            return nls.localize(8520, null, extension.displayName, dependents[0].manifest.displayName, dependents[1].manifest.displayName);
        }
        return nls.localize(8521, null, extension.displayName, dependents[0].manifest.displayName, dependents[1].manifest.displayName);
    }
    ...
    doInstall(extension, installTask, progressLocation) {
        const title = extension ? nls.localize(8522, null, extension.displayName) : nls.localize(8523, null);
        return this.withProgress({
    ...
    getErrorMessageForDisablingAnExtensionWithDependents(extension, dependents) {
        if (dependents.length === 1) {
            return nls.localize(8526, null, extension.displayName, dependents[0].displayName);
        }
        if (dependents.length === 2) {
            return nls.localize(8527, null, extension.displayName, dependents[0].displayName, dependents[1].displayName);
        }
        return nls.localize(8528, null, extension.displayName, dependents[0].displayName, dependents[1].displayName);
    }
