...
let PromptExtensionInstallFailureAction = class PromptExtensionInstallFailureAction extends Action {
    ...
    async run() {
        ...
        if (this.error.name === "Unsupported" /* ExtensionManagementErrorCode.Unsupported */) {
            const productName = isWeb ? localize(8139, null, this.productService.nameLong) : this.productService.nameLong;
            const message = localize(8140, null, this.extension.displayName || this.extension.identifier.id, productName);
            const { confirmed } = await this.dialogService.confirm({
                type: Severity.Info,
                message,
                primaryButton: localize(8141, null),
                cancelButton: localize(8142, null)
            });
            if (confirmed) {
        ...
        if ("ReleaseVersionNotFound" /* ExtensionManagementErrorCode.ReleaseVersionNotFound */ === this.error.name) {
            await this.dialogService.prompt({
                ...
                cancelButton: localize(8144, null)
            });
            return;
        }
        ...
        if ("PackageNotSigned" /* ExtensionManagementErrorCode.PackageNotSigned */ === this.error.name) {
            await this.dialogService.prompt({
                type: 'error',
                message: localize(8145, null, this.extension.displayName),
                detail: getErrorMessage(this.error),
                buttons: [{
                        label: localize(8146, null),
                        run: () => {
        ...
        if ("SignatureVerificationFailed" /* ExtensionManagementErrorCode.SignatureVerificationFailed */ === this.error.name) {
            await this.dialogService.prompt({
                type: 'error',
                message: localize(8147, null, this.extension.displayName, this.productService.nameLong),
                detail: getErrorMessage(this.error),
                buttons: [{
                        label: localize(8148, null),
        ...
        if ("SignatureVerificationInternal" /* ExtensionManagementErrorCode.SignatureVerificationInternal */ === this.error.name) {
            await this.dialogService.prompt({
                type: 'error',
                message: localize(8150, null, this.extension.displayName, this.productService.nameLong),
                detail: getErrorMessage(this.error),
                buttons: [{
                        label: localize(8151, null),
                        run: () => this.openerService.open('https://code.visualstudio.com/docs/editor/extension-marketplace#_the-extension-signature-cannot-be-verified-by-vs-code')
                    }, {
                        label: localize(8152, null),
                        run: () => this.workbenchIssueService.openReporter({
                            issueTitle: localize(8153, null, this.extension.displayName),
                            issueBody: localize(8154, null)
                        })
                    }, {
                        label: localize(8155, null),
                        run: () => {
                            const installAction = this.instantiationService.createInstance(InstallAction, { ...this.options, donotVerifySignature: true, });
                            installAction.extension = this.extension;
                            return installAction.run();
        ...
        const operationMessage = this.installOperation === 3 /* InstallOperation.Update */ ? localize(8156, null, this.extension.displayName || this.extension.identifier.id)
            : localize(8157, null, this.extension.displayName || this.extension.identifier.id);
        let additionalMessage;
        const promptChoices = [];
        const downloadUrl = await this.getDownloadUrl();
        if (downloadUrl) {
            additionalMessage = localize(8158, null, createCommandUri(showWindowLogActionId).toString());
            promptChoices.push({
                label: localize(8159, null),
                run: () => this.openerService.open(downloadUrl).then(() => {
                    this.notificationService.prompt(Severity.Info, localize(8160, null, this.extension.identifier.id), [{
                            label: localize(8161, null),
...
let InstallAction = class InstallAction extends ExtensionAction {
    ...
    constructor(options, extensionsWorkbenchService, instantiationService, runtimeExtensionService, workbenchThemeService, labelService, dialogService, preferencesService, telemetryService, contextService, allowedExtensionsService, extensionGalleryManifestService) {
        super('extensions.install', localize(8162, null), InstallAction_1.CLASS, false);
    ...
    async run() {
        ...
        if (this.extension.gallery && !this.extension.gallery.isSigned && shouldRequireRepositorySignatureFor(this.extension.private, await this.extensionGalleryManifestService.getExtensionGalleryManifest())) {
            const { result } = await this.dialogService.prompt({
                ...
                message: localize(8163, null, this.extension.displayName),
                detail: localize(8164, null),
                buttons: [
                    {
                        label: localize(8165, null),
                        run: () => {
        ...
        if (this.extension.deprecationInfo) {
            ...
            const buttons = [
                {
                    label: localize(8167, null),
                    run: () => DeprecationChoice.InstallAnyway
                }
            ];
            if (this.extension.deprecationInfo.extension) {
                detail = localize(8168, null, this.extension.deprecationInfo.extension.displayName);
                const alternateExtension = this.extension.deprecationInfo.extension;
                buttons.push({
                    label: localize(8169, null, this.extension.deprecationInfo.extension.displayName),
                    run: async () => {
            ...
            const { result } = await this.dialogService.prompt({
                type: Severity.Warning,
                message: localize(8172, null, this.extension.displayName),
        ...
        alert(localize(8173, null, this.extension.displayName));
        /* __GDPR__
        ...
        */
        ...
        if (extension?.local) {
            alert(localize(8174, null, this.extension.displayName));
    ...
    getLabel(primary) {
        if (this.extension?.isWorkspaceScoped && this.extension.resourceExtension && this.contextService.isInsideWorkspace(this.extension.resourceExtension.location)) {
            return localize(8175, null);
        }
        /* install pre-release version */
        if (this.options.installPreReleaseVersion && this.extension?.hasPreReleaseVersion) {
            return primary ? localize(8176, null) : localize(8177, null);
        }
        /* install released version that has a pre release version */
        if (this.extension?.hasPreReleaseVersion) {
            return primary ? localize(8178, null) : localize(8179, null);
        }
        return localize(8180, null);
    }
};
...
export class InstallingLabelAction extends ExtensionAction {
    static { this.LABEL = localize(8181, null); }
...
let InstallInOtherServerAction = class InstallInOtherServerAction extends ExtensionAction {
    ...
    static { this.INSTALL_LABEL = localize(8182, null); }
    static { this.INSTALLING_LABEL = localize(8183, null); }
    ...
    async run() {
        ...
        alert(localize(8184, null, this.extension.displayName));
...
let RemoteInstallAction = class RemoteInstallAction extends InstallInOtherServerAction {
    ...
    getInstallLabel() {
        return this.extensionManagementServerService.remoteExtensionManagementServer
            ? localize(8185, null, this.extensionManagementServerService.remoteExtensionManagementServer.label)
...
let UninstallAction = class UninstallAction extends ExtensionAction {
    ...
    static { this.UninstallLabel = localize(8188, null); }
    static { this.UninstallingLabel = localize(8189, null); }
    ...
    update() {
        ...
        this.label = this.extension.local?.isApplicationScoped && this.userDataProfilesService.profiles.length > 1 ? localize(8190, null) : UninstallAction_1.UninstallLabel;
    ...
    async run() {
        ...
        alert(localize(8191, null, this.extension.displayName));
        try {
            await this.extensionsWorkbenchService.uninstall(this.extension);
            alert(localize(8192, null, this.extension.displayName));
        }
        catch (error) {
...
let UpdateAction = class UpdateAction extends ExtensionAction {
    ...
    constructor(verbose, extensionsWorkbenchService, dialogService, openerService, instantiationService) {
        super(`extensions.update`, localize(8193, null), UpdateAction_1.DisabledClass, false);
    ...
    update() {
        this.updateThrottler.queue(() => this.computeAndUpdateEnablement());
        if (this.extension) {
            this.label = this.verbose ? localize(8194, null, this.extension.latestVersion) : localize(8195, null);
        }
    }
    async computeAndUpdateEnablement() {
        this.enabled = false;
    ...
    async run() {
        ...
        if (consent) {
            const { result } = await this.dialogService.prompt({
                type: 'warning',
                title: localize(8196, null, this.extension.displayName),
                message: localize(8197, null, consent),
                buttons: [{
                        label: localize(8198, null),
                        run: () => 'update'
                    }, {
                        label: localize(8199, null),
                        run: () => 'review'
                    }, {
                        label: localize(8200, null),
                        run: () => 'cancel'
                    }]
            });
            if (result === 'cancel') {
        ...
        try {
            alert(localize(8201, null, this.extension.displayName, this.extension.latestVersion));
            await this.extensionsWorkbenchService.install(this.extension, installOptions);
            alert(localize(8202, null, this.extension.displayName, this.extension.latestVersion));
        }
        catch (err) {
...
let ToggleAutoUpdateForExtensionAction = class ToggleAutoUpdateForExtensionAction extends ExtensionAction {
    ...
    static { this.LABEL = localize2(8312, "Auto Update"); }
    ...
    async run() {
        ...
        if (enableAutoUpdate) {
            alert(localize(8203, null, this.extension.displayName));
        }
        else {
            alert(localize(8204, null, this.extension.displayName));
        }
    }
};
...
let ToggleAutoUpdatesForPublisherAction = class ToggleAutoUpdatesForPublisherAction extends ExtensionAction {
    ...
    static { this.LABEL = localize(8205, null); }
    ...
    async run() {
        ...
        alert(localize(8206, null, this.extension.publisherDisplayName));
        ...
        if (enableAutoUpdate) {
            alert(localize(8207, null, this.extension.displayName));
        }
        else {
            alert(localize(8208, null, this.extension.displayName));
        }
    }
};
...
let MigrateDeprecatedExtensionAction = class MigrateDeprecatedExtensionAction extends ExtensionAction {
    ...
    constructor(small, extensionsWorkbenchService) {
        super('extensionsAction.migrateDeprecatedExtension', localize(8209, null), MigrateDeprecatedExtensionAction_1.DisabledClass, false);
        this.small = small;
        this.extensionsWorkbenchService = extensionsWorkbenchService;
        this.update();
    }
    update() {
        ...
        this.tooltip = localize(8210, null, this.extension.deprecationInfo.extension.displayName);
        this.label = this.small ? localize(8211, null) : this.tooltip;
    }
    async run() {
        if (!this.extension?.deprecationInfo?.extension) {
            return;
...
let ManageExtensionAction = class ManageExtensionAction extends DropDownExtensionAction {
    ...
    constructor(instantiationService, extensionService, contextKeyService) {
        ...
        this.tooltip = localize(8212, null);
        this.update();
    }
...
export class ExtensionEditorManageExtensionAction extends DropDownExtensionAction {
    constructor(contextKeyService, instantiationService) {
        ...
        this.contextKeyService = contextKeyService;
        this.tooltip = localize(8213, null);
    }
    update() { }
    async run() {
        const actionGroups = [];
...
let TogglePreReleaseExtensionAction = class TogglePreReleaseExtensionAction extends ExtensionAction {
    ...
    static { this.LABEL = localize(8214, null); }
    ...
    update() {
        ...
        if (this.extension.preRelease) {
            this.label = localize(8215, null);
            this.tooltip = localize(8216, null);
        }
        else {
            this.label = localize(8217, null);
            this.tooltip = localize(8218, null);
        }
    }
    async run() {
        if (!this.extension) {
...
let InstallAnotherVersionAction = class InstallAnotherVersionAction extends ExtensionAction {
    ...
    static { this.LABEL = localize(8219, null); }
    ...
    async run() {
        ...
        if (!allVersions.length) {
            await this.dialogService.info(localize(8220, null));
            return;
        }
        const picks = allVersions.map((v, i) => {
            return {
                id: v.version,
                label: v.version,
                description: `${fromNow(new Date(Date.parse(v.date)), true)}${v.isPreReleaseVersion ? ` (${localize(8221, null)})` : ''}${v.version === this.extension?.local?.manifest.version ? ` (${localize(8222, null)})` : ''}`,
                ariaLabel: `${v.isPreReleaseVersion ? 'Pre-Release version' : 'Release version'} ${v.version}`,
                isPreReleaseVersion: v.isPreReleaseVersion
            };
        });
        const pick = await this.quickInputService.pick(picks, {
            placeHolder: localize(8223, null),
            matchOnDetail: true
        });
        if (pick) {
...
let EnableForWorkspaceAction = class EnableForWorkspaceAction extends ExtensionAction {
    ...
    static { this.LABEL = localize(8224, null); }
    constructor(extensionsWorkbenchService, extensionEnablementService) {
        ...
        this.extensionsWorkbenchService = extensionsWorkbenchService;
        this.extensionEnablementService = extensionEnablementService;
        this.tooltip = localize(8225, null);
        this.update();
    }
    update() {
        this.enabled = false;
...
let DisableForWorkspaceAction = class DisableForWorkspaceAction extends ExtensionAction {
    ...
    static { this.LABEL = localize(8228, null); }
    constructor(workspaceContextService, extensionsWorkbenchService, extensionEnablementService, extensionService) {
        ...
        this.extensionEnablementService = extensionEnablementService;
        this.extensionService = extensionService;
        this.tooltip = localize(8229, null);
        this.update();
...
let DisableGloballyAction = class DisableGloballyAction extends ExtensionAction {
    ...
    static { this.LABEL = localize(8230, null); }
    constructor(extensionsWorkbenchService, extensionEnablementService, extensionService) {
        ...
        this.extensionEnablementService = extensionEnablementService;
        this.extensionService = extensionService;
        this.tooltip = localize(8231, null);
        this.update();
        this._register(this.extensionService.onDidChangeExtensions(() => this.update()));
    }
    update() {
...
let ExtensionRuntimeStateAction = class ExtensionRuntimeStateAction extends ExtensionAction {
    ...
    update() {
        ...
        this.label = runtimeState.action === "reloadWindow" /* ExtensionRuntimeActionType.ReloadWindow */ ? localize(8232, null)
            : runtimeState.action === "restartExtensions" /* ExtensionRuntimeActionType.RestartExtensions */ ? localize(8233, null)
                : runtimeState.action === "quitAndInstall" /* ExtensionRuntimeActionType.QuitAndInstall */ ? localize(8234, null)
                    : runtimeState.action === "applyUpdate" /* ExtensionRuntimeActionType.ApplyUpdate */ || runtimeState.action === "downloadUpdate" /* ExtensionRuntimeActionType.DownloadUpdate */ ? localize(8235, null, this.productService.nameShort) : '';
    }
    async run() {
        const runtimeState = this.extension?.runtimeState;
...
function getQuickPickEntries(themes, currentTheme, extension, showCurrentTheme) {
    ...
    if (showCurrentTheme) {
        picks.push({ type: 'separator', label: localize(8236, null) });
...
let SetColorThemeAction = class SetColorThemeAction extends ExtensionAction {
    ...
    static { this.TITLE = localize2(8313, 'Set Color Theme'); }
    ...
    async run({ showCurrentTheme, ignoreFocusLost } = { showCurrentTheme: false, ignoreFocusLost: false }) {
        ...
        const pickedTheme = await this.quickInputService.pick(picks, {
            placeHolder: localize(8237, null),
...
let SetFileIconThemeAction = class SetFileIconThemeAction extends ExtensionAction {
    ...
    static { this.TITLE = localize2(8314, 'Set File Icon Theme'); }
    static { this.EnabledClass = `${ExtensionAction.LABEL_ACTION_CLASS} theme`; }
    static { this.DisabledClass = `${this.EnabledClass} disabled`; }
    constructor(extensionService, workbenchThemeService, quickInputService, extensionEnablementService) {
    ...
    async run({ showCurrentTheme, ignoreFocusLost } = { showCurrentTheme: false, ignoreFocusLost: false }) {
        ...
        const pickedTheme = await this.quickInputService.pick(picks, {
            placeHolder: localize(8238, null),
...
let SetProductIconThemeAction = class SetProductIconThemeAction extends ExtensionAction {
    ...
    static { this.TITLE = localize2(8315, 'Set Product Icon Theme'); }
    static { this.EnabledClass = `${ExtensionAction.LABEL_ACTION_CLASS} theme`; }
    static { this.DisabledClass = `${this.EnabledClass} disabled`; }
    constructor(extensionService, workbenchThemeService, quickInputService, extensionEnablementService) {
    ...
    async run({ showCurrentTheme, ignoreFocusLost } = { showCurrentTheme: false, ignoreFocusLost: false }) {
        ...
        const pickedTheme = await this.quickInputService.pick(picks, {
            placeHolder: localize(8239, null),
...
let SetLanguageAction = class SetLanguageAction extends ExtensionAction {
    ...
    static { this.TITLE = localize2(8316, 'Set Display Language'); }
...
let ClearLanguageAction = class ClearLanguageAction extends ExtensionAction {
    ...
    static { this.TITLE = localize2(8317, 'Clear Display Language'); }
    static { this.EnabledClass = `${ExtensionAction.LABEL_ACTION_CLASS} language`; }
    static { this.DisabledClass = `${this.EnabledClass} disabled`; }
...
let ShowRecommendedExtensionAction = class ShowRecommendedExtensionAction extends Action {
    ...
    static { this.LABEL = localize(8240, null); }
...
let InstallRecommendedExtensionAction = class InstallRecommendedExtensionAction extends Action {
    ...
    static { this.LABEL = localize(8241, null); }
...
let IgnoreExtensionRecommendationAction = class IgnoreExtensionRecommendationAction extends Action {
    ...
    constructor(extension, extensionRecommendationsManagementService) {
        ...
        this.tooltip = localize(8242, null);
        this.enabled = true;
    }
    run() {
...
let UndoIgnoreExtensionRecommendationAction = class UndoIgnoreExtensionRecommendationAction extends Action {
    ...
    constructor(extension, extensionRecommendationsManagementService) {
        ...
        this.tooltip = localize(8243, null);
        this.enabled = true;
    }
    run() {
...
let AbstractConfigureRecommendedExtensionsAction = class AbstractConfigureRecommendedExtensionsAction extends Action {
    ...
    openExtensionsFile(extensionsFileResource) {
        ...
        })), error => Promise.reject(new Error(localize(8244, null, error))));
    }
...
let ConfigureWorkspaceRecommendedExtensionsAction = class ConfigureWorkspaceRecommendedExtensionsAction extends AbstractConfigureRecommendedExtensionsAction {
    ...
    static { this.LABEL = localize(8245, null); }
...
let ConfigureWorkspaceFolderRecommendedExtensionsAction = class ConfigureWorkspaceFolderRecommendedExtensionsAction extends AbstractConfigureRecommendedExtensionsAction {
    ...
    static { this.LABEL = localize(8246, null); }
    ...
    computeLabel() {
        ...
        if (currentEnablementState !== null) {
            ...
            if (!currentlyEnabled && enabled) {
                return canAddExtension() ? localize(8250, null) : null;
            }
            if (currentlyEnabled && !enabled) {
                return canRemoveExtension() ? localize(8251, null) : null;
            }
        }
        return null;
    }
...
let ToggleSyncExtensionAction = class ToggleSyncExtensionAction extends DropDownExtensionAction {
    ...
    update() {
        ...
        if (this.extension) {
            ...
            this.tooltip = isIgnored ? localize(8252, null) : localize(8253, null);
        }
    }
    async run() {
        return super.run([
            [
                new Action('extensions.syncignore', this.extensionsWorkbenchService.isExtensionIgnoredToSync(this.extension) ? localize(8254, null) : localize(8255, null), undefined, true, () => this.extensionsWorkbenchService.toggleExtensionIgnoredToSync(this.extension))
            ]
        ]);
    }
};
...
let ExtensionStatusAction = class ExtensionStatusAction extends ExtensionAction {
    ...
    async computeAndUpdateStatus() {
        ...
        if (this.extension.isMalicious) {
            this.updateStatus({ icon: warningIcon, message: new MarkdownString(localize(8256, null)) }, true);
            return;
        }
        if (this.extension.state === 3 /* ExtensionState.Uninstalled */ && this.extension.gallery && !this.extension.gallery.isSigned && shouldRequireRepositorySignatureFor(this.extension.private, await this.extensionGalleryManifestService.getExtensionGalleryManifest())) {
            this.updateStatus({ icon: warningIcon, message: new MarkdownString(localize(8257, null)) }, true);
            return;
        }
        if (this.extension.deprecationInfo) {
            if (this.extension.deprecationInfo.extension) {
                ...
                this.updateStatus({ icon: warningIcon, message: new MarkdownString(localize(8258, null, link)) }, true);
            }
            else if (this.extension.deprecationInfo.settings) {
                const link = `[${localize(8259, null)}](${createCommandUri('workbench.action.openSettings', this.extension.deprecationInfo.settings.map(setting => `@id:${setting}`).join(' '))}})`;
                this.updateStatus({ icon: warningIcon, message: new MarkdownString(localize(8260, null, link)) }, true);
            }
            else {
                const message = new MarkdownString(localize(8261, null));
        ...
        if (this.extension.missingFromGallery) {
            this.updateStatus({ icon: warningIcon, message: new MarkdownString(localize(8262, null)) }, true);
            return;
        }
        if (this.extensionsWorkbenchService.canSetLanguage(this.extension)) {
            return;
        ...
        if (this.extension.outdated) {
            ...
            if (message) {
                ...
                markdown.appendMarkdown(localize(8263, null, this.extension.hasChangelog()
        ...
        if (this.extension.enablementState === 7 /* EnablementState.DisabledByAllowlist */) {
            ...
            if (result !== true) {
                this.updateStatus({ icon: warningIcon, message: new MarkdownString(localize(8264, null, result.value)) }, true);
                return;
            }
        }
        // Extension is disabled by environment
        if (this.extension.enablementState === 2 /* EnablementState.DisabledByEnvironment */) {
            this.updateStatus({ message: new MarkdownString(localize(8265, null)) }, true);
            return;
        }
        // Extension is enabled by environment
        if (this.extension.enablementState === 3 /* EnablementState.EnabledByEnvironment */) {
            this.updateStatus({ message: new MarkdownString(localize(8266, null)) }, true);
            return;
        }
        ...
        if (this.extension.enablementState === 5 /* EnablementState.DisabledByVirtualWorkspace */) {
            ...
            this.updateStatus({ icon: infoIcon, message: new MarkdownString(details ? escapeMarkdownSyntaxTokens(details) : localize(8267, null)) }, true);
            return;
        }
        ...
        if (isVirtualWorkspace(this.contextService.getWorkspace())) {
            ...
            if (virtualSupportType === 'limited' || details) {
                this.updateStatus({ icon: warningIcon, message: new MarkdownString(details ? escapeMarkdownSyntaxTokens(details) : localize(8268, null)) }, true);
                return;
            }
        }
        // Unification
        if (this.extension.enablementState === 9 /* EnablementState.DisabledByUnification */) {
            this.updateStatus({ icon: infoIcon, message: new MarkdownString(localize(8269, null, '`chat.extensionUnification.enabled`')) }, true);
            return;
        }
        if (!this.workspaceTrustService.isWorkspaceTrusted() &&
            ...
            this.updateStatus({ icon: trustIcon, message: new MarkdownString(untrustedDetails ? escapeMarkdownSyntaxTokens(untrustedDetails) : localize(8270, null)) }, true);
            return;
        }
        ...
        if (this.workspaceTrustEnablementService.isWorkspaceTrustEnabled() && !this.workspaceTrustService.isWorkspaceTrusted()) {
            ...
            if (untrustedSupportType === 'limited' || untrustedDetails) {
                this.enabled = true;
                this.updateStatus({ icon: trustIcon, message: new MarkdownString(untrustedDetails ? escapeMarkdownSyntaxTokens(untrustedDetails) : localize(8271, null)) }, true);
                return;
            }
        }
                ...
                if (this.extensionManagementServerService.localExtensionManagementServer === this.extension.server) {
                    if (this.extensionManifestPropertiesService.prefersExecuteOnWorkspace(this.extension.local.manifest)) {
                        if (this.extensionManagementServerService.remoteExtensionManagementServer) {
                            message = new MarkdownString(`${localize(8272, null, this.extensionManagementServerService.remoteExtensionManagementServer.label)} [${localize(8273, null)}](https://code.visualstudio.com/api/advanced-topics/remote-extensions#architecture-and-extension-kinds)`);
                        }
                    }
                }
                // Extension on Remote Server
                    ...
                    if (this.extensionManifestPropertiesService.prefersExecuteOnUI(this.extension.local.manifest)) {
                        if (this.extensionManagementServerService.localExtensionManagementServer) {
                            message = new MarkdownString(`${localize(8274, null, this.extensionManagementServerService.remoteExtensionManagementServer.label)} [${localize(8275, null)}](https://code.visualstudio.com/api/advanced-topics/remote-extensions#architecture-and-extension-kinds)`);
                        }
                        else if (isWeb) {
                            message = new MarkdownString(`${localize(8276, null, this.productService.nameLong)} [${localize(8277, null)}](https://code.visualstudio.com/api/advanced-topics/remote-extensions#architecture-and-extension-kinds)`);
                        }
                    }
                }
                // Extension on Web Server
                else if (this.extensionManagementServerService.webExtensionManagementServer === this.extension.server) {
                    message = new MarkdownString(`${localize(8278, null, this.productService.nameLong)} [${localize(8279, null)}](https://code.visualstudio.com/api/advanced-topics/remote-extensions#architecture-and-extension-kinds)`);
                }
                if (message) {
                    this.updateStatus({ icon: warningIcon, message }, true);
                }
        ...
        for (const feature of features) {
            ...
            const manageAccessLink = `[${localize(8280, null)}](${createCommandUri('extension.open', this.extension.identifier.id, "features" /* ExtensionEditorTab.Features */, false, feature.id)})`;
        ...
        if (this.extensionManagementServerService.remoteExtensionManagementServer) {
            if (isLanguagePackExtension(this.extension.local.manifest)) {
                if (!this.extensionsWorkbenchService.installed.some(e => areSameExtensions(e.identifier, this.extension.identifier) && e.server !== this.extension.server)) {
                    const message = this.extension.server === this.extensionManagementServerService.localExtensionManagementServer
                        ? new MarkdownString(localize(8281, null, this.extensionManagementServerService.remoteExtensionManagementServer.label))
                        : new MarkdownString(localize(8282, null));
            ...
            if (this.extension.server === this.extensionManagementServerService.localExtensionManagementServer && runningExtensionServer === this.extensionManagementServerService.remoteExtensionManagementServer) {
                if (this.extensionManifestPropertiesService.prefersExecuteOnWorkspace(this.extension.local.manifest)) {
                    this.updateStatus({ icon: infoIcon, message: new MarkdownString(`${localize(8283, null)} [${localize(8284, null)}](https://code.visualstudio.com/api/advanced-topics/remote-extensions#architecture-and-extension-kinds)`) }, true);
                }
                return;
            }
            if (this.extension.server === this.extensionManagementServerService.remoteExtensionManagementServer && runningExtensionServer === this.extensionManagementServerService.localExtensionManagementServer) {
                if (this.extensionManifestPropertiesService.prefersExecuteOnUI(this.extension.local.manifest)) {
                    this.updateStatus({ icon: infoIcon, message: new MarkdownString(`${localize(8285, null)} [${localize(8286, null)}](https://code.visualstudio.com/api/advanced-topics/remote-extensions#architecture-and-extension-kinds)`) }, true);
                }
                return;
            }
            if (this.extension.server === this.extensionManagementServerService.remoteExtensionManagementServer && runningExtensionServer === this.extensionManagementServerService.webExtensionManagementServer) {
                if (this.extensionManifestPropertiesService.canExecuteOnWeb(this.extension.local.manifest)) {
                    this.updateStatus({ icon: infoIcon, message: new MarkdownString(`${localize(8287, null)} [${localize(8288, null)}](https://code.visualstudio.com/api/advanced-topics/remote-extensions#architecture-and-extension-kinds)`) }, true);
                }
                return;
            }
        }
        ...
        if (this.extension.enablementState === 8 /* EnablementState.DisabledByExtensionDependency */) {
            this.updateStatus({
                icon: warningIcon,
                message: new MarkdownString(localize(8289, null))
                    .appendMarkdown(`&nbsp;[${localize(8290, null)}](${createCommandUri('extension.open', this.extension.identifier.id, "dependencies" /* ExtensionEditorTab.Dependencies */)})`)
            }, true);
            return;
        }
        ...
        if (!this.extension.isWorkspaceScoped && isEnabled && isRunning) {
            if (this.extension.enablementState === 13 /* EnablementState.EnabledWorkspace */) {
                this.updateStatus({ message: new MarkdownString(localize(8291, null)) }, true);
                return;
            }
            if (this.extensionManagementServerService.localExtensionManagementServer && this.extensionManagementServerService.remoteExtensionManagementServer) {
                if (this.extension.server === this.extensionManagementServerService.remoteExtensionManagementServer) {
                    this.updateStatus({ message: new MarkdownString(localize(8292, null, this.extension.server.label)) }, true);
                    return;
                }
            }
        ...
        if (!isEnabled && !isRunning) {
            if (this.extension.enablementState === 10 /* EnablementState.DisabledGlobally */) {
                this.updateStatus({ message: new MarkdownString(localize(8293, null)) }, true);
                return;
            }
            if (this.extension.enablementState === 11 /* EnablementState.DisabledWorkspace */) {
                this.updateStatus({ message: new MarkdownString(localize(8294, null)) }, true);
                return;
            }
        }
    }
...
let InstallSpecificVersionOfExtensionAction = class InstallSpecificVersionOfExtensionAction extends Action {
    ...
    static { this.LABEL = localize(8295, null); }
    ...
    async run() {
        const extensionPick = await this.quickInputService.pick(this.getExtensionEntries(), { placeHolder: localize(8296, null), matchOnDetail: true });
...
let AbstractInstallExtensionsInServerAction = class AbstractInstallExtensionsInServerAction extends Action {
    ...
    async selectAndInstallExtensions() {
        ...
        if (localExtensionsToInstall.length) {
            ...
            quickPick.placeholder = localize(8297, null);
        ...
        if (selectedItems.length) {
            ...
            if (localExtensionsToInstall.length) {
                await this.progressService.withProgress({
                    ...
                    title: localize(8299, null)
                }, () => this.installExtensions(localExtensionsToInstall));
                this.notificationService.info(localize(8300, null));
            }
        }
    }
};
...
let InstallLocalExtensionsInRemoteAction = class InstallLocalExtensionsInRemoteAction extends AbstractInstallExtensionsInServerAction {
    ...
    get label() {
        if (this.extensionManagementServerService && this.extensionManagementServerService.remoteExtensionManagementServer) {
            return localize(8301, null, this.extensionManagementServerService.remoteExtensionManagementServer.label);
        }
        return '';
    }
    getQuickPickTitle() {
        return localize(8302, null, this.extensionManagementServerService.remoteExtensionManagementServer.label);
    }
...
let InstallRemoteExtensionsInLocalAction = class InstallRemoteExtensionsInLocalAction extends AbstractInstallExtensionsInServerAction {
    ...
    get label() {
        return localize(8303, null);
    }
    getQuickPickTitle() {
        return localize(8304, null);
    }
    getExtensionsToInstall(local) {
...
registerColor('extensionButton.separator', buttonSeparator, localize(8308, null));
