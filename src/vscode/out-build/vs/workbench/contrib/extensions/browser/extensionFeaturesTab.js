...
let RuntimeStatusMarkdownRenderer = class RuntimeStatusMarkdownRenderer extends Disposable {
    ...
    createElement(manifest, disposables) {
        ...
        if (this.extensionService.extensions.some(extension => ExtensionIdentifier.equals(extension.identifier, extensionId))) {
            ...
            data.appendMarkdown(`### ${localize(7948, null)}\n\n`);
        ...
        for (const feature of features) {
            ...
            if (accessData) {
                this.renderMarkdown(new MarkdownString(`\n ### ${localize(7949, null, feature.label)}\n\n`), container, disposables);
                if (accessData.accessTimes.length) {
                    const description = append(container, $('.feature-chart-description', undefined, localize(7950, null, accessData?.accessTimes.length, feature.accessDataLabel ?? feature.label)));
...
let ExtensionFeaturesTab = class ExtensionFeaturesTab extends Themable {
    ...
    create() {
        ...
        if (features.length === 0) {
            append($('.no-features'), this.domNode).textContent = localize(7954, null);
            return;
        }
...
let ExtensionFeatureItemRenderer = class ExtensionFeatureItemRenderer {
    ...
    renderTemplate(container) {
        ...
        disabledElement.textContent = localize(7956, null);
...
let ExtensionFeatureView = class ExtensionFeatureView extends Disposable {
        ...
        if (this.feature.access.canToggle) {
            ...
            this._register(button.onDidClick(async () => {
                ...
                const confirmationResult = await this.dialogService.confirm({
                    title: localize(7957, null, this.feature.label),
                    message: enabled
                        ? localize(7958, null, this.manifest.displayName ?? this.extensionId.value, this.feature.label)
                        : localize(7959, null, this.manifest.displayName ?? this.extensionId.value, this.feature.label),
                    custom: true,
                    primaryButton: enabled ? localize(7960, null) : localize(7961, null),
                    cancelButton: localize(7962, null),
                });
    ...
    updateButtonLabel(button) {
        button.label = this.extensionFeaturesManagementService.isEnabled(this.extensionId, this.feature.id) ? localize(7963, null) : localize(7964, null);
    }
