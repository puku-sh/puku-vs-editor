...
let ExtensionHostProfileService = class ExtensionHostProfileService extends Disposable {
    ...
    updateProfilingStatusBarIndicator(visible) {
        ...
        if (visible) {
            const indicator = {
                name: nls.localize(8565, null),
                text: nls.localize(8566, null),
                showProgress: true,
                ariaLabel: nls.localize(8567, null),
                tooltip: nls.localize(8568, null),
                command: 'workbench.action.extensionHostProfiler.stop'
            };
            const timeStarted = Date.now();
            const handle = disposableWindowInterval(mainWindow, () => {
                this.profilingStatusBarIndicator?.update({ ...indicator, text: nls.localize(8569, null, Math.round((new Date().getTime() - timeStarted) / 1000)), });
            }, 1000);
    ...
    async startProfiling() {
        ...
        if (inspectPorts.length === 0) {
            return this._dialogService.confirm({
                type: 'info',
                message: nls.localize(8570, null),
                detail: nls.localize(8571, null, this._productService.nameLong),
                primaryButton: nls.localize(8572, null)
            }).then(res => {
                if (res.confirmed) {
