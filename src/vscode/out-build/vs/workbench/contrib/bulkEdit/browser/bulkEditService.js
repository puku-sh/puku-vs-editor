...
let BulkEdit = class BulkEdit {
    ...
    ariaMessage() {
        ...
        if (this._edits.length === 0) {
            return localize(4916, null);
        }
        else if (otherResources.size === 0) {
            if (textEditCount > 1 && textEditResources.size > 1) {
                return localize(4917, null, textEditCount, textEditResources.size);
            }
            else {
                return localize(4918, null, textEditCount);
            }
        }
        else {
            return localize(4919, null, textEditCount, textEditResources.size, otherResources.size);
        }
    }
    ...
    async _performFileEdits(edits, undoRedoGroup, undoRedoSource, confirmBeforeUndo, progress) {
        ...
        const model = this._instaService.createInstance(BulkFileEdits, this._label || localize(4920, null), this._code || 'undoredo.workspaceEdit', undoRedoGroup, undoRedoSource, confirmBeforeUndo, progress, this._token, edits);
        return await model.apply();
    }
    async _performTextEdits(edits, undoRedoGroup, undoRedoSource, progress, reason) {
        this._logService.debug('_performTextEdits', JSON.stringify(edits));
        const model = this._instaService.createInstance(BulkTextEdits, this._label || localize(4921, null), this._code || 'undoredo.workspaceEdit', this._editor, undoRedoGroup, undoRedoSource, progress, this._token, edits);
...
let BulkEditService = class BulkEditService {
    ...
    async apply(editsIn, options) {
        ...
        if (edits.length === 0) {
            return { ariaSummary: localize(4922, null), isApplied: false };
        }
    ...
    async _shouldVeto(label, reason) {
        ...
        switch (reason) {
            case 1 /* ShutdownReason.CLOSE */:
                message = localize(4923, null);
                break;
            case 4 /* ShutdownReason.LOAD */:
                message = localize(4924, null);
                break;
            case 3 /* ShutdownReason.RELOAD */:
                message = localize(4925, null);
                break;
            default:
                message = isMacintosh ? localize(4926, null) : localize(4927, null);
                break;
        }
        const result = await this._dialogService.confirm({
            message,
            detail: localize(4928, null, label || localize(4929, null)),
        });
...
Registry.as(Extensions.Configuration).registerConfiguration({
    ...
    properties: {
        [autoSaveSetting]: {
            description: localize(4930, null),
            default: true,
            type: 'boolean'
        }
    }
