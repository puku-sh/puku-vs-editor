...
const turnOffSyncCommand = { id: 'workbench.userDataSync.actions.turnOff', title: localize2(14238, 'Turn Off') };
const configureSyncCommand = { id: CONFIGURE_SYNC_COMMAND_ID, title: localize2(14239, 'Configure...') };
...
const syncNowCommand = {
    ...
    title: localize2(14240, 'Sync Now'),
    description(userDataSyncService) {
        if (userDataSyncService.status === "syncing" /* SyncStatus.Syncing */) {
            return localize(14168, null);
        }
        if (userDataSyncService.lastSyncTime) {
            return localize(14169, null, fromNow(userDataSyncService.lastSyncTime, true));
        }
        return undefined;
    }
};
const showSyncSettingsCommand = { id: 'workbench.userDataSync.actions.settings', title: localize2(14241, 'Show Settings'), };
const showSyncedDataCommand = { id: 'workbench.userDataSync.actions.showSyncedData', title: localize2(14242, 'Show Synced Data'), };
...
let UserDataSyncWorkbenchContribution = class UserDataSyncWorkbenchContribution extends Disposable {
        ...
        if (conflicts.length) {
            ...
            for (const conflict of this.userDataSyncService.conflicts) {
                ...
                if (!this.conflictsDisposables.has(key)) {
                    ...
                    const handle = this.notificationService.prompt(Severity.Warning, localize(14170, null, conflictsArea.toLowerCase()), [
                        {
                            label: localize(14171, null),
                            run: () => {
                                this.acceptLocal(conflict, conflict.conflicts[0]);
                            }
                        },
                        {
                            label: localize(14172, null),
                            run: () => {
                                this.acceptRemote(conflict, conflict.conflicts[0]);
                            }
                        },
                        {
                            label: localize(14173, null),
                            run: () => {
    ...
    async acceptRemote(syncResource, conflict) {
        try {
            ...
            this.notificationService.error(localize(14174, null, `command:${SHOW_SYNC_LOG_COMMAND_ID}`));
        }
    }
    async acceptLocal(syncResource, conflict) {
        try {
            ...
            this.notificationService.error(localize(14175, null, `command:${SHOW_SYNC_LOG_COMMAND_ID}`));
        }
    }
        ...
        switch (error.code) {
            case "SessionExpired" /* UserDataSyncErrorCode.SessionExpired */:
                this.notificationService.notify({
                    ...
                    message: localize(14176, null),
                    actions: {
                        primary: [toAction({
                                id: 'turn on sync',
                                label: localize(14177, null),
                                run: () => this.turnOn()
                            })]
                    }
                });
            ...
            case "TurnedOff" /* UserDataSyncErrorCode.TurnedOff */:
                this.notificationService.notify({
                    severity: Severity.Info,
                    message: localize(14178, null),
                    actions: {
                        primary: [toAction({
                                id: 'turn on sync',
                                label: localize(14179, null),
                                run: () => this.turnOn()
                            })]
                    }
                });
            ...
            case "TooLarge" /* UserDataSyncErrorCode.TooLarge */:
                if (error.resource === "keybindings" /* SyncResource.Keybindings */ || error.resource === "settings" /* SyncResource.Settings */ || error.resource === "tasks" /* SyncResource.Tasks */) {
                    ...
                    this.handleTooLargeError(error.resource, localize(14180, null, sourceArea.toLowerCase(), sourceArea.toLowerCase(), '100kb'), error);
                }
                break;
            case "LocalTooManyProfiles" /* UserDataSyncErrorCode.LocalTooManyProfiles */:
                this.disableSync("profiles" /* SyncResource.Profiles */);
                this.notificationService.error(localize(14181, null));
                break;
            ...
            case "UpgradeRequired" /* UserDataSyncErrorCode.UpgradeRequired */: {
                const message = localize(14182, null, this.productService.version, this.productService.commit);
                const operationId = error.operationId ? localize(14183, null, error.operationId) : undefined;
                this.notificationService.notify({
            ...
            case "MethodNotFound" /* UserDataSyncErrorCode.MethodNotFound */: {
                const message = localize(14184, null);
                const operationId = error.operationId ? localize(14185, null, error.operationId) : undefined;
                this.notificationService.notify({
                    severity: Severity.Error,
                    message: operationId ? `${message} ${operationId}` : message,
                    actions: {
            ...
            case "IncompatibleRemoteContent" /* UserDataSyncErrorCode.IncompatibleRemoteContent */:
                this.notificationService.notify({
                    severity: Severity.Error,
                    message: localize(14188, null),
                    actions: {
                        primary: [
                            toAction({
                                id: 'reset',
                                label: localize(14189, null),
                                run: () => this.userDataSyncWorkbenchService.resetSyncedData()
                            }),
                            toAction({
                                id: 'show synced data',
                                label: localize(14190, null),
            ...
            case "DefaultServiceChanged" /* UserDataSyncErrorCode.DefaultServiceChanged */:
                ...
                // If settings sync got turned off then ask user to turn on sync again.
                    ...
                    this.notificationService.notify({
                        severity: Severity.Info,
                        message: localize(14194, null, this.productService.nameLong),
                        actions: {
                            primary: [toAction({
                                    id: 'turn on sync',
                                    label: localize(14195, null),
                                    run: () => this.turnOn()
                                })]
                        }
                    });
    ...
    handleTooLargeError(resource, message, error) {
        const operationId = error.operationId ? localize(14196, null, error.operationId) : undefined;
        this.notificationService.notify({
            severity: Severity.Error,
            message: operationId ? `${message} ${operationId}` : message,
            actions: {
                primary: [toAction({
                        id: 'open sync file',
                        label: localize(14197, null, getSyncAreaLabel(resource)),
    ...
    handleInvalidContentError({ profile, syncResource: source }) {
        ...
        const handle = this.notificationService.notify({
            severity: Severity.Error,
            message: localize(14198, null, errorArea.toLowerCase()),
            actions: {
                primary: [toAction({
                        id: 'open sync file',
                        label: localize(14199, null, errorArea),
    ...
    async updateGlobalActivityBadge() {
        ...
        if (this.userDataSyncService.conflicts.length && this.userDataSyncEnablementService.isEnabled()) {
            badge = new NumberBadge(this.getConflictsCount(), () => localize(14200, null, SYNC_TITLE.value));
        }
        else if (this.turningOnSync) {
            badge = new ProgressBadge(() => localize(14201, null));
        }
        if (badge) {
    ...
    async updateAccountBadge() {
        ...
        if (this.userDataSyncService.status !== "uninitialized" /* SyncStatus.Uninitialized */ && this.userDataSyncEnablementService.isEnabled() && this.userDataSyncWorkbenchService.accountStatus === "unavailable" /* AccountStatus.Unavailable */) {
            badge = new NumberBadge(1, () => localize(14202, null));
        }
        if (badge) {
    ...
    async turnOn() {
        try {
            if (!this.userDataSyncWorkbenchService.authenticationProviders.length) {
                throw new Error(localize(14203, null));
            }
                ...
                switch (e.code) {
                    case "TooLarge" /* UserDataSyncErrorCode.TooLarge */:
                        if (e.resource === "keybindings" /* SyncResource.Keybindings */ || e.resource === "settings" /* SyncResource.Settings */ || e.resource === "tasks" /* SyncResource.Tasks */) {
                            this.handleTooLargeError(e.resource, localize(14204, null, getSyncAreaLabel(e.resource).toLowerCase(), '100kb'), e);
                            return;
                        }
                        break;
                    case "IncompatibleLocalContent" /* UserDataSyncErrorCode.IncompatibleLocalContent */:
                    case "Gone" /* UserDataSyncErrorCode.Gone */:
                    case "UpgradeRequired" /* UserDataSyncErrorCode.UpgradeRequired */: {
                        const message = localize(14205, null, this.productService.version, this.productService.commit);
                        const operationId = e.operationId ? localize(14206, null, e.operationId) : undefined;
                        this.notificationService.notify({
                            severity: Severity.Error,
                            message: operationId ? `${message} ${operationId}` : message,
                        });
                    ...
                    case "IncompatibleRemoteContent" /* UserDataSyncErrorCode.IncompatibleRemoteContent */:
                        this.notificationService.notify({
                            severity: Severity.Error,
                            message: localize(14207, null),
                            actions: {
                                primary: [
                                    toAction({
                                        id: 'reset',
                                        label: localize(14208, null),
                                        run: () => this.userDataSyncWorkbenchService.resetSyncedData()
                                    }),
                                    toAction({
                                        id: 'show synced data',
                                        label: localize(14209, null),
                                        run: () => this.userDataSyncWorkbenchService.showSyncActivity()
                                    })
                                ]
                            }
                    ...
                    case "Forbidden" /* UserDataSyncErrorCode.Forbidden */:
                        this.notificationService.error(localize(14210, null));
                        return;
                }
                this.notificationService.error(localize(14211, null, `command:${SHOW_SYNC_LOG_COMMAND_ID}`));
            }
            else {
                this.notificationService.error(localize(14212, null, getErrorMessage(e)));
            }
        }
    }
    async askToConfigure() {
        return new Promise((c, e) => {
            ...
            quickPick.customLabel = localize(14213, null);
            quickPick.description = localize(14214, null);
    ...
    async configureSyncOptions() {
        return new Promise((c, e) => {
            ...
            const quickPick = this.quickInputService.createQuickPick();
            disposables.add(quickPick);
            quickPick.title = localize(14215, null, SYNC_TITLE.value);
            quickPick.placeholder = localize(14216, null);
            quickPick.canSelectMany = true;
            quickPick.ignoreFocusOut = true;
            quickPick.ok = true;
            const items = this.getConfigureSyncQuickPickItems();
    ...
    async turnOff() {
        const result = await this.dialogService.confirm({
            message: localize(14217, null),
            detail: localize(14218, null),
            primaryButton: localize(14219, null),
            checkbox: this.userDataSyncWorkbenchService.accountStatus === "available" /* AccountStatus.Available */ ? {
                label: localize(14220, null)
            } : undefined
        });
        if (result.confirmed) {
    ...
    async selectSettingsSyncService(userDataSyncStore) {
        return new Promise((c, e) => {
            ...
            quickPick.title = localize(14221, null, SYNC_TITLE.value);
            quickPick.description = localize(14222, null);
            quickPick.hideInput = true;
            quickPick.ignoreFocusOut = true;
            const getDescription = (url) => {
                const isDefault = isEqual(url, userDataSyncStore.defaultUrl);
                if (isDefault) {
                    return localize(14223, null);
                }
                return undefined;
            };
            quickPick.items = [
        ...
        this._register(registerAction2(class TurningOnSyncAction extends Action2 {
            constructor() {
                super({
                    ...
                    title: localize2(14243, 'Backup and Sync Settings...'),
                    category: SYNC_TITLE,
                    f1: true,
                    precondition: when,
                    menu: [{
    ...
    registerSignInAction() {
        ...
        this._register(MenuRegistry.appendMenuItem(MenuId.AccountsContext, {
            ...
            command: {
                id,
                title: localize(14229, null),
            },
            when
        }));
    }
    getShowConflictsTitle() {
        return localize2(14244, "Show Conflicts ({0})", this.getConflictsCount());
    }
        ...
        this._register(registerAction2(class StopSyncAction extends Action2 {
            ...
            async run() {
                try {
                    ...
                    if (!isCancellationError(e)) {
                        that.notificationService.error(localize(14231, null, `command:${SHOW_SYNC_LOG_COMMAND_ID}`));
                    }
                }
            }
        }));
        ...
        this._register(registerAction2(class ShowSyncActivityAction extends Action2 {
            constructor() {
                super({
                    ...
                    title: localize(14233, null, SYNC_TITLE.value),
                    tooltip: localize(14234, null),
                    icon: Codicon.output,
                    menu: [{
                            id: MenuId.CommandPalette,
    ...
    registerDownloadSyncActivityAction() {
        this._register(registerAction2(class DownloadSyncActivityAction extends Action2 {
            ...
            async run(accessor) {
                ...
                if (folder) {
                    notificationService.info(localize(14236, null));
                }
            }
        }));
    }
