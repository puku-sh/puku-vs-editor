    ...
    update(askToRelaunch) {
        ...
        if (askToRelaunch && changed && this.hostService.hasFocus) {
            this.doConfirm(isNative ?
                localize(11173, null) :
                localize(11174, null), isNative ?
                localize(11175, null, this.productService.nameLong) :
                localize(11176, null, this.productService.nameLong), isNative ?
                localize(11177, null) :
                localize(11178, null), () => this.hostService.restart());
        }
    }
    ...
    constructor(contextService, extensionService, hostService, environmentService) {
        ...
        this.extensionHostRestarter = this._register(new RunOnceScheduler(async () => {
            ...
            if (environmentService.remoteAuthority) {
                ...
                const stopped = await extensionService.stopExtensionHosts(localize(11179, null));
                if (stopped) {
