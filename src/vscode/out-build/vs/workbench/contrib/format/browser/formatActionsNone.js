...
registerEditorAction(class FormatDocumentMultipleAction extends EditorAction {
    constructor() {
        super({
            ...
            label: nls.localize2(9037, "Format Document"),
    ...
    async run(accessor, editor) {
        ...
        if (formatterCount > 1) {
            ...
            notificationService.warn(nls.localize(9034, null));
        }
        else {
            const langName = model.getLanguageId();
            const message = nls.localize(9035, null, langName);
            const { confirmed } = await dialogService.confirm({
                message,
                primaryButton: nls.localize(9036, null)
            });
            if (confirmed) {
