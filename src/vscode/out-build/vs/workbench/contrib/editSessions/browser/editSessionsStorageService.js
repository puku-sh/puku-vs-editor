...
let EditSessionsWorkbenchService = class EditSessionsWorkbenchService extends Disposable {
    ...
    async getAccountPreference(reason) {
        ...
        quickpick.ok = false;
        quickpick.placeholder = reason === 'read' ? localize(7837, null) : localize(7838, null);
    ...
    async createQuickpickItems() {
        const options = [];
        options.push({ type: 'separator', label: localize(7839, null) });
        const sessions = await this.getAllSessions();
        options.push(...sessions);
        options.push({ type: 'separator', label: localize(7840, null) });
        for (const authenticationProvider of (await this.getAuthenticationProviders())) {
            ...
            if (!signedInForProvider || this.authenticationService.getProvider(authenticationProvider.id).supportsMultipleAccounts) {
                ...
                options.push({ label: localize(7841, null, providerName), provider: authenticationProvider });
            }
        }
        return options;
    }
        ...
        this._register(registerAction2(class ResetEditSessionAuthenticationAction extends Action2 {
            ...
            async run() {
                const result = await that.dialogService.confirm({
                    message: localize(7845, null),
                    checkbox: { label: localize(7846, null) }
                });
                if (result.confirmed) {
