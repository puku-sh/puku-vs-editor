...
let EditSessionsDataViews = class EditSessionsDataViews extends Disposable {
    ...
    registerViews(container) {
        ...
        viewsRegistry.registerViewWelcomeContent(viewId, {
            content: localize(7847, null, `[${localize(7848, null)}](${createCommandUri('workbench.editSessions.actions.store')})`),
        ...
        this._register(registerAction2(class extends Action2 {
            ...
            async run(accessor, handle) {
                ...
                const result = await dialogService.confirm({
                    message: localize(7852, null, editSessionId),
                    detail: localize(7853, null),
                    type: 'warning',
        ...
        this._register(registerAction2(class extends Action2 {
            ...
            async run(accessor) {
                ...
                const editSessionStorageService = accessor.get(IEditSessionsStorageService);
                const result = await dialogService.confirm({
                    message: localize(7855, null),
                    detail: localize(7856, null),
                    type: 'warning',
                    title: EDIT_SESSIONS_TITLE.value
                });
                if (result.confirmed) {
            ...
            if (currentWorkspaceFolder?.uri) {
                ...
                if (change.type === ChangeType.Addition && await this.fileService.exists(localCopy)) {
                    return {
                        ...
                        command: {
                            id: 'vscode.diff',
                            title: localize(7857, null),
                            arguments: [
                                localCopy,
                                cloudChangeUri,
                                `${basename(change.relativeFilePath)} (${localize(7858, null)} \u2194 ${localize(7859, null)})`,
                                undefined
                            ]
                        }
                    };
