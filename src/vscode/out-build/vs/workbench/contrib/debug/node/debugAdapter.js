        ...
        try {
            ...
            if (command) {
                if (path.isAbsolute(command)) {
                    ...
                    if (!commandExists) {
                        throw new Error(nls.localize(7774, null, command));
                    }
                }
                else {
                    // relative path
                ...
                throw new Error(nls.localize(7775, null, this.debugType));
            }
            let env = process.env;
            ...
            if (command === 'node') {
                if (Array.isArray(args) && args.length > 0) {
                    ...
                    if (!child.pid) {
                        throw new Error(nls.localize(7776, null, args[0]));
                    }
                    this.serverProcess = child;
                }
                else {
                    throw new Error(nls.localize(7777, null));
                }
            }
            else {
