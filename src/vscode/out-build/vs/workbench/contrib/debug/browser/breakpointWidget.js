...
let BreakpointWidget = class BreakpointWidget extends ZoneWidget {
    ...
    get placeholder() {
        ...
        switch (this.context) {
            case 2 /* Context.LOG_MESSAGE */:
                return nls.localize(7108, null, acceptString, closeString);
            case 1 /* Context.HIT_COUNT */:
                return nls.localize(7109, null, acceptString, closeString);
            default:
                return nls.localize(7110, null, acceptString, closeString);
        }
    }
    ...
    _fillContainer(container) {
        ...
        const selectBox = this.store.add(new SelectBox([
            { text: nls.localize(7111, null) },
            { text: nls.localize(7112, null) },
            { text: nls.localize(7113, null) },
            { text: nls.localize(7114, null) },
        ], this.context, this.contextViewService, defaultSelectBoxStyles, { ariaLabel: nls.localize(7115, null), useCustomDrawn: !hasNativeContextMenu(this._configurationService) }));
    ...
    createModesInput(container) {
        ...
        const sb = this.selectModeBox = new SelectBox([
            { text: nls.localize(7116, null), isDisabled: true },
    ...
    createTriggerBreakpointInput(container) {
        ...
        const breakpointOptions = [
            { text: nls.localize(7117, null), isDisabled: true },
            ...breakpoints.map(bp => ({
                text: `${this.labelService.getUriLabel(bp.uri, { relative: true })}: ${bp.lineNumber}`,
                description: nls.localize(7118, null)
            })),
        ];
        ...
        for (const [i, bp] of breakpoints.entries()) {
            this.textModelService.createModelReference(bp.uri).then(ref => {
                ...
                breakpointOptions[i + 1].description = nls.localize(7119, null);
            });
        }
        const selectBreakpointBox = this.selectBreakpointBox = this.store.add(new SelectBox(breakpointOptions, index + 1, this.contextViewService, defaultSelectBoxStyles, { ariaLabel: nls.localize(7120, null), useCustomDrawn: !hasNativeContextMenu(this._configurationService) }));
        ...
        closeButton.label = nls.localize(7121, null);
