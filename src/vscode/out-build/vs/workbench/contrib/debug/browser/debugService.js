...
let DebugService = class DebugService {
    constructor(editorService, paneCompositeService, viewsService, viewDescriptorService, notificationService, dialogService, layoutService, contextService, contextKeyService, lifecycleService, instantiationService, extensionService, fileService, configurationService, extensionHostDebugService, activityService, commandService, quickInputService, workspaceTrustRequestService, uriIdentityService, testService) {
        ...
        this.disposables.add(this.model.onDidChangeCallStack(() => {
            ...
            if (numberOfSessions > 0) {
                ...
                if (viewContainer) {
                    this.activity = this.activityService.showViewContainerActivity(viewContainer.id, { badge: new NumberBadge(numberOfSessions, n => n === 1 ? nls.localize(7433, null) : nls.localize(7434, null, n)) });
                }
            }
        }));
        ...
        this.disposables.add(extensionService.onWillStop(evt => {
            evt.veto(this.model.getSessions().length > 0, nls.localize(7435, null));
        }));
    ...
    async startDebugging(launch, configOrName, options, saveBeforeStart = !options?.parentSession) {
        const message = options && options.noDebug ? nls.localize(7436, null) : nls.localize(7437, null);
        ...
        try {
            ...
            if (compound) {
                ...
                if (!compound.configurations) {
                    throw new Error(nls.localize(7438, null));
                }
                ...
                const values = await Promise.all(compound.configurations.map(configData => {
                    ...
                    if (typeof configData === 'string') {
                        ...
                        if (launchesContainingName.length === 1) {
                            ...
                            throw new Error(launchesContainingName.length === 0 ? nls.localize(7439, null, name)
                                : nls.localize(7440, null, name));
                        }
                    }
                        ...
                        if (launchesMatchingConfigData.length === 1) {
                            ...
                            throw new Error(nls.localize(7441, null, configData.folder, configData.name, compound.name));
                        }
                    }
            ...
            if (configOrName && !config) {
                const message = !!launch ? nls.localize(7442, null, typeof configOrName === 'string' ? configOrName : configOrName.name) :
                    nls.localize(7443, null);
            ...
            try {
                ...
                if (!dbg || (configByProviders.request !== 'attach' && configByProviders.request !== 'launch')) {
                    ...
                    if (configByProviders.request !== 'attach' && configByProviders.request !== 'launch') {
                        message = configByProviders.request ? nls.localize(7444, null, 'request', configByProviders.request)
                            : nls.localize(7445, null, 'request');
                    }
                    else {
                        message = resolvedConfig.type ? nls.localize(7446, null, resolvedConfig.type) :
                            nls.localize(7447, null);
                    }
                    const actionList = [];
                    actionList.push(toAction({
                        id: 'installAdditionalDebuggers',
                        label: nls.localize(7448, null, resolvedConfig.type),
                        enabled: true,
                ...
                if (err && err.message) {
                    ...
                    await this.showError(nls.localize(7449, null));
                }
    ...
    async confirmConcurrentSession(sessionLabel) {
        const result = await this.dialogService.confirm({
            message: nls.localize(7450, null, sessionLabel)
        });
        ...
        listenerDisposables.add(session.onDidEndAdapter(async (adapterExitEvent) => {
            if (adapterExitEvent) {
                if (adapterExitEvent.error) {
                    this.notificationService.error(nls.localize(7451, null, adapterExitEvent.error.message || adapterExitEvent.error.toString()));
                }
                ...
                if (editor.input === DisassemblyViewInput.instance) {
                    ...
                    if (stackFrame && isCodeEditor(control) && control.hasModel()) {
                        ...
                        if (lineNumber >= 1 && lineNumber <= model.getLineCount()) {
                            ...
                            aria.alert(nls.localize(7452, null, lineContent, thread && thread.stoppedDetails ? `, reason ${thread.stoppedDetails.reason}` : '', stackFrame.source ? stackFrame.source.name : '', stackFrame.range.startLineNumber));
                        }
                    }
                }
            }
    ...
    async addBreakpoints(uri, rawBreakpoints, ariaAnnounce = true) {
        ...
        if (ariaAnnounce) {
            breakpoints.forEach(bp => aria.status(nls.localize(7453, null, bp.lineNumber, uri.fsPath)));
        }
    ...
    async removeBreakpoints(id) {
        ...
        toRemove.forEach(bp => aria.status(nls.localize(7454, null, bp.lineNumber, bp.uri.fsPath)));
