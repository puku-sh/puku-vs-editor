    ...
    constructor(options, contextMenuService, debugService, keybindingService, instantiationService, viewDescriptorService, configurationService, contextKeyService, openerService, themeService, hoverService, menuService) {
        ...
        this.onCallStackChangeScheduler = this._register(new RunOnceScheduler(async () => {
            ...
            if (stoppedDetails && (thread || typeof stoppedDetails.threadId !== 'number')) {
                ...
                this.stateMessageLabel.textContent = localize(7124, null);
...
let SessionsRenderer = class SessionsRenderer {
    ...
    doRenderElement(session, matches, data) {
        const sessionHover = data.elementDisposable.add(this.hoverService.setupManagedHover(getDefaultHoverDelegate('mouse'), data.session, localize(7126, null)));
        ...
        if (stoppedDetails) {
            ...
            data.stateLabel.textContent = localize(7127, null);
...
let StackFramesRenderer = class StackFramesRenderer {
    ...
    renderElement(element, index, data) {
        ...
        data.actionBar.clear();
        if (hasActions) {
            const action = data.elementDisposables.add(new Action('debug.callStack.restartFrame', localize(7128, null), ThemeIcon.asClassName(icons.debugRestartFrame), true, async () => {
                try {
...
class LoadMoreRenderer {
    ...
    static { this.LABEL = localize(7129, null); }
    constructor() { }
    get templateId() {
...
class ShowMoreRenderer {
    ...
    renderElement(element, index, data) {
        ...
        if (stackFrames.every(sf => !!(sf.source && sf.source.origin && sf.source.origin === stackFrames[0].source.origin))) {
            data.label.textContent = localize(7130, null, stackFrames.length, stackFrames[0].source.origin);
        }
        else {
            data.label.textContent = localize(7131, null, stackFrames.length);
        }
    }
    renderCompressedElements(node, index, templateData) {
        throw new Error('Method not implemented.');
...
function stoppedDescription(stoppedDetails) {
    return stoppedDetails.description ||
        (stoppedDetails.reason ? localize(7132, null, stoppedDetails.reason) : localize(7133, null));
}
...
class CallStackAccessibilityProvider {
    ...
    getAriaLabel(element) {
        if (element instanceof Thread) {
            return localize(7135, null, element.name, element.stateLabel);
        }
        if (element instanceof StackFrame) {
            return localize(7136, null, element.name, element.range.startLineNumber, getSpecificSourceName(element));
        }
        if (isDebugSession(element)) {
            const thread = element.getAllThreads().find(t => t.stopped);
            const state = thread ? thread.stateLabel : localize(7137, null);
            return localize(7138, null, element.getLabel(), state);
        }
        if (typeof element === 'string') {
            return element;
        }
        if (element instanceof Array) {
            return localize(7139, null, element.length);
        }
