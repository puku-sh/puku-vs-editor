...
class ExceptionBreakpointsRenderer {
    ...
    renderElement(exceptionBreakpoint, index, data) {
        ...
        const exceptionBreakpointtitle = exceptionBreakpoint.verified ? (exceptionBreakpoint.description || data.name.textContent) : exceptionBreakpoint.message || localize(7038, null);
        ...
        data.elementDisposables.add(this.hoverService.setupManagedHover(getDefaultHoverDelegate('mouse'), data.condition, localize(7039, null, exceptionBreakpoint.condition)));
...
let FunctionBreakpointsRenderer = class FunctionBreakpointsRenderer {
    ...
    renderElement(functionBreakpoint, _index, data) {
        ...
        if (functionBreakpoint.condition && functionBreakpoint.hitCondition) {
            data.condition.textContent = localize(7040, null, functionBreakpoint.condition, functionBreakpoint.hitCondition);
        }
        else {
        ...
        if (session && !session.capabilities.supportsFunctionBreakpoints) {
            data.elementDisposables.add(this.hoverService.setupManagedHover(getDefaultHoverDelegate('mouse'), data.breakpoint, localize(7041, null)));
        }
        this.breakpointSupportsCondition.set(!session || !!session.capabilities.supportsConditionalBreakpoints);
...
let DataBreakpointsRenderer = class DataBreakpointsRenderer {
    ...
    renderElement(dataBreakpoint, _index, data) {
        ...
        if (session && !session.capabilities.supportsDataBreakpoints) {
            data.elementDisposables.add(this.hoverService.setupManagedHover(getDefaultHoverDelegate('mouse'), data.breakpoint, localize(7042, null)));
        }
        if (dataBreakpoint.accessType) {
            const accessType = dataBreakpoint.accessType === 'read' ? localize(7043, null) : dataBreakpoint.accessType === 'write' ? localize(7044, null) : localize(7045, null);
        ...
        if (dataBreakpoint.condition && dataBreakpoint.hitCondition) {
            data.condition.textContent = localize(7046, null, dataBreakpoint.condition, dataBreakpoint.hitCondition);
        }
        else {
...
let InstructionBreakpointsRenderer = class InstructionBreakpointsRenderer {
    ...
    renderElement(breakpoint, index, data) {
        ...
        data.elementDisposables.add(this.hoverService.setupManagedHover(getDefaultHoverDelegate('mouse'), data.name, localize(7047, null, breakpoint.address.toString())));
        data.checkbox.checked = breakpoint.enabled;
        const { message, icon } = getBreakpointMessageAndIcon(this.debugService.state, this.debugService.getModel().areBreakpointsActivated(), breakpoint, this.labelService, this.debugService.getModel());
        data.icon.className = ThemeIcon.asClassName(icon);
        data.elementDisposables.add(this.hoverService.setupManagedHover(getDefaultHoverDelegate('mouse'), data.breakpoint, breakpoint.message || message || ''));
...
class FunctionBreakpointInputRenderer {
    ...
    renderElement(functionBreakpoint, _index, data) {
        ...
        let placeholder = localize(7048, null);
        let ariaLabel = localize(7049, null);
        if (data.type === 'condition') {
            data.inputBox.value = functionBreakpoint.condition || '';
            placeholder = localize(7050, null);
            ariaLabel = localize(7051, null);
        }
        else if (data.type === 'hitCount') {
            data.inputBox.value = functionBreakpoint.hitCondition || '';
            placeholder = localize(7052, null);
            ariaLabel = localize(7053, null);
        }
...
class DataBreakpointInputRenderer {
    ...
    renderElement(dataBreakpoint, _index, data) {
        ...
        if (data.type === 'condition') {
            ...
            placeholder = localize(7054, null);
            ariaLabel = localize(7055, null);
        }
        else if (data.type === 'hitCount') {
            data.inputBox.value = dataBreakpoint.hitCondition || '';
            placeholder = localize(7056, null);
            ariaLabel = localize(7057, null);
        }
        data.inputBox.setAriaLabel(ariaLabel);
        data.inputBox.setPlaceHolder(placeholder);
        setTimeout(() => {
...
class ExceptionBreakpointInputRenderer {
    ...
    renderElement(exceptionBreakpoint, _index, data) {
        const placeHolder = exceptionBreakpoint.conditionDescription || localize(7059, null);
        data.inputBox.setPlaceHolder(placeHolder);
...
export function getBreakpointMessageAndIcon(state, breakpointsActivated, breakpoint, labelService, debugModel) {
    ...
    if (!breakpoint.enabled || !breakpointsActivated) {
        return {
            ...
            message: breakpoint.logMessage ? localize(7061, null) : localize(7062, null),
        };
    }
    ...
    if (debugActive && !breakpoint.verified) {
        return {
            ...
            message: ('message' in breakpoint && breakpoint.message) ? breakpoint.message : (breakpoint.logMessage ? localize(7063, null) : localize(7064, null)),
    ...
    if (breakpoint instanceof DataBreakpoint) {
        if (!breakpoint.supported) {
            return {
                icon: breakpointIcon.unverified,
                message: localize(7065, null),
            };
        }
        return {
            icon: breakpointIcon.regular,
            message: breakpoint.message || localize(7066, null)
        };
    }
    if (breakpoint instanceof FunctionBreakpoint) {
        if (!breakpoint.supported) {
            return {
                icon: breakpointIcon.unverified,
                message: localize(7067, null),
            };
        }
        const messages = [];
        messages.push(breakpoint.message || localize(7068, null));
        if (breakpoint.condition) {
            messages.push(localize(7069, null, breakpoint.condition));
        }
        if (breakpoint.hitCondition) {
            messages.push(localize(7070, null, breakpoint.hitCondition));
        }
        return {
            icon: breakpointIcon.regular,
    ...
    if (breakpoint instanceof InstructionBreakpoint) {
        ...
        if (breakpoint.message) {
            ...
            messages.push(localize(7072, null, breakpoint.instructionReference));
        }
        else {
            messages.push(localize(7073, null));
        }
        if (breakpoint.hitCondition) {
            messages.push(localize(7074, null, breakpoint.hitCondition));
        }
        return {
            icon: breakpointIcon.regular,
            message: appendMessage(messages.join('\n'))
    ...
    if (breakpoint.logMessage || breakpoint.condition || breakpoint.hitCondition || triggeringBreakpoint) {
        ...
        if (!breakpoint.supported) {
            ...
            messages.push(localize(7075, null));
        }
        if (breakpoint.logMessage) {
            messages.push(localize(7076, null, breakpoint.logMessage));
        }
        if (breakpoint.condition) {
            messages.push(localize(7077, null, breakpoint.condition));
        }
        if (breakpoint.hitCondition) {
            messages.push(localize(7078, null, breakpoint.hitCondition));
        }
        if (triggeringBreakpoint) {
            messages.push(localize(7079, null, `${labelService.getUriLabel(triggeringBreakpoint.uri, { relative: true })}: ${triggeringBreakpoint.lineNumber}`));
        }
        return {
            icon,
            message: appendMessage(messages.join('\n'))
    ...
    const message = ('message' in breakpoint && breakpoint.message) ? breakpoint.message : breakpoint instanceof Breakpoint && labelService ? labelService.getUriLabel(breakpoint.uri) : localize(7080, null);
    return {
        icon: breakpointIcon.regular,
        message
    };
...
registerAction2(class extends Action2 {
    constructor() {
        super({
            ...
            title: {
                ...localize2(7100, "Add Function Breakpoint"),
                mnemonicTitle: localize(7081, null),
            },
            f1: true,
...
class MemoryBreakpointAction extends Action2 {
    async run(accessor, existingBreakpoint) {
        ...
        try {
            ...
            notifications.error(localize(7082, null, range.address, e.message));
        }
        if (!info?.dataId) {
            return;
        }
        ...
        if (info.accessTypes && info.accessTypes?.length > 1) {
            ...
            const selectedAccessType = await quickInput.pick(accessTypes, { placeHolder: localize(7083, null) });
    ...
    getRange(quickInput, defaultValue) {
        return new Promise(resolve => {
            ...
            input.prompt = localize(7084, null);
            input.placeholder = localize(7085, null);
            if (defaultValue) {
    ...
    parseAddress(range, isFinal) {
        ...
        if (!parts) {
            return { error: localize(7086, null) };
        }
        ...
        for (const n of [startStr, endStr]) {
            if (!isNum(n)) {
                return { error: localize(7087, null, n) };
            }
        }
        if (!isFinal) {
            return;
...
registerAction2(class extends MemoryBreakpointAction {
    constructor() {
        super({
            ...
            title: {
                ...localize2(7101, "Add Data Breakpoint at Address"),
                mnemonicTitle: localize(7088, null),
            },
            f1: true,
...
registerAction2(class extends MemoryBreakpointAction {
    constructor() {
        super({
            ...
            title: localize2(7102, "Edit Address..."),
            menu: [{
...
registerAction2(class extends Action2 {
    constructor() {
        super({
            ...
            title: localize2(7103, 'Toggle Activate Breakpoints'),
            f1: true,
...
registerAction2(class extends Action2 {
    constructor() {
        super({
            ...
            title: {
                ...localize2(7104, "Remove All Breakpoints"),
                mnemonicTitle: localize(7090, null),
            },
            f1: true,
...
registerAction2(class extends Action2 {
    constructor() {
        super({
            ...
            title: {
                ...localize2(7105, "Enable All Breakpoints"),
                mnemonicTitle: localize(7091, null),
            },
            f1: true,
...
registerAction2(class extends Action2 {
    constructor() {
        super({
            ...
            title: {
                ...localize2(7106, "Disable All Breakpoints"),
                mnemonicTitle: localize(7092, null),
            },
            f1: true,
            precondition: CONTEXT_DEBUGGERS_AVAILABLE,
            menu: [{
...
registerAction2(class extends Action2 {
    constructor() {
        super({
            ...
            title: localize2(7107, 'Reapply All Breakpoints'),
            f1: true,
...
registerAction2(class extends ViewAction {
    ...
    async runInView(accessor, view, breakpoint) {
        ...
        if (breakpoint instanceof Breakpoint) {
            ...
            const actions = [new Action('breakpoint.editCondition', localize(7094, null), undefined, true, async () => view.renderInputBox({ breakpoint, type: 'condition' })),
                new Action('breakpoint.editCondition', localize(7095, null), undefined, true, async () => view.renderInputBox({ breakpoint, type: 'hitCount' }))];
...
registerAction2(class extends ViewAction {
    ...
    async runInView(accessor, view, breakpoint) {
        ...
        const picked = await accessor.get(IQuickInputService).pick(modes.map(mode => ({ label: mode.label, description: mode.description, mode: mode.mode })), { placeHolder: localize(7099, null) });
        if (!picked) {
            return;
        }
        if (kind === 'source') {
