...
const ABORT_LABEL = nls.localize(7504, null);
const DEBUG_ANYWAY_LABEL = nls.localize(7505, null);
const DEBUG_ANYWAY_LABEL_NO_MEMO = nls.localize(7506, null);
let DebugTaskRunner = class DebugTaskRunner {
    ...
    async runTaskAndCheckErrors(root, taskId) {
        try {
            ...
            const message = errorCount > 1
                ? nls.localize(7507, null, taskLabel)
                : errorCount === 1
                    ? nls.localize(7508, null, taskLabel)
                    : taskSummary && typeof taskSummary.exitCode === 'number'
                        ? nls.localize(7509, null, taskLabel, taskSummary.exitCode)
                        : nls.localize(7510, null, taskLabel);
            let DebugChoice;
    ...
    async runTask(root, taskId, token = this.globalCancellation.token) {
        ...
        if (!root) {
            return Promise.reject(new Error(nls.localize(7515, null, typeof taskId === 'string' ? taskId : taskId.type)));
        }
        // run a task before starting a debug session
        const task = await this.taskService.getTask(root, taskId);
        if (!task) {
            const errorMessage = typeof taskId === 'string'
                ? nls.localize(7516, null, taskId)
                : nls.localize(7517, null);
        ...
        const result = new Promise((resolve, reject) => {
            ...
            store.add(didAcquireInput.event(() => {
                ...
                store.add(disposableTimeout(() => {
                    if (!taskStarted) {
                        const errorMessage = nls.localize(7518, null, typeof taskId === 'string' ? taskId : JSON.stringify(taskId));
                ...
                if (!hideSlowPreLaunchWarning) {
                    ...
                    store.add(disposableTimeout(() => {
                        const message = nls.localize(7519, null, task.configurationProperties.name);
                        ...
                        if (canConfigure) {
                            buttons.splice(1, 0, nls.localize(7520, null));
                        }
