...
const disassemblyNotAvailable = {
    ...
    instruction: {
        address: '-1',
        instruction: localize(7529, null)
    },
};
    ...
    openSourceCode(instruction) {
        if (instruction) {
            ...
            this.editorService.openEditor({
                resource: sourceURI,
                description: localize(7531, null),
                options: {
                    preserveFocus: false,
                    selection: selection,
                    revealIfOpened: true,
...
class AccessibilityProvider {
    getWidgetAriaLabel() {
        return localize(7532, null);
    }
    getAriaLabel(element) {
        let label = '';
        const instruction = element.instruction;
        if (instruction.address !== '-1') {
            label += `${localize(7533, null)}: ${instruction.address}`;
        }
        if (instruction.instructionBytes) {
            label += `, ${localize(7534, null)}: ${instruction.instructionBytes}`;
        }
        label += `, ${localize(7535, null)}: ${instruction.instruction}`;
        return label;
    }
}
