            ...
            delta.added.forEach(added => {
                added.value.forEach(rawAdapter => {
                    if (!rawAdapter.type || (typeof rawAdapter.type !== 'string')) {
                        added.collector.error(nls.localize(7269, null));
                    }
                    if (rawAdapter.type !== '*') {
        ...
        const definitions = {
            'common': {
                properties: {
                    'name': {
                        type: 'string',
                        description: nls.localize(7270, null),
                        default: 'Launch'
                    },
                    'debugServer': {
                        type: 'number',
                    ...
                    'suppressMultipleSessionWarning': {
                        type: 'boolean',
                        description: nls.localize(7274, null),
                        default: true
                    }
                }
            }
    ...
    async guessDebugger(gettingConfigurations) {
        ...
        if (candidates.length === 0 && languageLabel) {
            ...
            const { confirmed } = await this.dialogService.confirm({
                ...
                message: nls.localize(7275, null, languageLabel),
                primaryButton: nls.localize(7276, null, languageLabel)
            });
            if (confirmed) {
        ...
        if (suggestedCandidates.length > 0) {
            picks.push({ type: 'separator', label: nls.localize(7277, null) }, ...suggestedCandidates.map(c => ({ label: c.label, pick: () => ({ debugger: c }) })));
        }
        ...
        if (dynamic.length) {
            ...
            for (const d of dynamic) {
                picks.push({
                    label: nls.localize(7278, null, d.label),
                    pick: async () => {
        ...
        picks.push({ type: 'separator', label: '' }, { label: languageLabel ? nls.localize(7279, null, languageLabel) : nls.localize(7280, null) });
        ...
        const placeHolder = nls.localize(7281, null);
