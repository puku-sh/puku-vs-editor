...
let WorkspacesFinderContribution = class WorkspacesFinderContribution extends Disposable {
    ...
    doHandleWorkspaceFiles(folder, workspaces) {
        ...
        if (workspaces.length === 1) {
            ...
            this.notificationService.prompt(Severity.Info, localize(14749, null, workspaceFile, 'https://go.microsoft.com/fwlink/?linkid=2025315'), [{



                    label: localize(14750, null),
        ...
        // Prompt to select a workspace from many
            ...
            this.notificationService.prompt(Severity.Info, localize(14751, null, 'https://go.microsoft.com/fwlink/?linkid=2025315'), [{



                    label: localize(14752, null),
                    run: () => {
                        this.quickInputService.pick(workspaces.map(workspace => ({ label: workspace })), { placeHolder: localize(14753, null) }).then(pick => {
                            if (pick) {
...
registerAction2(class extends Action2 {
    constructor() {
        super({
            ...
            title: localize2(14755, "Open Workspace"),
            f1: false,
            menu: {
    ...
    async run(accessor, uri) {
        ...
        if (contextService.getWorkbenchState() === 3 /* WorkbenchState.WORKSPACE */) {
            ...
            if (workspaceConfiguration && isEqual(workspaceConfiguration, uri)) {
                notificationService.info(localize(14754, null));
