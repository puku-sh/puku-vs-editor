...
const SHOW_ALL = nls.localize(12259, null);
export const configureTaskIcon = registerIcon('tasks-list-configure', Codicon.gear, nls.localize(12260, null));
const removeTaskIcon = registerIcon('tasks-remove', Codicon.close, nls.localize(12261, null));
...
let TaskQuickPick = TaskQuickPick_1 = class TaskQuickPick extends Disposable {
    ...
    _createTaskEntry(task, extraButtons = []) {
        const buttons = [
            { iconClass: ThemeIcon.asClassName(configureTaskIcon), tooltip: nls.localize(12262, null) },
            ...extraButtons
        ];
    ...
    _createTypeEntries(entries, types) {
        entries.push({ type: 'separator', label: nls.localize(12263, null) });
        types.forEach(type => {
            entries.push({ label: `$(folder) ${type}`, task: type, ariaLabel: nls.localize(12264, null, type) });
        });
    ...
    async getTopLevelEntries(defaultEntry) {
        ...
        if (recentTasks.length > 0) {
            const removeRecentButton = {
                ...
                tooltip: nls.localize(12265, null)
            };
            this._createEntriesForGroup(this._topLevelEntries, recentTasks, nls.localize(12266, null), [removeRecentButton]);
        }
        if (configuredTasks.length > 0) {
            if (dedupedConfiguredTasks.length > 0) {
                this._createEntriesForGroup(this._topLevelEntries, dedupedConfiguredTasks, nls.localize(12267, null));
            }
        }
        if (defaultEntry && (configuredTasks.length === 0)) {
            this._topLevelEntries.push({ type: 'separator', label: nls.localize(12268, null) });
    ...
    async handleSettingOption(selectedType) {
        const { confirmed } = await this._dialogService.confirm({
            ...
            message: nls.localize(12269, null, selectedType),
            cancelButton: nls.localize(12270, null)
        });
        if (confirmed) {
            await this._configurationService.updateValue(`${selectedType}.autoDetect`, 'on');
            await new Promise(resolve => setTimeout(() => resolve(), 100));
            return this.show(nls.localize(12271, null), undefined, selectedType);
        }
        return undefined;
    }
    ...
    static getSettingEntry(configurationService, type) {
        if (configurationService.getValue(`${type}.autoDetect`) === 'off') {
            return {
                label: nls.localize(12272, null, type[0].toUpperCase() + type.slice(1), type),
                task: null,
                settingType: type,
                alwaysShow: true
            };
    ...
    async _getEntriesForProvider(type) {
        ...
        if (tasks.length > 0) {
            ...
            taskQuickPickEntries = [{
                    label: nls.localize(12274, null, type),
                    task: null,
                    alwaysShow: true
                }];
        }
    ...
    async _toTask(task) {
        ...
        if (!resolvedTask) {
            this._notificationService.error(nls.localize(12275, null, task.type));
        }
        return resolvedTask;
    }
};
