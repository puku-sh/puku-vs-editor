...
(function (ConfigureTaskAction) {
    ...
    ConfigureTaskAction.TEXT = nls.localize2(12205, "Configure Task");
...
let AbstractTaskService = class AbstractTaskService extends Disposable {
    ...
    static { this.OutputChannelLabel = nls.localize(12092, null); }
    ...
    constructor(_configurationService, _markerService, _outputService, _paneCompositeService, _viewsService, _commandService, _editorService, _fileService, _contextService, _telemetryService, _textFileService, _modelService, _extensionService, _quickInputService, _configurationResolverService, _terminalService, _terminalGroupService, _storageService, _progressService, _openerService, _dialogService, _notificationService, _contextKeyService, _environmentService, _terminalProfileResolverService, _pathService, _textModelResolverService, _preferencesService, _viewDescriptorService, _workspaceTrustRequestService, _workspaceTrustManagementService, _logService, _themeService, _lifecycleService, remoteAgentService, _instantiationService, _chatService, _chatAgentService, _hostService) {
        ...
        this._configurationResolverService.contributeVariable('defaultBuildTask', async () => {
            ...
            if (tasks && tasks.length > 0) {
                entry = await this._showQuickPick(tasks, nls.localize(12093, null));
            }
        ...
        this._register(this.onDidStateChange(async (e) => {
            this._log(nls.localize(12094, null, e.kind), true);
            switch (e.kind) {
    ...
    _attemptTaskReconnection() {
        if (this._lifecycleService.startupKind !== 3 /* StartupKind.ReloadedWindow */) {
            this._log(nls.localize(12095, null), true);
            this._tasksReconnected = true;
            this._storageService.remove(AbstractTaskService_1.PersistentTasks_Key, 1 /* StorageScope.WORKSPACE */);
        }
        if (!this._configurationService.getValue("task.reconnection" /* TaskSettingId.Reconnection */) || this._tasksReconnected) {
            this._log(nls.localize(12096, null, this._configurationService.getValue("task.reconnection" /* TaskSettingId.Reconnection */), this._tasksReconnected), true);
            this._tasksReconnected = true;
            return;
        }
        this._log(nls.localize(12097, null), true);
        this.getWorkspaceTasks(4 /* TaskRunSource.Reconnect */).then(async () => {
            this._tasksReconnected = await this._reconnectTasks();
            this._log(nls.localize(12098, null), true);
            this._onDidReconnectToTasks.fire();
        });
    }
    async _handleLongRunningTaskCompletion(event, durationMs) {
        ...
        const message = taskLabel
            ? nls.localize(12099, null, taskLabel, durationText)
            : nls.localize(12100, null, durationText);
    ...
    _formatTaskDuration(durationMs) {
        ...
        if (minutes > 0) {
            return seconds > 0
                ? nls.localize(12101, null, minutes, seconds)
                : nls.localize(12102, null, minutes);
        }
        return nls.localize(12103, null, seconds);
    }
    async _reconnectTasks() {
        const tasks = await this.getSavedTasks('persistent');
        if (!tasks.length) {
            this._log(nls.localize(12104, null), true);
            return true;
        }
        const taskLabels = tasks.map(task => task._label).join(', ');
        this._log(nls.localize(12105, null, taskLabels), true);
        ...
        CommandsRegistry.registerCommand({
            ...
            metadata: {
                ...
                args: [{
                        name: 'args',
                        isOptional: true,
                        description: nls.localize(12106, null),
                        schema: {
                            anyOf: [
                                {
                                    type: 'string',
                                    description: nls.localize(12107, null)
                                },
                                {
                                    type: 'object',
                                    properties: {
                                        type: {
                                            type: 'string',
                                            description: nls.localize(12108, null)
                                        },
                                        task: {
                                            type: 'string',
                                            description: nls.localize(12109, null)
                                        }
                                    }
                                }
                            ]
    ...
    _showOutput(runSource = 1 /* TaskRunSource.User */, userRequested, errorMessage) {
        if (!VirtualWorkspaceContext.getValue(this._contextKeyService) && ((runSource === 1 /* TaskRunSource.User */) || (runSource === 3 /* TaskRunSource.ConfigurationChange */))) {
            if (userRequested) {
                ...
                actions.push({
                    label: nls.localize(12111, null),
                    run: () => {
                        this._outputService.showChannel(this._outputChannel.id, true);
                    }
                });
                if (chatEnabled && actions.length > 1) {
                    this._notificationService.prompt(Severity.Warning, nls.localize(12112, null), actions);
                }
                else {
                    this._notificationService.prompt(Severity.Warning, nls.localize(12113, null), actions);
                }
            }
        }
    }
    ...
    async getTask(folder, identifier, compareId = false, type = undefined) {
        ...
        if (this.ignoredWorkspaceFolders.some(ignored => ignored.name === name)) {
            return Promise.reject(new Error(nls.localize(12114, null, name)));
        }
    ...
    async tryResolveTask(configuringTask) {
        ...
        if (!matchingProvider) {
            if (matchingProviderUnavailable) {
                this._log(nls.localize(12115, null, configuringTask.configures.type));
            }
            return;
        }
    ...
    _getPersistentTasks() {
        if (this._persistentTasks) {
            this._log(nls.localize(12116, null, this._persistentTasks.size), true);
    ...
    async getSavedTasks(type) {
        ...
        const tasks = [];
        this._log(nls.localize(12117, null), true);
        ...
        for (const entry of storedTasks.entries()) {
            try {
                ...
                this._log(nls.localize(12118, null, key, task, folderInfo.folder), true);
                addTaskToMap(folderInfo.isWorkspaceFile ? workspaceToTaskMap : folderToTasksMap, folderInfo.folder, task);
            }
            catch (error) {
                this._log(nls.localize(12119, null, error), true);
            }
        }
        ...
        for (const key of storedTasks.keys()) {
            if (readTasksMap.has(key)) {
                ...
                this._log(nls.localize(12120, null, key), true);
            }
            else {
                this._log(nls.localize(12121, null, key), true);
            }
        }
        return tasks;
    }
    ...
    removePersistentTask(key) {
        this._log(nls.localize(12122, null, key), true);
    ...
    async _setPersistentTask(task) {
        ...
        if (!InMemoryTask.is(task) && key) {
            ...
            this._log(nls.localize(12123, null, key), true);
    ...
    _savePersistentTasks() {
        ...
        this._log(nls.localize(12124, null, keys.join(', ')), true);
        ...
        if (!runnable || !runnable.task) {
            if (test) {
                if (this.schemaVersion === 1 /* JsonSchemaVersion.V0_1_0 */) {
                    throw new TaskError(Severity.Info, nls.localize(12125, null), 3 /* TaskErrors.NoTestTask */);
                }
                else {
                    throw new TaskError(Severity.Info, nls.localize(12126, null), 3 /* TaskErrors.NoTestTask */);
                }
            }
            else {
                if (this.schemaVersion === 1 /* JsonSchemaVersion.V0_1_0 */) {
                    throw new TaskError(Severity.Info, nls.localize(12127, null), 2 /* TaskErrors.NoBuildTask */);
                }
                else {
                    throw new TaskError(Severity.Info, nls.localize(12128, null), 2 /* TaskErrors.NoBuildTask */);
                }
            }
        }
    ...
    async run(task, options, runSource = 0 /* TaskRunSource.System */) {
        ...
        if (!task) {
            throw new TaskError(Severity.Info, nls.localize(12129, null), 5 /* TaskErrors.TaskNotFound */);
        }
    ...
    async _attachProblemMatcher(task) {
        ...
        entries.unshift({ type: 'separator', label: nls.localize(12130, null) });
        let taskType;
        if (CustomTask.is(task)) {
            const configProperties = task._source.config.element;
            // eslint-disable-next-line local/code-no-any-casts
        ...
        entries.unshift({ label: nls.localize(12131, null), matcher: undefined }, { label: nls.localize(12132, null), matcher: undefined, never: true }, { label: nls.localize(12133, null, taskType), matcher: undefined, setting: taskType }, { label: nls.localize(12134, null), matcher: undefined, learnMore: true });
        const problemMatcher = await this._quickInputService.pick(entries, { placeHolder: nls.localize(12135, null) });
        if (!problemMatcher) {
            return task;
        }
    ...
    async customize(task, properties, openConfig) {
        ...
        if (configuration.hasParseErrors) {
            this._notificationService.warn(nls.localize(12136, null));
            return Promise.resolve(undefined);
        }
    ...
    _createRunnableTask(tasks, group) {
        ...
        if (workspaceTasks.length > 0) {
            if (workspaceTasks.length > 1) {
                this._log(nls.localize(12138, null));
            }
    ...
    async _saveBeforeRun() {
        ...
        if (saveBeforeRunTaskConfig === SaveBeforeRunConfigOptions.Never) {
            ...
            const { confirmed } = await this._dialogService.confirm({
                message: nls.localize(12139, null),
                detail: nls.localize(12140, null),
                primaryButton: nls.localize(12141, null),
                cancelButton: nls.localize(12142, null),
            });
            if (!confirmed) {
                return false;
            }
    ...
    async _handleExecuteResult(executeResult, runSource) {
        ...
        if (executeResult.kind === 2 /* TaskExecuteKind.Active */) {
            ...
            if (active && active.same) {
                ...
                throw new TaskError(Severity.Warning, nls.localize(12143, null), 1 /* TaskErrors.RunningTask */);
            }
        }
    ...
    _handleInstancePolicy(task, policy) {
        ...
        switch (policy) {
            ...
            case "warn" /* InstancePolicy.warn */:
                this._notificationService.warn(nls.localize(12144, null));
                break;
            case "prompt" /* InstancePolicy.prompt */:
            default:
                this._showQuickPick(this._taskSystem.getActiveTasks().filter(t => task._id === t._id), nls.localize(12145, null), {
                    label: nls.localize(12146, null),
                    task: undefined
    ...
    async _restart(task) {
        ...
        if (isTaskRunning) {
            ...
            if (!response.success) {
                this._notificationService.warn(nls.localize(12147, null, Types.isString(task) ? task : task.configurationProperties.name));
                return;
            }
        }
        ...
        try {
            ...
            if (updatedTask) {
                ...
                this._notificationService.warn(nls.localize(12148, null, task.configurationProperties.name));
            }
        }
        catch {
                        ...
                        raceTimeout(provider.provideTasks(validTypes).then((taskSet) => {
                            ...
                            for (const task of taskSet.tasks) {
                                if (task.type !== this._providerTypes.get(handle)) {
                                    this._log(nls.localize(12149, null, this._providerTypes.get(handle), task.type));
                ...
                if (configurations || legacyTaskConfigurations) {
                    ...
                    const unUsedConfigurationPromises = unUsedConfigurationsAsArray.map(async (value) => {
                        ...
                        if (requiredTaskProviderUnavailable) {
                            this._log(nls.localize(12150, null, configuringTask.configures.type));
                        }
                        else if (!waitToActivate) {
                            this._log(nls.localize(12151, null, configuringTask.configures.type, JSON.stringify(configuringTask._source.config.element, undefined, 4)));
                        }
                    });
    ...
    async _computeWorkspaceFolderTasks(workspaceFolder, runSource = 1 /* TaskRunSource.User */) {
        ...
        if (problemReporter.status.isFatal()) {
            problemReporter.fatal(nls.localize(12152, null));
    ...
    _testParseExternalConfig(config, location) {
        ...
        if (parseErrors) {
            ...
            if (isAffected) {
                this._log(nls.localize(12153, null, location));
                this._showOutput(undefined, undefined, nls.localize(12154, null, location));
    ...
    async _computeWorkspaceFileTasks(workspaceFolder, runSource = 1 /* TaskRunSource.User */) {
        ...
        const configuration = this._testParseExternalConfig(workspaceFileConfig.config, nls.localize(12155, null));
        ...
        if (engine === ExecutionEngine.Process) {
            this._notificationService.warn(nls.localize(12156, null));
            return this._emptyWorkspaceTaskResults(workspaceFolder);
        }
    ...
    async _computeUserTasks(workspaceFolder, runSource = 1 /* TaskRunSource.User */) {
        ...
        const configuration = this._testParseExternalConfig(userTasksConfig.config, nls.localize(12157, null));
        const customizedTasks = {
            byIdentifier: Object.create(null)
        };
        const custom = [];
        ...
        const engine = configuration.config ? TaskConfig.ExecutionEngine.from(configuration.config) : ExecutionEngine.Terminal;
        if (engine === ExecutionEngine.Process) {
            this._notificationService.warn(nls.localize(12158, null));
            return this._emptyWorkspaceTaskResults(workspaceFolder);
        }
        return { workspaceFolder, set: { tasks: custom }, configurations: customizedTasks, hasErrors: configuration.hasParseErrors };
    }
    ...
    async _computeTasksForSingleConfig(workspaceFolder, config, runSource, custom, customized, source, isRecentTask = false) {
        ...
        if (problemReporter.status.isFatal()) {
            problemReporter.fatal(nls.localize(12159, null));
            return hasErrors;
        }
        if (parseResult.configured && parseResult.configured.length > 0) {
            for (const task of parseResult.configured) {
        ...
        if (this._contextService.getWorkbenchState() === 2 /* WorkbenchState.FOLDER */) {
            ...
            for (const workspaceFolder of this._contextService.getWorkspace().folders) {
                if (schemaVersion === this._computeJsonSchemaVersion(workspaceFolder)) {
                    ...
                    this._log(nls.localize(12160, null, workspaceFolder.uri.fsPath));
                }
            }
        }
    ...
    _getConfiguration(workspaceFolder, source) {
        ...
        if (parseErrors) {
            ...
            if (isAffected) {
                this._log(nls.localize(12161, null));
                this._showOutput(undefined, undefined, nls.localize(12162, null));
        ...
        if (err instanceof TaskError) {
            ...
            if (needsConfig || needsTerminate) {
                this._notificationService.prompt(buildError.severity, buildError.message, [{
                        label: needsConfig ? ConfigureTaskAction.TEXT.value : nls.localize(12163, null),
                        run: () => {
                            if (needsConfig) {
                                this._runConfigureTasks();
                            }
            ...
            this._notificationService.error(nls.localize(12164, null));
        }
        if (showOutput) {
    ...
    async _createTaskQuickPickEntries(tasks, group = false, sort = false, selectedEntry, includeRecents = true) {
        ...
        function fillEntries(entries, tasks, groupLabel) {
            ...
            for (const task of tasks) {
                ...
                entry.buttons = [{ iconClass: ThemeIcon.asClassName(configureTaskIcon), tooltip: nls.localize(12165, null) }];
        ...
        if (group) {
            ...
            if (tasks.length === 1) {
                ...
                if (includeRecents) {
                    fillEntries(entries, recent, nls.localize(12166, null));
                }
                configured = configured.sort((a, b) => sorter.compare(a, b));
                fillEntries(entries, configured, nls.localize(12167, null));
                detected = detected.sort((a, b) => sorter.compare(a, b));
                fillEntries(entries, detected, nls.localize(12168, null));
            }
        }
        else {
            if (sort) {
    ...
    _showIgnoredFoldersMessage() {
        ...
        this._notificationService.prompt(Severity.Info, nls.localize(12169, null, this.ignoredWorkspaceFolders.map(f => f.name).join(', ')), [{
                label: nls.localize(12170, null),
                isSecondary: true,
                run: () => {
    ...
    async _trust() {
        ...
        if (!this._workspaceTrustManagementService.isWorkspaceTrusted()) {
            return (await this._workspaceTrustRequestService.requestWorkspaceTrust({
                message: nls.localize(12171, null)
            })) === true;
        }
        return true;
    }
    ...
    _doRunTaskCommand(tasks, type, name) {
        ...
        const placeholder = nls.localize(12172, null);
        this._showIgnoredFoldersMessage().then(() => {
            if (this._configurationService.getValue(USE_SLOW_PICKER)) {
                ...
                this._showQuickPick(tasks ? tasks : taskResult.tasks, placeholder, {
                    label: '$(plus) ' + nls.localize(12173, null),
                    task: null
                ...
                this._showTwoLevelQuickPick(placeholder, {
                    label: '$(plus) ' + nls.localize(12174, null),
                    task: null
                }, type, name).
                    then(pickThen);
            }
    ...
    _runBuildCommand() {
        ...
        return this._runTaskGroupCommand(TaskGroup.Build, {
            fetching: nls.localize(12175, null),
            select: nls.localize(12176, null),
            notFoundConfigure: nls.localize(12177, null)
        }, this._runConfigureDefaultBuildTask, this._build);
    }
    _runTestCommand() {
        return this._runTaskGroupCommand(TaskGroup.Test, {
            fetching: nls.localize(12178, null),
            select: nls.localize(12179, null),
            notFoundConfigure: nls.localize(12180, null)
    ...
    _runTerminateCommand(arg) {
        ...
        const runQuickPick = (promise) => {
            this._showQuickPick(promise || this.getActiveTasks(), nls.localize(12181, null), {
                label: nls.localize(12182, null),
                task: undefined
            }, false, true, undefined, [{
                    label: nls.localize(12183, null),
                    id: 'terminateAll',
                    task: undefined
                }]).then(entry => {
                ...
                if (active) {
                    this._terminateAll().then((responses) => {
                        ...
                        if (response.code && response.code === 3 /* TerminateResponseCode.ProcessNotFound */) {
                            this._notificationService.error(nls.localize(12184, null));
                        }
                        else {
                            this._notificationService.error(nls.localize(12185, null));
                        }
                    });
                }
            });
    ...
    async _runRestartTaskCommand(arg) {
        ...
        if (this.inTerminal()) {
            ...
            const entry = await this._showQuickPick(activeTasks, nls.localize(12186, null), {
                label: nls.localize(12187, null),
                task: null
            }, false, true);
            if (entry && entry.task) {
    ...
    async _runRerunAllRunningTasksCommand() {
        const activeTasks = await this.getActiveTasks();
        if (activeTasks.length === 0) {
            this._notificationService.info(nls.localize(12188, null));
            return;
        }
    ...
    _openTaskFile(resource, taskSource) {
        ...
        this._fileService.stat(resource).then((stat) => stat, () => undefined).then(async (stat) => {
            ...
            let content;
            if (!tasksExistInFile) {
                const pickTemplateResult = await this._quickInputService.pick(getTaskTemplates(), { placeHolder: nls.localize(12189, null) });
    ...
    getTaskDescription(task) {
        ...
        if (task._source.kind === TaskSourceKind.User) {
            description = nls.localize(12190, null);
        }
    ...
    async _runConfigureTasks() {
        ...
        const createLabel = nls.localize(12191, null);
        const openLabel = nls.localize(12192, null);
        ...
        this._quickInputService.pick(entriesWithSettings, { placeHolder: nls.localize(12193, null) }, cancellationToken).
                ...
                this._showIgnoredFoldersMessage().then(async () => {
                    ...
                    for (const task of tasks) {
                        if (task === defaultBuildTask) {
                            const label = nls.localize(12194, null, TaskQuickPick.getTaskLabelWithIcon(task, task.getQualifiedLabel()));
                            selectedTask = task;
                    ...
                    const tokenSource = new CancellationTokenSource();
                    const cancellationToken = tokenSource.token;
                    this._quickInputService.pick(entries, { placeHolder: nls.localize(12195, null) }, cancellationToken).
                        then(async (entry) => {
                        if (cancellationToken.isCancellationRequested) {
                            // canceled when there's only one task
                            const task = (await entries)[0];
                    ...
                    this._quickInputService.pick(entries, {
                        placeHolder: nls.localize(12196, null)
                    }).
                        then((entry) => {
                        const task = entry && 'task' in entry ? entry.task : undefined;
                        if ((task === undefined) || (task === null)) {
    ...
    _runConfigureDefaultTestTask() {
        if (this.schemaVersion === 2 /* JsonSchemaVersion.V2_0_0 */) {
            this.tasks().then((tasks => {
                ...
                if (selectedTask) {
                    selectedEntry = {
                        label: nls.localize(12197, null, selectedTask.getQualifiedLabel()),
                        task: selectedTask,
                        detail: this._showDetail() ? selectedTask.configurationProperties.detail : undefined
                    };
                }
                this._showIgnoredFoldersMessage().then(() => {
                    this._showQuickPick(tasks, nls.localize(12198, null), undefined, true, false, selectedEntry).then((entry) => {
                        const task = entry ? entry.task : undefined;
                        if (!task) {
                            return;
                        }
    ...
    async runShowTasks() {
        ...
        if (activeTasks.length === 1) {
            ...
            this._showQuickPick(activeTasksPromise, nls.localize(12199, null), {
                label: nls.localize(12200, null),
                task: null
    ...
    async _upgrade() {
        ...
        this._notificationService.prompt(Severity.Warning, fileDiffs.length === 1 ?
            nls.localize(12201, null)
            : nls.localize(12202, null), [{
                label: fileDiffs.length === 1 ? nls.localize(12203, null) : nls.localize(12204, null),
                run: async () => {
