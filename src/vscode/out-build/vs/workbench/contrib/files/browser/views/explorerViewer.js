...
let ExplorerFindProvider = class ExplorerFindProvider {
    ...
    async doFilterFind(pattern, matchType, token) {
        ...
        return {
            ...
            warningMessage: hitMaxResults ? localize(8960, null) : undefined
        };
    }
    ...
    async doHighlightFind(pattern, matchType, token) {
        ...
        return {
            ...
            matchCount: searchResults.reduce((acc, { files, directories }) => acc + files.length + directories.length, 0),
            warningMessage: hitMaxResults ? localize(8961, null) : undefined
        };
    }
...
let FilesRenderer = class FilesRenderer {
    ...
    renderStat(stat, label, domId, filterData, templateData) {
        ...
        if (highlightResults > 0) {
            ...
            badge.setTitleFormat(localize(8963, null, highlightResults));
...
let FileDragAndDrop = class FileDragAndDrop {
    ...
    async handleExplorerDrop(data, target, targetIndex, targetSector, originalEvent) {
        ...
        if (confirmDragAndDrop) {
            const message = items.length > 1 && items.every(s => s.isRoot) ? localize(8965, null)
                : items.length > 1 ? localize(8966, null, items.length, target.name)
                    : items[0].isRoot ? localize(8967, null, items[0].name)
                        : localize(8968, null, items[0].name, target.name);
            const detail = items.length > 1 && !items.every(s => s.isRoot) ? getFileNamesMessage(items.map(i => i.resource)) : undefined;
            const confirmation = await this.dialogService.confirm({
                message,
                detail,
                checkbox: {
                    label: localize(8969, null)
                },
                primaryButton: localize(8970, null)
            });
    ...
    async doHandleExplorerDropOnCopy(sources, target) {
        ...
        await this.explorerService.applyBulkEdit(resourceFileEdits, {
            ...
            undoLabel: localize(8971, null, labelSuffix),
            progressLabel: localize(8972, null, labelSuffix),
        });
    ...
    async doHandleExplorerDropOnMove(sources, target) {
        ...
        const options = {
            ...
            undoLabel: localize(8973, null, labelSuffix),
            progressLabel: localize(8974, null, labelSuffix)
        };
        try {
...
function getFileOrFolderLabelSuffix(items) {
    ...
    if (items.every(i => i.isDirectory)) {
        return localize(8975, null, items.length);
    }
    if (items.every(i => !i.isDirectory)) {
        return localize(8976, null, items.length);
    }
