            ...
            return uploadLimiter.queue(async () => {
                ...
                if (target && entry.name && target.getChild(entry.name)) {
                    ...
                    await this.explorerService.applyBulkEdit([new ResourceFileEdit(joinPath(target.resource, entry.name), undefined, { recursive: true, folder: target.getChild(entry.name)?.isDirectory })], {
                        undoLabel: localize(8803, null, entry.name),
                        progressLabel: localize(8804, null, entry.name),
                    });
                    if (token.isCancellationRequested) {
                        return;
                    }
        ...
        const reportProgress = (fileSize, bytesUploaded) => {
            ...
            if (fileSize < ByteSize.MB) {
                if (operation.filesTotal === 1) {
                    ...
                    message = localize(8805, null, operation.filesUploaded, operation.filesTotal, ByteSize.formatSize(bytesUploadedPerSecond));
                }
            }
            // Large file
            else {
                message = localize(8806, null, entry.name, ByteSize.formatSize(fileBytesUploaded), ByteSize.formatSize(fileSize), ByteSize.formatSize(bytesUploadedPerSecond));
            }
        ...
        if (folders.length > 0 && target.isRoot) {
            ...
            if (folders.some(folder => workspaceFolderSchemas.indexOf(folder.uri.scheme) >= 0)) {
                ...
                message = folders.length > 1 ?
                    localize(8812, null) :
                    localize(8813, null, basename(folders[0].uri));
            }
            else {
                message = folders.length > 1 ?
                    localize(8814, null) :
                    localize(8815, null, basename(folders[0].uri));
            }
    ...
    async importResources(target, resources, token) {
        if (resources && resources.length > 0) {
            ...
            if (inaccessibleFileCount > 0) {
                this.notificationService.error(inaccessibleFileCount > 1 ? localize(8816, null) : localize(8817, null));
            }
            ...
            await this.explorerService.applyBulkEdit(resourceFileEdits, {
                undoLabel: resourcesFiltered.length === 1 ?
                    localize(8818, null, basename(resourcesFiltered[0])) :
                    localize(8819, null, resourcesFiltered.length),
                progressLabel: resourcesFiltered.length === 1 ?
                    localize(8820, null, basename(resourcesFiltered[0])) :
                    localize(8821, null, resourcesFiltered.length),
...
let FileDownload = class FileDownload {
    ...
    reportProgress(name, fileSize, bytesDownloaded, operation) {
        ...
        if (fileSize < ByteSize.MB) {
            if (operation.filesTotal === 1) {
                ...
                message = localize(8823, null, operation.filesDownloaded, operation.filesTotal, ByteSize.formatSize(bytesDownloadedPerSecond));
            }
        }
        // Large file
        else {
            message = localize(8824, null, name, ByteSize.formatSize(operation.fileBytesDownloaded), ByteSize.formatSize(fileSize), ByteSize.formatSize(bytesDownloadedPerSecond));
        }
        // Report progress but limit to update only once per second
        operation.progressScheduler.work({ message });
    }
    async doDownloadNative(explorerItem, progress, cts) {
        ...
        const destination = await this.fileDialogService.showSaveDialog({
            ...
            saveLabel: localize(8825, null),
            title: localize(8826, null),
            defaultUri
        });
        if (destination) {
            ...
            await this.explorerService.applyBulkEdit([new ResourceFileEdit(explorerItem.resource, destination, { overwrite: true, copy: true })], {
                undoLabel: localize(8827, null, explorerItem.name),
                progressLabel: localize(8828, null, explorerItem.name),
                progressLocation: 10 /* ProgressLocation.Window */
            });
        }
        else {
...
export function getFileOverwriteConfirm(name) {
    return {
        message: localize(8829, null, name),
        detail: localize(8830, null),
        primaryButton: localize(8831, null),
        type: 'warning'
    };
}
export function getMultipleFilesOverwriteConfirm(files) {
    if (files.length > 1) {
        return {
            message: localize(8832, null, files.length),
            detail: getFileNamesMessage(files) + '\n' + localize(8833, null),
            primaryButton: localize(8834, null),
            type: 'warning'
        };
    }
