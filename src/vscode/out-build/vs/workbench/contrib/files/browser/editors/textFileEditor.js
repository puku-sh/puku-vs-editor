    ...
    async handleSetInputError(error, input, options) {
        ...
        if (error.fileOperationResult === 0 /* FileOperationResult.FILE_IS_DIRECTORY */) {
            ...
            actions.push(toAction({
                id: 'workbench.files.action.openFolder', label: localize(8613, null), run: async () => {
            ...
            if (this.contextService.isInsideWorkspace(input.preferredResource)) {
                actions.push(toAction({
                    id: 'workbench.files.action.reveal', label: localize(8614, null), run: async () => {
            ...
            throw createEditorOpenError(localize(8615, null), actions, { forceMessage: true });
        }
        // Handle case where a file is too large to open without confirmation
        if (error.fileOperationResult === 7 /* FileOperationResult.FILE_TOO_LARGE */) {
            let message;
            if (error instanceof TooLargeFileOperationError) {
                message = localize(8616, null, ByteSize.formatSize(error.size));
            }
            else {
                message = localize(8617, null);
            }
        ...
        if (error.fileOperationResult === 1 /* FileOperationResult.FILE_NOT_FOUND */ &&
            ...
            const fileNotFoundError = createEditorOpenError(new FileOperationError(localize(8618, null), 1 /* FileOperationResult.FILE_NOT_FOUND */), [
                toAction({
                    id: 'workbench.files.action.createMissingFile', label: localize(8619, null), run: async () => {
