...
KeybindingsRegistry.registerCommandAndKeybindingRule({
    ...
    handler: async (accessor, resource) => {
        ...
        if (uri && fileService.hasProvider(uri)) {
            ...
            const editorLabel = nls.localize(8789, null, name, name);
            try {
...
async function doSaveEditors(accessor, editors, options) {
    ...
    try {
        ...
        if (!isCancellationError(error)) {
            const actions = [toAction({ id: 'workbench.action.files.saveEditors', label: nls.localize(8790, null), run: () => instantiationService.invokeFunction(accessor => doSaveEditors(accessor, editors, options)) })];
            ...
            if (editorsToRevert.length > 0) {
                actions.push(toAction({ id: 'workbench.action.files.revertEditors', label: editorsToRevert.length > 1 ? nls.localize(8791, null) : nls.localize(8792, null), run: () => editorService.revert(editorsToRevert) }));
            }
            notificationService.notify({
                id: editors.map(({ editor }) => hash(editor.resource?.toString())).join(), // ensure unique notification ID per set of editor
                severity: Severity.Error,
                message: nls.localize(8793, null, editors.map(({ editor }) => editor.getName()).join(', '), toErrorMessage(error, false)),
...
CommandsRegistry.registerCommand({
    ...
    handler: async (accessor) => {
        ...
        try {
            ...
            const notificationService = accessor.get(INotificationService);
            notificationService.error(nls.localize(8794, null, editors.map(({ editor }) => editor.getName()).join(', '), toErrorMessage(error, false)));
        }
    }
});
CommandsRegistry.registerCommand({
...
CommandsRegistry.registerCommand({
    ...
    handler: async (accessor, args) => {
        ...
        const fileService = accessor.get(IFileService);
        const createFileLocalized = nls.localize(8795, null);
