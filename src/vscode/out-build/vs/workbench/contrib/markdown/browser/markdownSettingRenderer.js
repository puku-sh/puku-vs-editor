...
let SimpleSettingRenderer = class SimpleSettingRenderer {
    ...
    viewInSettingsMessage(settingId, alreadyDisplayed) {
        if (alreadyDisplayed) {
            return nls.localize(9389, null);
        }
        else {
            const displayName = settingKeyToDisplayFormat(settingId);
            return nls.localize(9390, null, displayName.category, displayName.label);
        }
    }
    restorePreviousSettingMessage(settingId) {
        const displayName = settingKeyToDisplayFormat(settingId);
        return nls.localize(9391, null, displayName.category, displayName.label);
    }
    ...
    booleanSettingMessage(setting, booleanValue) {
        ...
        if (this.isAlreadySet(setting, booleanValue)) {
            if (booleanValue) {
                return nls.localize(9392, null, displayName.category, displayName.label);
            }
            else {
                return nls.localize(9393, null, displayName.category, displayName.label);
            }
        }
        if (booleanValue) {
            return nls.localize(9394, null, displayName.category, displayName.label);
        }
        else {
            return nls.localize(9395, null, displayName.category, displayName.label);
        }
    }
    stringSettingMessage(setting, stringValue) {
        const displayName = settingKeyToDisplayFormat(setting.key);
        if (this.isAlreadySet(setting, stringValue)) {
            return nls.localize(9396, null, displayName.category, displayName.label, stringValue);
        }
        return nls.localize(9397, null, displayName.category, displayName.label, stringValue);
    }
    numberSettingMessage(setting, numberValue) {
        const displayName = settingKeyToDisplayFormat(setting.key);
        if (this.isAlreadySet(setting, numberValue)) {
            return nls.localize(9398, null, displayName.category, displayName.label, numberValue);
        }
        return nls.localize(9399, null, displayName.category, displayName.label, numberValue);
    }
    renderSetting(setting, newValue) {
        const href = this.settingToUriString(setting.key, newValue);
        const title = nls.localize(9400, null);
    ...
    getActions(uri) {
        ...
        actions.push({
            ...
            enabled: true,
            id: 'copySettingId',
            tooltip: nls.localize(9401, null),
            label: nls.localize(9402, null),
            run: () => {
