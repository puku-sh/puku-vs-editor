...
let TestingExplorerView = class TestingExplorerView extends ViewPane {
        ...
        for (const { profiles, controller } of this.testProfileService.all()) {
            ...
            for (const profile of profiles) {
                ...
                profileActions.push(toAction({
                    ...
                    label: defaults.includes(profile) ? localize(13670, null, profile.label) : profile.label,
                    run: () => {
    ...
    getDropdownAction() {
        return new Action('selectRunConfig', localize(13673, null), 'codicon-chevron-down', true);
    }
...
let ResultSummaryView = class ResultSummaryView extends Disposable {
    ...
    render() {
        ...
        if (!results.length) {
            ...
            this.container.innerText = localize(13674, null);
    ...
    renderActivityBadge(countSummary) {
        if (countSummary && this.badgeType !== "off" /* TestingCountBadge.Off */ && countSummary[this.badgeType] !== 0) {
            ...
            this.lastBadge = new IconBadge(icons.testingContinuousIsOn, () => localize(13675, null));
        }
        else {
            if (!this.lastBadge) {
                return;
    ...
    getLocalizedBadgeString(countBadgeType, count) {
        switch (countBadgeType) {
            case "passed" /* TestingCountBadge.Passed */:
                return localize(13676, null, count);
            case "skipped" /* TestingCountBadge.Skipped */:
                return localize(13677, null, count);
            default:
                return localize(13678, null, count);
        }
    }
};
...
let NoTestsForDocumentWidget = class NoTestsForDocumentWidget extends Disposable {
    constructor(container, filterState) {
        ...
        emptyParagraph.innerText = localize(13679, null);
        const buttonLabel = localize(13680, null);
...
const getLabelForTestTreeElement = (element) => {
    ...
    if (element instanceof TestItemTreeElement) {
        if (element.duration !== undefined) {
            label = localize(13681, null, label, formatDuration(element.duration));



        }
        if (element.retired) {
            label = localize(13682, null, label);
