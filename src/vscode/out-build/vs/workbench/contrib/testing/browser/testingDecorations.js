        ...
        get hoverMessage() {
            if (!hoverMessage) {
                ...
                if (testIdWithMessages) {
                    ...
                    building.appendMarkdown(` [${localize(13640, null)}](command:vscode.peekTestError?${args})`);
                }
            }
            return hoverMessage;
        },
...
let RunTestDecoration = class RunTestDecoration {
    ...
    getGutterLabel() {
        switch (getTestingConfiguration(this.configurationService, "testing.defaultGutterClickAction" /* TestingConfigKeys.DefaultGutterClickAction */)) {
            case "contextMenu" /* DefaultGutterClickAction.ContextMenu */:
                return localize(13643, null);
            case "debug" /* DefaultGutterClickAction.Debug */:
                return localize(13644, null);
            case "runWithCoverage" /* DefaultGutterClickAction.Coverage */:
                return localize(13645, null);
            case "run" /* DefaultGutterClickAction.Run */:
            default:
                return localize(13646, null);
        }
    }
    /**
    ...
    getTestContextMenuActions(test, resultItem) {
        ...
        [
            { bitset: 2 /* TestRunProfileBitset.Run */, label: localize(13647, null) },
            { bitset: 4 /* TestRunProfileBitset.Debug */, label: localize(13648, null) },
            { bitset: 8 /* TestRunProfileBitset.Coverage */, label: localize(13649, null) },
        ...
        if (capabilities & 16 /* TestRunProfileBitset.HasNonDefaultProfile */) {
            testActions.push(new Action('testing.runUsing', localize(13650, null), undefined, undefined, async () => {
        ...
        if (resultItem && isFailedState(resultItem.computedState)) {
            testActions.push(new Action('testing.gutter.peekFailure', localize(13651, null), undefined, undefined, () => this.commandService.executeCommand('vscode.peekTestError', test.item.extId)));
        }
        if (resultItem?.computedState === 2 /* TestResultState.Running */) {
            testActions.push(new Action('testing.gutter.cancel', localize(13652, null), undefined, undefined, () => this.commandService.executeCommand("testing.cancelRun" /* TestCommandId.CancelTestRunAction */)));
        }
        testActions.push(new Action('testing.gutter.reveal', localize(13653, null), undefined, undefined, () => this.commandService.executeCommand('_revealTestInExplorer', test.item.extId)));
...
let MultiRunTestDecoration = class MultiRunTestDecoration extends RunTestDecoration {
    ...
    getContextMenuActions() {
        ...
        const allActions = [];
        [
            { bitset: 2 /* TestRunProfileBitset.Run */, label: localize(13654, null) },
            { bitset: 8 /* TestRunProfileBitset.Coverage */, label: localize(13655, null) },
            { bitset: 4 /* TestRunProfileBitset.Debug */, label: localize(13656, null) },
        ...
        if (overflow > 0) {
            ...
            testSubmenus.push(new Action('testing.gutter.overflow', localize(13657, null, overflow), undefined, undefined, () => this.pickAndRun(testItems)));
        }
    ...
    async pickAndRun(testItems) {
        ...
        const item = await doPick(testItems.map(({ currentLabel, testItem }) => ({ label: currentLabel, test: testItem.test, result: testItem.resultItem })), localize(13658, null));
        if (!item) {
            return;
        }
