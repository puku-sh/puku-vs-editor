...
let OutlinePane = class OutlinePane extends ViewPane {
    ...
    async _handleEditorControlChanged(pane) {
        ...
        if (!pane || !this._outlineService.canCreateOutline(pane) || !resource) {
            return this._showMessage(localize(10674, null));
        }
        let loadingMessage;
        if (!didCapture) {
            loadingMessage = new TimeoutTimer(() => {
                this._showMessage(localize(10675, null, basename(resource)));
            }, 100);
        }
        ...
        const updateTree = () => {
            if (newOutline.isEmpty) {
                // no more elements
                this._showMessage(localize(10676, null, basename(resource)));
