...
let TroubleshootIssueService = class TroubleshootIssueService extends Disposable {
    ...
    async start() {
        ...
        const res = await this.dialogService.confirm({
            message: localize(9244, null),
            detail: localize(9245, null, this.productService.nameLong),
            primaryButton: localize(9246, null),
            custom: true
        });
        if (!res.confirmed) {
            return;
    ...
    async reproduceIssueWithExtensionsDisabled() {
        ...
        const result = await this.askToReproduceIssue(localize(9247, null));
        if (result === 'good') {
            const profile = this.userDataProfilesService.profiles.find(p => p.id === this.state.profile) ?? this.userDataProfilesService.defaultProfile;
    ...
    async reproduceIssueWithEmptyProfile() {
        ...
        const result = await this.askToReproduceIssue(localize(9248, null));
        if (result === 'stop') {
            await this.stop();
        }
        if (result === 'good') {
            await this.askToReportIssue(localize(9249, null));
        }
        if (result === 'bad') {
            await this.askToReportIssue(localize(9250, null, this.productService.nameLong));
        }
    }
    ...
    askToReproduceIssue(message) {
        return new Promise((c, e) => {
            const goodPrompt = {
                label: localize(9251, null),
                run: () => c('good')
            };
            const badPrompt = {
                label: localize(9252, null),
                run: () => c('bad')
            };
            const stop = {
                label: localize(9253, null),
                run: () => c('stop')
            };
        ...
        if (this.productService.quality === 'stable') {
            ...
            if (res === 'good') {
                await this.dialogService.prompt({
                    type: Severity.Info,
                    message: localize(9254, null),
                    detail: localize(9255, null, this.productService.nameLong),
                    custom: true
                });
                return;
            }
    ...
    async askToReproduceIssueWithInsiders() {
        const confirmRes = await this.dialogService.confirm({
            type: 'info',
            message: localize(9256, null),
            primaryButton: localize(9257, null, this.productService.nameLong),
            cancelButton: localize(9258, null),
            detail: localize(9259, null, this.productService.nameLong),
            custom: {
        ...
        const res = await this.dialogService.prompt({
            type: 'info',
            message: localize(9260, null),
            buttons: [{
                    label: localize(9261, null),
                    run: () => 'good'
                }, {
                    label: localize(9262, null),
                    run: () => 'bad'
                }],
            cancelButton: {
                label: localize(9263, null),
                run: () => 'stop'
            },
            detail: localize(9264, null, this.productService.nameLong),
            custom: {
                disableCloseAction: true,
            }
        });
...
registerAction2(class TroubleshootIssueAction extends Action2 {
    constructor() {
        super({
            ...
            title: localize2(9265, 'Troubleshoot Issue...'),
...
registerAction2(class extends Action2 {
    constructor() {
        super({
            ...
            title: localize2(9266, 'Stop Troubleshoot Issue'),
            category: Categories.Help,
            f1: true,
