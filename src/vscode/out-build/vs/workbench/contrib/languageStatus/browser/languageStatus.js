...
let LanguageStatus = class LanguageStatus {
    ...
    _update() {
        ...
        if (model.combined.length === 0) {
            ...
            const props = {
                name: localize(9283, null),
                ariaLabel: localize(9284, null, ariaLabels.join(', next: ')),
    ...
    _renderStatus(container, status, showSeverity, isPinned, store) {
        ...
        const actionLabel = isPinned ? localize(9285, null) : localize(9286, null);
    ...
    static _accessibilityInformation(status) {
        ...
        if (status.detail) {
            return { label: localize(9287, null, textValue, status.detail) };
        }
        else {
            return { label: localize(9288, null, textValue) };
        }
    }
    // ---
    static _asStatusbarEntry(item) {
        ...
        return {
            name: localize(9289, null, item.name),
...
export class ResetAction extends Action2 {
    constructor() {
        super({
            ...
            title: localize2(9290, "Reset Language Status Interaction Counter"),
