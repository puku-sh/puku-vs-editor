        ...
        for (let i = 0; i < str.length; i++) {
            if (str[i] === '\\') {
                ...
                if (escaped === 'n') {
                    ...
                    throw Error(localize(11975, null));
                }
            }
            else {
                out += str[i];
...
export const serializeSearchResultForEditor = (searchResult, rawIncludePattern, rawExcludePattern, contextLines, labelFormatter, sortOrder, limitHit) => {
    ...
    const filecount = searchResult.fileCount() > 1 ? localize(11976, null, searchResult.fileCount()) : localize(11977, null);
    const resultcount = searchResult.count() > 1 ? localize(11978, null, searchResult.count()) : localize(11979, null);
    const info = [
        searchResult.count()
