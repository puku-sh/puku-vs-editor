...
const SearchEditorIcon = registerIcon('search-editor-label-icon', Codicon.search, localize(11971, null));
let SearchEditorInput = class SearchEditorInput extends EditorInput {
    ...
    getName(maxLength = 12) {
        ...
        if (this.backingUri) {
            ...
            return localize(11972, null, basename((originalURI ?? this.backingUri).path, SEARCH_EDITOR_EXT));
        }
        const query = this._cachedConfigurationModel?.config?.query?.trim();
        if (query) {
            return localize(11973, null, trimToMax(query));
        }
        return localize(11974, null);
    }
    setDirty(dirty) {
