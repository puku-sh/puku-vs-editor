...
(function (snippetExt) {
    function toValidSnippet(extension, snippet, languageService) {
        if (isFalsyOrWhitespace(snippet.path)) {
            extension.collector.error(localize(12043, null, extension.description.name, String(snippet.path)));
            return null;
        }
        if (isFalsyOrWhitespace(snippet.language) && !snippet.path.endsWith('.code-snippets')) {
            extension.collector.error(localize(12044, null, extension.description.name, String(snippet.path)));
            return null;
        }
        if (!isFalsyOrWhitespace(snippet.language) && !languageService.isRegisteredLanguageId(snippet.language)) {
            extension.collector.error(localize(12045, null, extension.description.name, String(snippet.language)));
            return null;
        }
        ...
        if (!resources.isEqualOrParent(snippetLocation, extensionLocation)) {
            extension.collector.error(localize(12046, null, extension.description.name, snippetLocation.path, extensionLocation.path));
            return null;
        }
        return {
    ...
    snippetExt.snippetsContribution = {
        description: localize(12047, null),
        type: 'array',
        ...
        items: {
            ...
            properties: {
                language: {
                    description: localize(12048, null),
                    type: 'string'
                },
                path: {
                    description: localize(12049, null),
                    type: 'string'
                }
            }
        }
                        ...
                        if (this._environmentService.isExtensionDevelopment) {
                            file.load().then(file => {
                                ...
                                if (file.data.some(snippet => snippet.isBogous)) {
                                    extension.collector.warn(localize(12050, null, extension.description.name));
                                }
                            }, err => {
                                // generic error
                                extension.collector.warn(localize(12051, null, file.location.toString()));
                            });
                        }
                    }
                }
