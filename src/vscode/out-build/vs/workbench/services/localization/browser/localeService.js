...
let WebLocaleService = class WebLocaleService {
    ...
    async setLocale(languagePackItem, _skipDialog = false) {
        ...
        const restartDialog = await this.dialogService.confirm({
            type: 'info',
            message: localize(15596, null, this.productService.nameLong),
            detail: localize(15597, null, languagePackItem.label),
            primaryButton: localize(15598, null),
        });
    ...
    async clearLocalePreference() {
        ...
        const restartDialog = await this.dialogService.confirm({
            type: 'info',
            message: localize(15599, null, this.productService.nameLong),
            detail: localize(15600, null),
            primaryButton: localize(15601, null),
        });
        if (restartDialog.confirmed) {
            this.hostService.restart();
        }
