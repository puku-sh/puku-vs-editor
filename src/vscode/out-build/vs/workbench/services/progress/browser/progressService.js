    ...
    updateWindowProgress(idx = 0) {
        ...
        if (idx < this.windowProgressStack.length) {
            ...
            if (progressTitle && progressMessage) {
                // <title>: <message>
                text = localize(15660, null, progressTitle, progressMessage);
                title = source ? localize(15661, null, source, progressTitle, progressMessage) : text;
            }
            else if (progressTitle) {
                // <title>
                text = progressTitle;
                title = source ? localize(15662, null, source, progressTitle) : text;
            }
            else if (progressMessage) {
                // <message>
                text = progressMessage;
                title = source ? localize(15663, null, source, progressMessage) : text;
            }
            else {
        ...
        const createNotification = (message, priority, increment) => {
            ...
            if (options.cancellable) {
                const cancelAction = new class extends Action {
                    constructor() {
                        super('progress.cancel', typeof options.cancellable === 'string' ? options.cancellable : localize(15665, null), undefined, true);
                    }
                    async run() {
        ...
        const createDialog = (message) => {
            ...
            if (!options.sticky) {
                buttons.push(options.cancellable
                    ? (typeof options.cancellable === 'boolean' ? localize(15666, null) : options.cancellable)
                    : localize(15667, null));
            }
