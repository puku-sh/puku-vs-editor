    ...
    async handleURL(uri, options) {
        ...
        if (!trusted) {
            ...
            const result = await this.dialogService.confirm({
                message: localize(15361, null, extensionDisplayName),
                checkbox: {
                    label: localize(15362, null),
                },
                primaryButton: localize(15363, null),
                custom: {
                    markdownDetails: [{
    ...
    async handleUnhandledURL(uri, extensionId, options) {
        try {
            await this.commandService.executeCommand('workbench.extensions.installExtension', extensionId, {
                justification: {
                    reason: `${localize(15364, null)}\n${uri.toString()}`,
                    action: localize(15365, null)
                },
                enable: true,
        ...
        /* Extension cannot be added and require window reload */
            ...
            const result = await this.dialogService.confirm({
                message: localize(15366, null, extensionId),
                primaryButton: localize(15367, null)
            });
            if (!result.confirmed) {
                return;
            }
...
class ManageAuthorizedExtensionURIsAction extends Action2 {
    constructor() {
        super({
            ...
            title: localize2(15369, 'Manage Authorized Extension URIs...'),
            category: localize2(15370, 'Extensions'),
            menu: {
    ...
    async run(accessor) {
        ...
        if (items.length === 0) {
            await quickInputService.pick([{ label: localize(15368, null) }]);
            return;
        }
