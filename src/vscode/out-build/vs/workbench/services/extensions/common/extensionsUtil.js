...
export function dedupExtensions(system, user, workspace, development, logService) {
    ...
    system.forEach((systemExtension) => {
        ...
        if (extension) {
            logService.warn(localize(15471, null, extension.extensionLocation.fsPath, systemExtension.extensionLocation.fsPath));
        }
    ...
    user.forEach((userExtension) => {
        ...
        if (extension) {
            if (extension.isBuiltin) {
                ...
                logService.warn(localize(15472, null, extension.extensionLocation.fsPath, userExtension.extensionLocation.fsPath));
            }
        }
    ...
    workspace.forEach(workspaceExtension => {
        ...
        if (extension) {
            logService.warn(localize(15473, null, extension.extensionLocation.fsPath, workspaceExtension.extensionLocation.fsPath));
        }
        result.set(workspaceExtension.identifier, workspaceExtension);
    });
    development.forEach(developedExtension => {
        logService.info(localize(15474, null, developedExtension.extensionLocation.fsPath));
