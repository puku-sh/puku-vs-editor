...
let AbstractExtensionService = AbstractExtensionService_1 = class AbstractExtensionService extends Disposable {
    ...
    async _deltaExtensions(lock, _toAdd, _toRemove) {
        ...
        if (result.removedDueToLooping.length > 0) {
            this._notificationService.notify({
                ...
                message: nls.localize(15373, null, result.removedDueToLooping.map(e => `'${e.identifier.value}'`).join(', '))
            });
        }
    ...
    async _resolveAndProcessExtensions(lock) {
        ...
        if (result.removedDueToLooping.length > 0) {
            this._notificationService.notify({
                severity: Severity.Error,
                message: nls.localize(15374, null, result.removedDueToLooping.map(e => `'${e.identifier.value}'`).join(', '))
            });
        }
    ...
    async _handleExtensionTests() {
        ...
        if (!extensionHostManager) {
            const msg = nls.localize(15375, null, this._environmentService.extensionTestsLocationURI.toString());
            console.error(msg);
            ...
            veto(value, reason) {
                ...
                if (typeof value === 'boolean') {
                    ...
                    value.then(value => {
                        ...
                        vetoReasons.add(nls.localize(15376, null, reason, toErrorMessage(error)));
                    });
                }
            }
        });
        ...
        if (!veto) {
            ...
            if (!auto) {
                ...
                const { confirmed } = await this._dialogService.confirm({
                    ...
                    message: nls.localize(15377, null),
                    detail: vetoReasonsArray.length === 1 ?
                        vetoReasonsArray[0] :
                        vetoReasonsArray.join('\n -'),
                    primaryButton: nls.localize(15378, null)
                });
                if (confirmed) {
                    return true;
                }
    ...
    async _onRemoteExtensionHostCrashed(extensionHost, reconnectionToken) {
        try {
            ...
            if (this._remoteCrashTracker.shouldAutomaticallyRestart()) {
                ...
                this._notificationService.status(nls.localize(15379, null), { hideAfter: 5000 });
                this._startExtensionHostsIfNecessary(false, Array.from(this._allRequestedActivateEvents.keys()));
            }
            else {
                this._notificationService.prompt(Severity.Error, nls.localize(15380, null), [{
                        label: nls.localize(15381, null),
                        run: () => {
                            this._startExtensionHostsIfNecessary(false, Array.from(this._allRequestedActivateEvents.keys()));
                        }
                    }]);
