...
let FileWorkingCopyManager = class FileWorkingCopyManager extends Disposable {
    ...
    static { this.FILE_WORKING_COPY_SAVE_CREATE_SOURCE = SaveSourceRegistry.registerSource('fileWorkingCopyCreate.source', localize(16180, null)); }
    static { this.FILE_WORKING_COPY_SAVE_REPLACE_SOURCE = SaveSourceRegistry.registerSource('fileWorkingCopyReplace.source', localize(16181, null)); }
    ...
    provideDecorations() {
        ...
        const provider = this._register(new class extends Disposable {
            constructor(stored) {
                super();
                this.stored = stored;
                this.label = localize(16182, null);
    ...
    async confirmOverwrite(resource) {
        const { confirmed } = await this.dialogService.confirm({
            type: 'warning',
            message: localize(16186, null, basename(resource)),
            detail: localize(16187, null, basename(resource), basename(dirname(resource))),
            primaryButton: localize(16188, null)
        });
        return confirmed;
    }
    async confirmMakeWriteable(resource) {
        const { confirmed } = await this.dialogService.confirm({
            type: 'warning',
            message: localize(16189, null, basename(resource)),
            detail: localize(16190, null),
            primaryButton: localize(16191, null)
        });
        return confirmed;
    }
