...
let StoredFileWorkingCopy = class StoredFileWorkingCopy extends ResourceWorkingCopy {
    ...
    async doSave(options) {
        ...
        return this.progressService.withProgress({
            title: localize(16192, null, this.name),
    ...
    doSaveSequential(versionId, options, progress, saveCancellation) {
        return this.saveSequentializer.run(versionId, (async () => {
            ...
            progress.report({ message: localize(16193, null) });
    ...
    doHandleSaveError(error, options) {
        ...
        if (fileOperationError.fileOperationResult === 3 /* FileOperationResult.FILE_MODIFIED_SINCE */) {
            message = localize(16194, null, this.name);
            primaryActions.push(toAction({ id: 'fileWorkingCopy.overwrite', label: localize(16195, null), run: () => this.save({ ...options, ignoreModifiedSince: true, reason: 1 /* SaveReason.EXPLICIT */ }) }));
            primaryActions.push(toAction({ id: 'fileWorkingCopy.revert', label: localize(16196, null), run: () => this.revert() }));
        }
        // Any other save error
            ...
            if (canSaveElevated && (isPermissionDenied || triedToUnlock)) {
                primaryActions.push(toAction({
                    ...
                    label: triedToUnlock ?
                        isWindows ? localize(16197, null) : localize(16198, null) :
                        isWindows ? localize(16199, null) : localize(16200, null),
                    run: () => {
            ...
            // Unlock
                ...
                primaryActions.push(toAction({ id: 'fileWorkingCopy.unlock', label: localize(16201, null), run: () => this.save({ ...options, writeUnlock: true, reason: 1 /* SaveReason.EXPLICIT */ }) }));
            }
            // Retry
            else {
                primaryActions.push(toAction({ id: 'fileWorkingCopy.retry', label: localize(16202, null), run: () => this.save({ ...options, reason: 1 /* SaveReason.EXPLICIT */ }) }));
            }
            // Save As
            primaryActions.push(toAction({
                id: 'fileWorkingCopy.saveAs',
                label: localize(16203, null),
                run: async () => {
            ...
            // Revert
            primaryActions.push(toAction({ id: 'fileWorkingCopy.revert', label: localize(16204, null), run: () => this.revert() }));
            // Message
            if (isWriteLocked) {
                if (triedToUnlock && canSaveElevated) {
                    message = isWindows ?
                        localize(16205, null, this.name) :
                        localize(16206, null, this.name);
                }
                else {
                    message = localize(16207, null, this.name);
                }
            }
            else if (canSaveElevated && isPermissionDenied) {
                message = isWindows ?
                    localize(16208, null, this.name) :
                    localize(16209, null, this.name);
            }
            else {
                message = localize(16210, null, this.name, toErrorMessage(error, false));
            }
        }
