...
let UserDataSyncWorkbenchService = class UserDataSyncWorkbenchService extends Disposable {
    ...
    async turnOn() {
        if (!this.authenticationProviders.length) {
            throw new Error(localize(16018, null));
        }
        ...
        if (this.accountStatus !== "available" /* AccountStatus.Available */) {
            throw new Error(localize(16019, null));
        }
        ...
        const disposable = isWeb ? Disposable.None : this.lifecycleService.onBeforeShutdown(e => e.veto((async () => {
            const { confirmed } = await this.dialogService.confirm({
                type: 'warning',
                message: localize(16020, null),
                title: localize(16021, null),
                primaryButton: localize(16022, null),
                cancelButton: localize(16023, null)
            });
            if (confirmed) {
        ...
        this.notificationService.info(localize(16024, null, SYNC_TITLE.value));
    ...
    async doTurnOnSync(token) {
        ...
        try {
            await this.progressService.withProgress({
                ...
                progress.report({ message: localize(16025, null) });
                disposables.add(this.userDataSyncService.onDidChangeStatus(status => {
                    if (status === "hasConflicts" /* SyncStatus.HasConflicts */) {
                        progress.report({ message: localize(16026, null) });
                    }
                    else {
                        progress.report({ message: localize(16027, null) });
                    }
                }));
    ...
    async handleConflictsWhileTurningOn(token) {
        ...
        const andSeparator = localize(16028, null);
        let conflictsText = '';
        ...
        await this.dialogService.prompt({
            ...
            message: localize(16029, null, conflictsText),
            detail: localize(16030, null),
            buttons: [
                {
                    label: localize(16031, null),
                    run: async () => {
                ...
                {
                    label: singleConflictResource ? localize(16032, null, singleConflictResource) : localize(16033, null),
                    run: async () => this.replace(true)
                },
                {
                    label: singleConflictResource ? localize(16034, null, singleConflictResource) : localize(16035, null),
    ...
    async resetSyncedData() {
        const { confirmed } = await this.dialogService.confirm({
            type: 'info',
            message: localize(16036, null),
            title: localize(16037, null),
            primaryButton: localize(16038, null),
        });
        if (confirmed) {
    ...
    async downloadSyncActivity() {
        const result = await this.fileDialogService.showOpenDialog({
            title: localize(16039, null),
            canSelectFiles: false,
            canSelectFolders: true,
            canSelectMany: false,
            openLabel: localize(16040, null),
        });
        if (!result?.[0]) {
            return;
        }
    ...
    async signIn() {
        ...
        if (authenticationProvider) {
            ...
            if (!this.authenticationProviders.length) {
                throw new Error(localize(16041, null));
            }
            await this.pick();
        }
    }
    ...
    async doPick() {
        ...
        quickPick.placeholder = localize(16042, null);
        quickPick.show();
        if (authenticationProviders.length > 1) {
            quickPick.busy = true;
            for (const { id, scopes } of authenticationProviders) {
    ...
    createQuickpickItems(authenticationProviders, allAccounts) {
        ...
        // Signed in Accounts
        if (allAccounts.size) {
            quickPickItems.push({ type: 'separator', label: localize(16043, null) });
            for (const authenticationProvider of authenticationProviders) {
                ...
                for (const account of accounts) {
                    quickPickItems.push({
                        ...
                        description: account.sessionId === this.current?.sessionId ? localize(16044, null) : undefined,
                        account,
                        authenticationProvider,
                    });
                }
            }
            quickPickItems.push({ type: 'separator', label: localize(16045, null) });
        }
        // Account Providers
        for (const authenticationProvider of authenticationProviders) {
            ...
            if (!allAccounts.has(authenticationProvider.id) || provider.supportsMultipleAccounts) {
                ...
                quickPickItems.push({ label: localize(16046, null, providerName), authenticationProvider });
            }
        }
