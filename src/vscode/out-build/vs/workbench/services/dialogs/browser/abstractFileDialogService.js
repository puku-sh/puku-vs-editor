...
let AbstractFileDialogService = class AbstractFileDialogService {
    ...
    async doShowSaveConfirm(fileNamesOrResources) {
        ...
        let message;
        let detail = nls.localize(15197, null);
        if (fileNamesOrResources.length === 1) {
            message = nls.localize(15198, null, typeof fileNamesOrResources[0] === 'string' ? fileNamesOrResources[0] : resources.basename(fileNamesOrResources[0]));
        }
        else {
            message = nls.localize(15199, null, fileNamesOrResources.length);
            ...
            buttons: [
                {
                    label: fileNamesOrResources.length > 1 ?
                        nls.localize(15200, null) :
                        nls.localize(15201, null),
                    run: () => 0 /* ConfirmResult.SAVE */
                },
                {
                    label: nls.localize(15202, null),
    ...
    async pickFileFolderAndOpenSimplified(schema, options, preferNewWindow) {
        const title = nls.localize(15203, null);
    ...
    async pickFileAndOpenSimplified(schema, options, preferNewWindow) {
        const title = nls.localize(15204, null);
        const availableFileSystems = this.addFileSchemaIfNeeded(schema);
    ...
    async pickFolderAndOpenSimplified(schema, options) {
        const title = nls.localize(15205, null);
    ...
    async pickWorkspaceAndOpenSimplified(schema, options) {
        const title = nls.localize(15206, null);
        const filters = [{ name: nls.localize(15207, null), extensions: [WORKSPACE_EXTENSION] }];
        const availableFileSystems = this.addFileSchemaIfNeeded(schema, true);
    ...
    async pickFileToSaveSimplified(schema, options) {
        ...
        options.title = nls.localize(15208, null);
    ...
    getPickFileToSaveDialogOptions(defaultUri, availableFileSystems) {
        ...
        options.filters = coalesce([
            { name: nls.localize(15210, null), extensions: ['*'] },
            matchingFilter,
            ...registeredLanguageFilters,
            { name: nls.localize(15211, null), extensions: [''] }
        ]);
        return options;
    }
};
