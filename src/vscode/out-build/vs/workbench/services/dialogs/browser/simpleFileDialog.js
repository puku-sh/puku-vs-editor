...
(function (OpenLocalFileCommand) {
    ...
    OpenLocalFileCommand.LABEL = nls.localize(15220, null);
    function handler() {
        return accessor => {
...
(function (SaveLocalFileCommand) {
    ...
    SaveLocalFileCommand.LABEL = nls.localize(15221, null);
    function handler() {
        return accessor => {
...
(function (OpenLocalFolderCommand) {
    ...
    OpenLocalFolderCommand.LABEL = nls.localize(15222, null);
    function handler() {
        return accessor => {
            const dialogService = accessor.get(IFileDialogService);
...
(function (OpenLocalFileFolderCommand) {
    ...
    OpenLocalFileFolderCommand.LABEL = nls.localize(15223, null);
    function handler() {
        return accessor => {
            const dialogService = accessor.get(IFileDialogService);
...
let SimpleFileDialog = class SimpleFileDialog extends Disposable {
    ...
    getOptions(options, isSave = false) {
        ...
        if ((this.scheme !== Schemas.file) && !this.fileService.hasProvider(defaultUri)) {
            this.notificationService.info(nls.localize(15224, null, defaultUri.toString()));
            return undefined;
        }
    ...
    async pickResource(isSave = false) {
        ...
        return new Promise((resolve) => {
            ...
            this.filePickBox.placeholder = nls.localize(15225, null);
            this.filePickBox.ok = true;
            this.filePickBox.okLabel = typeof this.options.openLabel === 'string' ? this.options.openLabel : this.options.openLabel?.withoutMnemonic;
            if ((this.scheme !== Schemas.file) && this.options && this.options.availableFileSystems && (this.options.availableFileSystems.length > 1) && (this.options.availableFileSystems.indexOf(Schemas.file) > -1)) {
                this.filePickBox.customButton = true;
                this.filePickBox.customLabel = nls.localize(15226, null);
                let action;
                if (isSave) {
    ...
    setButtons() {
        this.filePickBox.buttons = [{
                ...
                tooltip: this._showDotFiles ? nls.localize(15227, null) : nls.localize(15228, null),
                alwaysVisible: true
            }];
    }
    isBadSubpath(value) {
        ...
        if ((value.length > 0) && ((value[0] === '~') || this.canTildaEscapeHatch(value))) {
            ...
            if ((!newFolderIsOldFolder && (this.endsWithSlash(value) || newFolderIsParent || newFolderIsUnrelated)) || reset) {
                ...
                if (stat?.isDirectory && (resources.basename(valueUri) !== '.') && this.endsWithSlash(value)) {
                    ...
                    this.filePickBox.validationMessage = nls.localize(15229, null);
    ...
    yesNoPrompt(uri, message) {
        ...
        prompt.customLabel = nls.localize(15230, null);
    ...
    async validate(uri) {
        if (uri === undefined) {
            this.filePickBox.validationMessage = nls.localize(15231, null);
        ...
        if (this.requiresTrailing) { // save
            if (stat?.isDirectory) {
                // Can't do this
                this.filePickBox.validationMessage = nls.localize(15232, null);
                return Promise.resolve(false);
            }
            else if (stat) {
                // Replacing a file.
                // Show a yes/no prompt
                const message = nls.localize(15233, null, resources.basename(uri));
                return this.yesNoPrompt(uri, message);
            }
            else if (!(isValidBasename(resources.basename(uri), this.isWindows))) {
                // Filename not allowed
                this.filePickBox.validationMessage = nls.localize(15234, null);
                return Promise.resolve(false);
            }
            else if (!statDirname) {
                // Folder to save in doesn't exist
                const message = nls.localize(15235, null, resources.basename(resources.dirname(uri)));
                return this.yesNoPrompt(uri, message);
            }
            else if (!statDirname.isDirectory) {
                this.filePickBox.validationMessage = nls.localize(15236, null);
                return Promise.resolve(false);
            }
            else if (statDirname.readonly) {
                this.filePickBox.validationMessage = nls.localize(15237, null);
                return Promise.resolve(false);
            }
        }
        else { // open
            if (!stat) {
                ...
                this.filePickBox.validationMessage = nls.localize(15238, null);
                return Promise.resolve(false);
            }
            else if (uri.path === '/' && this.isWindows) {
                this.filePickBox.validationMessage = nls.localize(15239, null);
                return Promise.resolve(false);
            }
            else if (stat.isDirectory && !this.allowFolderSelection) {
                // Folder selected when folder selection not permitted
                this.filePickBox.validationMessage = nls.localize(15240, null);
                return Promise.resolve(false);
            }
            else if (!stat.isDirectory && !this.allowFileSelection) {
                // File selected when file selection not permitted
                this.filePickBox.validationMessage = nls.localize(15241, null);
                return Promise.resolve(false);
            }
        }
