...
let AuxiliaryWindow = class AuxiliaryWindow extends BaseWindow {
    ...
    preventUnload(e) {
        e.preventDefault();
        e.returnValue = localize(15089, null);
    }
...
let BrowserAuxiliaryWindowService = class BrowserAuxiliaryWindowService extends Disposable {
    ...
    async open(options) {
        ...
        if (!targetWindow) {
            throw new Error(localize(15090, null));
        }
    ...
    async openWindow(options) {
        ...
        if (!auxiliaryWindow && isWeb) {
            return (await this.dialogService.prompt({
                ...
                message: localize(15091, null),
                custom: {
                    markdownDetails: [{ markdown: new MarkdownString(localize(15092, null, 'https://aka.ms/allow-vscode-popup'), true) }]
                },
                buttons: [
                    {
                        label: localize(15093, null),
