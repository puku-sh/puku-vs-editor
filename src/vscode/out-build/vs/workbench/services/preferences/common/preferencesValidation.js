    ...
    return value => {
        ...
        if (prop.type === 'string') {
            if (prop.enum && !isStringArray(prop.enum)) {
                errors.push(nls.localize(15633, null));
            }
            else if (!isString(value)) {
                errors.push(nls.localize(15634, null));
            }
            else {
...
export function getInvalidTypeError(value, type) {
    ...
    if (!typeArr.some(_type => valueValidatesAsType(value, _type))) {
        return nls.localize(15635, null, JSON.stringify(type));
    }
    return;
}
...
function toRegExp(pattern) {
    try {
        ...
        try {
            ...
            console.error(nls.localize(15636, null), pattern);
            return /.*/;
        }
    }
}
function getStringValidators(prop) {
    ...
    return [
        ...
        },
        {
            ...
            message: nls.localize(15643, null)
        },
        {
            ...
            message: nls.localize(15644, null, prop.enum ? prop.enum.map(key => `"${key}"`).join(', ') : '[]')
        }
        ...
        if (propItems && !Array.isArray(propItems.type)) {
            ...
            return value => {
                ...
                if (!Array.isArray(value)) {
                    message += nls.localize(15651, null);
                    message += '\n';
                    return message;
                }
                ...
                if (prop.uniqueItems) {
                    if (new Set(arrayValue).size < arrayValue.length) {
                        message += nls.localize(15652, null);
                        message += '\n';
                    }
                }
                if (prop.minItems && arrayValue.length < prop.minItems) {
                    message += nls.localize(15653, null, prop.minItems);
                    message += '\n';
                }
                if (prop.maxItems && arrayValue.length > prop.maxItems) {
                    message += nls.localize(15654, null, prop.maxItems);
                    message += '\n';
                }
                if (propItems.type === 'string') {
                    if (!isStringArray(arrayValue)) {
                        message += nls.localize(15655, null);
                        message += '\n';
                        return message;
                    }
                            ...
                            if (!patternRegex.test(v)) {
                                message +=
                                    propItems.patternErrorMessage ||
                                        nls.localize(15656, null, withQuotes(v), withQuotes(propItems.pattern));
                            }
                        });
                    }
                    ...
                    if (propItemsEnum) {
                        arrayValue.forEach(v => {
                            if (propItemsEnum.indexOf(v) === -1) {
                                message += nls.localize(15657, null, withQuotes(v), '[' + propItemsEnum.map(withQuotes).join(', ') + ']');
                                message += '\n';
                            }
                        });
                    }
...
function getObjectValidator(prop) {
    if (prop.type === 'object') {
        ...
        return value => {
            ...
            const errors = [];
            if (!isObject(value)) {
                errors.push(nls.localize(15658, null));
            }
            else {
                Object.keys(value).forEach((key) => {
                    ...
                    if (additionalProperties === false) {
                        errors.push(nls.localize(15659, null, key));
                    }
