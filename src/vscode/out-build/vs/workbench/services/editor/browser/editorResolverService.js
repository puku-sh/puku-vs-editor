...
let EditorResolverService = class EditorResolverService extends Disposable {
    ...
    async doHandleConflictingDefaults(resource, editorName, untypedInput, currentEditor, group) {
        ...
        const handle = this.notificationService.prompt(Severity.Warning, localize(15242, null), [{
                label: localize(15243, null),
                run: async () => {
            ...
            {
                label: localize(15244, null, editorName),
    ...
    mapEditorsToQuickPickEntry(resource, showDefaultPicker) {
        ...
        const currentlyActiveLabel = localize(15245, null);
        const currentDefaultLabel = localize(15246, null);
        const currentDefaultAndActiveLabel = localize(15247, null);
        ...
        if (!showDefaultPicker && extname(resource) !== '') {
            ...
            const configureDefaultEntry = {
                ...
                label: localize(15248, null, `*${extname(resource)}`),
            };
    ...
    async doPickEditor(editor, showDefaultPicker) {
        ...
        const placeHolderMessage = showDefaultPicker ?
            localize(15249, null, `*${extname(resource)}`) :
            localize(15250, null, basename(resource));
