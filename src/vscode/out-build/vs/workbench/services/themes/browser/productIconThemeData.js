...
export class ProductIconThemeData {
    ...
    async load(fileService, logService) {
        ...
        this.isLoaded = true;
        if (warnings.length) {
            logService.error(nls.localize(15754, null, location.toString(), warnings.join('\n')));
        }
    ...
    static get defaultTheme() {
        ...
        if (!themeData) {
            themeData = ProductIconThemeData._defaultProductIconTheme = new ProductIconThemeData(DEFAULT_PRODUCT_ICON_THEME_ID, nls.localize(15755, null), ThemeSettingDefaults.PRODUCT_ICON_THEME);
...
function _loadProductIconThemeDocument(fileService, location, warnings) {
    return fileService.readExtensionResource(location).then((content) => {
        ...
        if (parseErrors.length > 0) {
            return Promise.reject(new Error(nls.localize(15756, null, parseErrors.map(e => getParseErrorMessage(e.error)).join(', '))));
        }
        else if (Json.getNodeType(contentValue) !== 'object') {
            return Promise.reject(new Error(nls.localize(15757, null)));
        }
        else if (!contentValue.iconDefinitions || !Array.isArray(contentValue.fonts) || !contentValue.fonts.length) {
            return Promise.reject(new Error(nls.localize(15758, null)));
        }
        ...
        for (const font of contentValue.fonts) {
            ...
            if (isString(fontId) && fontId.match(fontIdRegex)) {
                ...
                if (isString(font.weight) && font.weight.match(fontWeightRegex)) {
                    ...
                    warnings.push(nls.localize(15759, null, font.id));
                }
                ...
                if (isString(font.style) && font.style.match(fontStyleRegex)) {
                    ...
                    warnings.push(nls.localize(15760, null, font.id));
                }
                const sanitizedSrc = [];
                if (Array.isArray(font.src)) {
                    for (const s of font.src) {
                        if (isString(s.path) && isString(s.format) && s.format.match(fontFormatRegex)) {
                            ...
                            warnings.push(nls.localize(15761, null, font.id));
                        }
                    }
                }
                if (sanitizedSrc.length) {
                    ...
                    warnings.push(nls.localize(15762, null, font.id));
                }
            }
            else {
                warnings.push(nls.localize(15763, null, font.id));
            }
        }
        ...
        for (const iconId in contentValue.iconDefinitions) {
            ...
            if (isString(definition.fontCharacter)) {
                ...
                if (fontDefinition) {
                    ...
                    warnings.push(nls.localize(15764, null, iconId));
                }
            }
            else {
                warnings.push(nls.localize(15765, null, iconId));
            }
        }
