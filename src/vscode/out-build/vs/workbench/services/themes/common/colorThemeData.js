...
async function _loadColorTheme(extensionResourceLoaderService, themeLocation, result) {
    if (resources.extname(themeLocation) === '.json') {
        ...
        if (errors.length > 0) {
            return Promise.reject(new Error(nls.localize(15789, null, errors.map(e => getParseErrorMessage(e.error)).join(', '))));
        }
        else if (Json.getNodeType(contentValue) !== 'object') {
            return Promise.reject(new Error(nls.localize(15790, null)));
        }
        ...
        if (colors) {
            if (typeof colors !== 'object') {
                return Promise.reject(new Error(nls.localize(15791, null, themeLocation.toString())));
            }
        ...
        if (tokenColors) {
            if (Array.isArray(tokenColors)) {
                ...
                return Promise.reject(new Error(nls.localize(15792, null, themeLocation.toString())));
            }
        }
        ...
        if (semanticTokenColors && typeof semanticTokenColors === 'object') {
            for (const key in semanticTokenColors) {
                try {
                    ...
                    return Promise.reject(new Error(nls.localize(15793, null, themeLocation.toString())));
                }
            }
        }
    }
    ...
    return extensionResourceLoaderService.readExtensionResource(themeLocation).then(content => {
        try {
            ...
            if (!Array.isArray(settings)) {
                return Promise.reject(new Error(nls.localize(15794, null)));
            }
            convertSettings(settings, result);
            return Promise.resolve(null);
        }
        catch (e) {
            return Promise.reject(new Error(nls.localize(15795, null, e.message)));
        }
    }, error => {
        return Promise.reject(new Error(nls.localize(15796, null, themeLocation.toString(), error.message)));
    });
}
