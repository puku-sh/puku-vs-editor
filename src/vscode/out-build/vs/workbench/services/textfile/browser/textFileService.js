...
let AbstractTextFileService = class AbstractTextFileService extends Disposable {
    ...
    static { this.TEXTFILE_SAVE_CREATE_SOURCE = SaveSourceRegistry.registerSource('textFileCreate.source', localize(15712, null)); }
    static { this.TEXTFILE_SAVE_REPLACE_SOURCE = SaveSourceRegistry.registerSource('textFileOverwrite.source', localize(15713, null)); }
    ...
    provideDecorations() {
        ...
        const provider = this._register(new class extends Disposable {
            constructor(files) {
                super();
                this.files = files;
                this.label = localize(15714, null);
    ...
    async doRead(resource, options) {
        ...
        try {
            ...
            if (error.decodeStreamErrorKind === 1 /* DecodeStreamErrorKind.STREAM_IS_BINARY */) {
                throw new TextFileOperationError(localize(15718, null), 0 /* TextFileOperationResult.FILE_IS_BINARY */, options);
            }
    ...
    async confirmOverwrite(resource) {
        const { confirmed } = await this.dialogService.confirm({
            type: 'warning',
            message: localize(15719, null, basename(resource)),
            detail: localize(15720, null, basename(resource), basename(dirname(resource))),
            primaryButton: localize(15721, null),
        });
        return confirmed;
    }
    async confirmMakeWriteable(resource) {
        const { confirmed } = await this.dialogService.confirm({
            type: 'warning',
            message: localize(15722, null, basename(resource)),
            detail: localize(15723, null),
            primaryButton: localize(15724, null)
        });
        return confirmed;
    }
