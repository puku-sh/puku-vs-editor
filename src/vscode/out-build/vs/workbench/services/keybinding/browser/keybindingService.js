...
function isValidContributedKeyBinding(keyBinding, rejects) {
    if (!keyBinding) {
        rejects.push(nls.localize(15524, null));
        return false;
    }
    if (typeof keyBinding.command !== 'string') {
        rejects.push(nls.localize(15525, null, 'command'));
        return false;
    }
    if (keyBinding.key && typeof keyBinding.key !== 'string') {
        rejects.push(nls.localize(15526, null, 'key'));
        return false;
    }
    if (keyBinding.when && typeof keyBinding.when !== 'string') {
        rejects.push(nls.localize(15527, null, 'when'));
        return false;
    }
    if (keyBinding.mac && typeof keyBinding.mac !== 'string') {
        rejects.push(nls.localize(15528, null, 'mac'));
        return false;
    }
    if (keyBinding.linux && typeof keyBinding.linux !== 'string') {
        rejects.push(nls.localize(15529, null, 'linux'));
        return false;
    }
    if (keyBinding.win && typeof keyBinding.win !== 'string') {
        rejects.push(nls.localize(15530, null, 'win'));
        return false;
    }
    return true;
}
const keybindingType = {
    ...
    properties: {
        command: {
            description: nls.localize(15531, null),
            type: 'string'
        },
        args: {
            description: nls.localize(15532, null)
        },
        key: {
            description: nls.localize(15533, null),
            type: 'string'
        },
        mac: {
            description: nls.localize(15534, null),
            type: 'string'
        },
        linux: {
            description: nls.localize(15535, null),
            type: 'string'
        },
        win: {
            description: nls.localize(15536, null),
            type: 'string'
        },
        when: {
            description: nls.localize(15537, null),
            type: 'string'
        },
    }
};
const keybindingsExtPoint = ExtensionsRegistry.registerExtensionPoint({
    ...
    jsonSchema: {
        description: nls.localize(15538, null),
        oneOf: [
            keybindingType,
            {
                type: 'array',
...
let WorkbenchKeybindingService = WorkbenchKeybindingService_1 = class WorkbenchKeybindingService extends AbstractKeybindingService {
    ...
    _handleKeybinding(extensionId, isBuiltin, idx, keybindings, collector, result) {
        ...
        if (rejects.length > 0) {
            collector.error(nls.localize(15539, null, keybindingsExtPoint.name, rejects.join('\n')));
        }
    }
    ...
    static _getAllCommandsAsComment(boundCommands) {
        ...
        return '// ' + nls.localize(15540, null) + '\n// - ' + pretty;
    }
        ...
        this.schema = {
            ...
            definitions: {
                ...
                'commandNames': {
                    ...
                    'description': nls.localize(15542, null),
                },
                'commandType': {
                    'anyOf': [
                        {
                        ...
                        },
                        {
                            ...
                            'enumDescriptions': this.commandsEnumDescriptions,
                            'description': nls.localize(15543, null),
                        },
                        {
                            'type': 'string'
                        },
            ...
            items: {
                ...
                'properties': {
                    'key': {
                        'type': 'string',
                        'description': nls.localize(15544, null),
                    },
                    'command': {
                        'anyOf': [
                            {
                                ...
                                'then': {
                                    ...
                                    'errorMessage': nls.localize(15545, null, 'string')
                                },
                                'else': {
                    ...
                    'when': {
                        'type': 'string',
                        'description': nls.localize(15546, null)
                    },
                    'args': {
                        'description': nls.localize(15547, null)
                    }
                },
