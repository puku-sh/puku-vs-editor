...
let WorkspaceExtensionsConfigService = class WorkspaceExtensionsConfigService extends Disposable {
    ...
    async toggleRecommendation(extensionId) {
        ...
        const workspaceOrFolders = isRecommended
            ? await this.pickWorkspaceOrFolders(recommendedWorksapceFolders, isWorkspaceRecommended ? workspace : undefined, localize(15355, null))
            : await this.pickWorkspaceOrFolders(workspace.folders, workspace.configuration ? workspace : undefined, localize(15356, null));
    ...
    async toggleUnwantedRecommendation(extensionId) {
        ...
        const workspaceOrFolders = isUnwanted
            ? await this.pickWorkspaceOrFolders(unWantedWorksapceFolders, isWorkspaceUnwanted ? workspace : undefined, localize(15357, null))
            : await this.pickWorkspaceOrFolders(workspace.folders, workspace.configuration ? workspace : undefined, localize(15358, null));
        for (const workspaceOrWorkspaceFolder of workspaceOrFolders) {
            if (isWorkspace(workspaceOrWorkspaceFolder)) {
    ...
    async pickWorkspaceOrFolders(workspaceFolders, workspace, placeHolder) {
        ...
        const folderPicks = workspaceFolders.map(workspaceFolder => {
            return {
                ...
                description: localize(15359, null),
