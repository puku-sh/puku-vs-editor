...
let BaseWindow = class BaseWindow extends Disposable {
    ...
    static async confirmOnShutdown(accessor, reason) {
        ...
        const message = reason === 2 /* ShutdownReason.QUIT */ ?
            (isMacintosh ? localize(4114, null) : localize(4115, null)) :
            localize(4116, null);
        const primaryButton = reason === 2 /* ShutdownReason.QUIT */ ?
            (isMacintosh ? localize(4117, null) : localize(4118, null)) :
            localize(4119, null);
        const res = await dialogService.confirm({
            message,
            primaryButton,
            checkbox: {
                label: localize(4120, null)
            }
        });
    ...
    onWillShutdown() {
        ...
        Event.toPromise(Event.any(Event.once(new DomEmitter(mainWindow.document.body, EventType.KEY_DOWN, true).event), Event.once(new DomEmitter(mainWindow.document.body, EventType.MOUSE_DOWN, true).event))).then(async () => {
            ...
            await this.dialogService.prompt({
                type: Severity.Error,
                message: localize(4121, null),
                detail: localize(4122, null),
                buttons: [
                    {
                        label: localize(4123, null),
            ...
            openExternal: async (href) => {
                    ...
                    if (isSafari) {
                        ...
                        if (!opened) {
                            await this.dialogService.prompt({
                                ...
                                message: localize(4124, null),
                                custom: {
                                    markdownDetails: [{ markdown: new MarkdownString(localize(4125, null, 'https://aka.ms/allow-vscode-popup'), true) }]
                                },
                                buttons: [
                                    {
                                        label: localize(4126, null),
                ...
                // handling explicitly to prevent the workbench from going down.
                    ...
                    const showProtocolUrlOpenedDialog = async () => {
                        ...
                        const buttons = [
                            {
                                label: localize(4127, null),
                                run: () => invokeProtocolHandler()
                            }
                        ];
                        if (downloadUrl !== undefined) {
                            detail = localize(4128, null, this.productService.nameLong, this.productService.nameLong);
                            buttons.push({
                                label: localize(4129, null),
                                run: async () => {
                            ...
                            detail = localize(4130, null, this.productService.nameLong, this.productService.nameLong);
                        }
