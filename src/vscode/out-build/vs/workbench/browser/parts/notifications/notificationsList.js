...
let NotificationAccessibilityProvider = class NotificationAccessibilityProvider {
    ...
    getAriaLabel(element) {
        ...
        if (this._configurationService.getValue('accessibility.verbosity.notification')) {
            accessibleViewHint = keybinding ? localize(3956, null, keybinding) : localize(3957, null);
        }
        if (!element.source) {
            return withSeverityPrefix(accessibleViewHint ? localize(3958, null, element.message.raw, accessibleViewHint) : localize(3959, null, element.message.raw), element.severity);
        }
        return withSeverityPrefix(accessibleViewHint ? localize(3960, null, element.message.raw, element.source, accessibleViewHint) : localize(3961, null, element.message.raw, element.source), element.severity);
    }
    getWidgetAriaLabel() {
        return this._options.widgetAriaLabel ?? localize(3962, null);
    }
    getRole() {
