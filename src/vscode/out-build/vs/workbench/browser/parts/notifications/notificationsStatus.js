...
let NotificationsStatus = class NotificationsStatus extends Disposable {
    ...
    updateNotificationsCenterStatusItem() {
        ...
        if (this.notificationService.getFilter() === NotificationsFilter.ERROR) {
            statusProperties = {
                ...
                ariaLabel: localize(3965, null),
                tooltip: localize(3966, null)
            };
        }
    ...
    getTooltip(notificationsInProgress) {
        if (this.isNotificationsCenterVisible) {
            return localize(3967, null);
        }
        if (this.model.notifications.length === 0) {
            return localize(3968, null);
        }
        if (notificationsInProgress === 0) {
            if (this.newNotificationsCount === 0) {
                return localize(3969, null);
            }
            if (this.newNotificationsCount === 1) {
                return localize(3970, null);
            }
            return localize(3971, null, this.newNotificationsCount);
        }
        if (this.newNotificationsCount === 0) {
            return localize(3972, null, notificationsInProgress);
        }
        if (this.newNotificationsCount === 1) {
            return localize(3973, null, notificationsInProgress);
        }
        return localize(3974, null, this.newNotificationsCount, notificationsInProgress);
    }
