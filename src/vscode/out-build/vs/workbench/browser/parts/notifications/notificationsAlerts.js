...
export class NotificationsAlerts extends Disposable {
    ...
    doTriggerAriaAlert(notification) {
        ...
        if (notification.severity === Severity.Error) {
            alertText = localize(3937, null, notification.message.linkedText.toString());
        }
        else if (notification.severity === Severity.Warning) {
            alertText = localize(3938, null, notification.message.linkedText.toString());
        }
        else {
            alertText = localize(3939, null, notification.message.linkedText.toString());
        }
        alert(alertText);
    }
}
