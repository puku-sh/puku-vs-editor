...
let PaneCompositeBar = class PaneCompositeBar extends Disposable {
    ...
    getContextMenuActionsForComposite(compositeId) {
        ...
        actions.push(new SubmenuAction('moveToMenu', localize(3983, null), moveActions));
        // Reset Location
        if (defaultLocation !== currentLocation) {
            actions.push(toAction({
                id: 'resetLocationAction', label: localize(3984, null), run: () => {
            ...
            if (viewContainerModel.allViewDescriptors.length === 1) {
                ...
                if (defaultContainer !== viewContainer) {
                    actions.push(toAction({
                        id: 'resetLocationAction', label: localize(3985, null), run: () => {
    ...
    createMoveAction(viewContainer, newLocation, defaultLocation) {
        return toAction({
            ...
            label: newLocation === 1 /* ViewContainerLocation.Panel */ ? localize(3986, null) : newLocation === 0 /* ViewContainerLocation.Sidebar */ ? localize(3987, null) : localize(3988, null),
            run: () => {
                let index;
