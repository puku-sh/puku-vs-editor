...
let PanelPart = class PanelPart extends AbstractPaneCompositePart {
    ...
    fillExtraContextMenuActions(actions) {
        ...
        const toggleShowLabelsAction = toAction({
            ...
            label: panelShowLabels ? localize(4033, null) : localize(4034, null),
            run: () => this.configurationService.updateValue('workbench.panel.showLabels', !panelShowLabels)
        });
        actions.push(...[
            new Separator(),
            new SubmenuAction('workbench.action.panel.position', localize(4035, null), positionActions),
            new SubmenuAction('workbench.action.panel.align', localize(4036, null), alignActions),
            toggleShowLabelsAction,
            toAction({ id: TogglePanelAction.ID, label: localize(4037, null), run: () => this.commandService.executeCommand(TogglePanelAction.ID) }),
        ]);
    }
