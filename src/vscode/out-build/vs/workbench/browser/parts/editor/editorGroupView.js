...
let EditorGroupView = EditorGroupView_1 = class EditorGroupView extends Themable {
    ...
    updateContainer() {
        ...
        if (this.isEmpty) {
            ...
            this.element.setAttribute('aria-label', localize(3785, null, this.ariaLabel));
        }
    ...
    get label() {
        if (this.groupsLabel) {
            return localize(3786, null, this.groupsLabel, this._index + 1);
        }
        return localize(3787, null, this._index + 1);
    }
    get ariaLabel() {
        if (this.groupsLabel) {
            return localize(3788, null, this.groupsLabel, this._index + 1);
        }
        return localize(3789, null, this._index + 1);
    }
    get disposed() {
    ...
    doMoveOrCopyEditorAcrossGroups(editor, target, openOptions, internalOptions) {
        ...
        if (!keepCopy || editor.hasCapability(8 /* EditorInputCapabilities.Singleton */) /* singleton editors will always move */) {
            ...
            if (typeof canMoveVeto === 'string') {
                this.dialogService.error(canMoveVeto, localize(3790, null));
                return false;
            }
        }
