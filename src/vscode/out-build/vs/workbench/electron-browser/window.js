...
let NativeWindow = NativeWindow_1 = class NativeWindow extends BaseWindow {
    ...
    registerListeners() {
        ...
        ipcRenderer.on('vscode:reportSharedProcessCrash', (event, ...argsRaw) => {
            this.notificationService.prompt(Severity.Error, localize(14872, null), [{
                    label: localize(14873, null),
        ...
        ipcRenderer.on('vscode:showResolveShellEnvError', (event, ...argsRaw) => {
            ...
            this.notificationService.prompt(Severity.Error, message, [{
                    label: localize(14874, null),
                    run: () => this.nativeHostService.relaunch()
                },
                {
                    label: localize(14875, null),
                    run: () => this.preferencesService.openUserSettings({ query: 'application.shellEnvironmentResolutionTimeout' })
                },
                {
                    label: localize(14876, null),
                    run: () => this.openerService.open('https://go.microsoft.com/fwlink/?linkid=2149667')
                }]);
        });
        ipcRenderer.on('vscode:showCredentialsError', (event, ...argsRaw) => {
            const message = argsRaw[0];
            this.notificationService.prompt(Severity.Error, localize(14877, null, message), [{
                    label: localize(14878, null),
                    run: () => this.openerService.open('https://go.microsoft.com/fwlink/?linkid=2190713')
                }]);
        });
        ipcRenderer.on('vscode:showTranslatedBuildWarning', () => {
            this.notificationService.prompt(Severity.Warning, localize(14879, null, this.productService.nameLong), [{
                    label: localize(14880, null),
                    run: () => {
        ...
        ipcRenderer.on('vscode:showArgvParseWarning', () => {
            this.notificationService.prompt(Severity.Warning, localize(14881, null), [{
                    label: localize(14882, null),
        ...
        ipcRenderer.on('vscode:openProxyAuthenticationDialog', async (event, ...argsRaw) => {
            ...
            const result = await this.dialogService.input({
                type: 'warning',
                message: localize(14883, null),
                primaryButton: localize(14884, null),
                inputs: [
                    { placeholder: localize(14885, null), value: payload.username },
                    { placeholder: localize(14886, null), type: 'password', value: payload.password }
                ],
                detail: localize(14887, null, `${payload.authInfo.host}:${payload.authInfo.port}`),
                checkbox: {
                    label: localize(14888, null),
    ...
    onBeforeShutdownError({ error, reason }) {
        this.dialogService.error(this.toShutdownLabel(reason, true), localize(14889, null, toErrorMessage(error)));
    }
    onWillShutdown({ reason, force, joiners }) {
        ...
        const shutdownDialogScheduler = new RunOnceScheduler(() => {
            ...
            this.progressService.withProgress({
                ...
                title: this.toShutdownLabel(reason, false),
                detail: pendingJoiners.length > 0 ? localize(14890, null, pendingJoiners.map(joiner => `- ${joiner.label}`).join('\n')) : undefined
            }, () => {
        ...
        if (isError) {
            switch (reason) {
                case 1 /* ShutdownReason.CLOSE */:
                    return localize(14891, null);
                case 2 /* ShutdownReason.QUIT */:
                    return localize(14892, null);
                case 3 /* ShutdownReason.RELOAD */:
                    return localize(14893, null);
                case 4 /* ShutdownReason.LOAD */:
                    return localize(14894, null);
            }
        }
        switch (reason) {
            case 1 /* ShutdownReason.CLOSE */:
                return localize(14895, null);
            case 2 /* ShutdownReason.QUIT */:
                return localize(14896, null);
            case 3 /* ShutdownReason.RELOAD */:
                return localize(14897, null);
            case 4 /* ShutdownReason.LOAD */:
                return localize(14898, null);
        }
    }
    toForceShutdownLabel(reason) {
        switch (reason) {
            case 1 /* ShutdownReason.CLOSE */:
                return localize(14899, null);
            case 2 /* ShutdownReason.QUIT */:
                return localize(14900, null);
            case 3 /* ShutdownReason.RELOAD */:
                return localize(14901, null);
            case 4 /* ShutdownReason.LOAD */:
                return localize(14902, null);
        }
    }
    //#endregion
    ...
    async handleWarnings() {
        ...
        (async () => {
            ...
            if (isAdmin && !isWindows) {
                this.notificationService.warn(localize(14903, null, this.productService.nameShort));
            }
        })();
            ...
            for (const folder of this.contextService.getWorkspace().folders) {
                if (this.uriIdentityService.extUri.isEqualOrParent(folder.uri, installLocationUri)) {
                    this.bannerService.show({
                        ...
                        message: localize(14904, null, this.labelService.getUriLabel(installLocationUri)),
        ...
        if (isMacintosh) {
            ...
            if (eolReleases.has(majorVersion)) {
                const message = localize(14905, null, this.productService.nameLong, eolReleases.get(majorVersion));
                this.notificationService.prompt(Severity.Warning, message, [{
                        label: localize(14906, null),
        ...
        this.progressService.withProgress({
            title: localize(14907, null),
            location: 10 /* ProgressLocation.Window */,
            delay: 1600,
            buttons: [localize(14908, null)]
...
let ZoomStatusEntry = class ZoomStatusEntry extends Disposable {
    ...
    createZoomEntry(visibleOrText) {
        ...
        const zoomOutAction = disposables.add(new Action('workbench.action.zoomOut', localize(14909, null), ThemeIcon.asClassName(Codicon.remove), true, () => this.commandService.executeCommand(zoomOutAction.id)));
        const zoomInAction = disposables.add(new Action('workbench.action.zoomIn', localize(14910, null), ThemeIcon.asClassName(Codicon.plus), true, () => this.commandService.executeCommand(zoomInAction.id)));
        const zoomResetAction = disposables.add(new Action('workbench.action.zoomReset', localize(14911, null), undefined, true, () => this.commandService.executeCommand(zoomResetAction.id)));
        zoomResetAction.tooltip = localize(14912, null, zoomResetAction.label, this.keybindingService.lookupKeybinding(zoomResetAction.id)?.getLabel());
        const zoomSettingsAction = disposables.add(new Action('workbench.action.openSettings', localize(14913, null), ThemeIcon.asClassName(Codicon.settingsGear), true, () => this.commandService.executeCommand(zoomSettingsAction.id, 'window.zoom')));
        ...
        const name = localize(14914, null);
    ...
    updateZoomLevelLabel(targetWindowId) {
        if (this.zoomLevelLabel) {
            ...
            this.zoomLevelLabel.tooltip = localize(14915, null, zoomLevel, zoomFactor);
        }
    }
};
