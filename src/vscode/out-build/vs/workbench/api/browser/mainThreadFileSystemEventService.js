                ...
                if (showPreview === undefined) {
                    ...
                    if (data.extensionNames.length === 1) {
                        if (operation === 0 /* FileOperation.CREATE */) {
                            message = localize(2825, null, data.extensionNames[0]);
                        }
                        else if (operation === 3 /* FileOperation.COPY */) {
                            message = localize(2826, null, data.extensionNames[0]);
                        }
                        else if (operation === 2 /* FileOperation.MOVE */) {
                            message = localize(2827, null, data.extensionNames[0]);
                        }
                        else /* if (operation === FileOperation.DELETE) */ {
                            message = localize(2828, null, data.extensionNames[0]);
                        }
                    }
                    else {
                        if (operation === 0 /* FileOperation.CREATE */) {
                            message = localize(2829, null, data.extensionNames.length);
                        }
                        else if (operation === 3 /* FileOperation.COPY */) {
                            message = localize(2830, null, data.extensionNames.length);
                        }
                        else if (operation === 2 /* FileOperation.MOVE */) {
                            message = localize(2831, null, data.extensionNames.length);
                        }
                        else /* if (operation === FileOperation.DELETE) */ {
                            message = localize(2832, null, data.extensionNames.length);
                        }
                    }
                    if (needsConfirmation) {
                        ...
                        const { confirmed } = await dialogService.confirm({
                            type: Severity.Info,
                            message,
                            primaryButton: localize(2833, null),
                            cancelButton: localize(2834, null)
                        });
                        showPreview = true;
                        if (!confirmed) {
                            // no changes wanted
                        ...
                        const { result, checkboxChecked } = await dialogService.prompt({
                            ...
                            checkbox: { label: localize(2838, null) }
                        });
            ...
            _progressLabel(operation) {
                switch (operation) {
                    case 0 /* FileOperation.CREATE */:
                        return localize(2839, null);
                    case 2 /* FileOperation.MOVE */:
                        return localize(2840, null);
                    case 3 /* FileOperation.COPY */:
                        return localize(2841, null);
                    case 1 /* FileOperation.DELETE */:
                        return localize(2842, null);
                    case 4 /* FileOperation.WRITE */:
                        return localize(2843, null);
                }
            }
        };
