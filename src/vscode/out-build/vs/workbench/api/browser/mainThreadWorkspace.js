...
let MainThreadWorkspace = class MainThreadWorkspace {
    ...
    getStatusMessage(extensionName, addCount, removeCount) {
        ...
        if (wantsToAdd && !wantsToDelete) {
            if (addCount === 1) {
                message = localize(2871, null, extensionName);
            }
            else {
                message = localize(2872, null, extensionName, addCount);
            }
        }
        // Delete Folders
        else if (wantsToDelete && !wantsToAdd) {
            if (removeCount === 1) {
                message = localize(2873, null, extensionName);
            }
            else {
                message = localize(2874, null, extensionName, removeCount);
            }
        }
        // Change Folders
        else {
            message = localize(2875, null, extensionName);
        }
        return message;
    }
