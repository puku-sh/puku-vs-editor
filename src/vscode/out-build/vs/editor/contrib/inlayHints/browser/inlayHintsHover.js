    ...
    computeAsync(anchor, _lineDecorations, source, token) {
        ...
        return new AsyncIterableProducer(async (executor) => {
            ...
            if (isNonEmptyArray(part.item.hint.textEdits)) {
                executor.emitOne(new MarkdownHover(this, anchor.range, [new MarkdownString().appendText(localize(1326, null))], false, 10001));
            }
            ...
            if (part.part.location || part.part.command) {
                ...
                const kb = useMetaKey
                    ...
                    : platform.isMacintosh
                        ...
                        : localize(1330, null);
                if (part.part.location && part.part.command) {
                    linkHint = new MarkdownString().appendText(localize(1331, null, kb));
                }
                else if (part.part.location) {
                    linkHint = new MarkdownString().appendText(localize(1332, null, kb));
                }
                else if (part.part.command) {
                    linkHint = new MarkdownString(`[${localize(1333, null)}](${asCommandLink(part.part.command)} "${part.part.command.title}") (${kb})`, { isTrusted: true });
                }
                if (linkHint) {
