...
export const warningIcon = registerIcon('extensions-warning-message', Codicon.warning, nls.localize(1697, null));
        ...
        this._updateState = (state) => {
            if (state && state.hasMore) {
                ...
                if (state.nonBasicAsciiCharacterCount >= max) {
                    data = {
                        message: nls.localize(1698, null),
                        command: new DisableHighlightingOfNonBasicAsciiCharactersAction(),
                    };
                }
                else if (state.ambiguousCharacterCount >= max) {
                    data = {
                        message: nls.localize(1699, null),
                        command: new DisableHighlightingOfAmbiguousCharactersAction(),
                    };
                }
                else if (state.invisibleCharacterCount >= max) {
                    data = {
                        message: nls.localize(1700, null),
...
const configureUnicodeHighlightOptionsStr = nls.localize(1701, null);
let UnicodeHighlighterHoverParticipant = class UnicodeHighlighterHoverParticipant {
    ...
    computeSync(anchor, lineDecorations) {
        ...
        for (const d of lineDecorations) {
            ...
            switch (highlightInfo.reason.kind) {
                case 0 /* UnicodeHighlighterReasonKind.Ambiguous */: {
                    if (isBasicASCII(highlightInfo.reason.confusableWith)) {
                        reason = nls.localize(1702, null, codePointStr, formatCodePointMarkdown(highlightInfo.reason.confusableWith.codePointAt(0)));
                    }
                    else {
                        reason = nls.localize(1703, null, codePointStr, formatCodePointMarkdown(highlightInfo.reason.confusableWith.codePointAt(0)));
                    }
                    break;
                }
                case 1 /* UnicodeHighlighterReasonKind.Invisible */:
                    reason = nls.localize(1704, null, codePointStr);
                    break;
                case 2 /* UnicodeHighlighterReasonKind.NonBasicAscii */:
                    reason = nls.localize(1705, null, codePointStr);
                    break;
            }
            ...
            const adjustSettings = nls.localize(1706, null);
...
export class DisableHighlightingInCommentsAction extends EditorAction {
    ...
    constructor() {
        super({
            ...
            label: nls.localize2(1715, "Disable highlighting of characters in comments"),
            precondition: undefined
        });
        this.shortLabel = nls.localize(1707, null);
    }
...
export class DisableHighlightingInStringsAction extends EditorAction {
    ...
    constructor() {
        super({
            id: DisableHighlightingOfAmbiguousCharactersAction.ID,
            label: nls.localize2(1716, "Disable highlighting of characters in strings"),
            precondition: undefined
        });
        this.shortLabel = nls.localize(1708, null);
    }
    async run(accessor, editor) {
        const configurationService = accessor.get(IConfigurationService);
        if (configurationService) {
...
export class DisableHighlightingOfAmbiguousCharactersAction extends Action2 {
    ...
    constructor() {
        super({
            id: DisableHighlightingOfAmbiguousCharactersAction.ID,
            title: nls.localize2(1717, "Disable highlighting of ambiguous characters"),
            precondition: undefined,
            f1: false,
        });
        this.shortLabel = nls.localize(1709, null);
    }
    async run(accessor, editor) {
        const configurationService = accessor.get(IConfigurationService);
        if (configurationService) {
...
export class DisableHighlightingOfInvisibleCharactersAction extends Action2 {
    ...
    constructor() {
        super({
            ...
            title: nls.localize2(1718, "Disable highlighting of invisible characters"),
            precondition: undefined,
            f1: false,
        });
        this.shortLabel = nls.localize(1710, null);
    }
    async run(accessor, editor) {
        const configurationService = accessor.get(IConfigurationService);
        if (configurationService) {
...
export class DisableHighlightingOfNonBasicAsciiCharactersAction extends Action2 {
    ...
    constructor() {
        super({
            ...
            title: nls.localize2(1719, "Disable highlighting of non basic ASCII characters"),
            precondition: undefined,
            f1: false,
        });
        this.shortLabel = nls.localize(1711, null);
    }
    async run(accessor, editor) {
        const configurationService = accessor.get(IConfigurationService);
        if (configurationService) {
...
export class ShowExcludeOptions extends Action2 {
    ...
    constructor() {
        super({
            ...
            title: nls.localize2(1720, "Show Exclude Options"),
            precondition: undefined,
            f1: false,
        });
    }
    async run(accessor, args) {
        ...
        function getExcludeCharFromBeingHighlightedLabel(codePoint) {
            if (InvisibleCharacters.isInvisibleCharacter(codePoint)) {
                return nls.localize(1712, null, codePointToHex(codePoint));
            }
            return nls.localize(1713, null, `${codePointToHex(codePoint)} "${char}"`);
        }
        const options = [];
        if (reason.kind === 0 /* UnicodeHighlighterReasonKind.Ambiguous */) {
            for (const locale of reason.notAmbiguousInLocales) {
                options.push({
                    label: nls.localize(1714, null, locale),
                    run: async () => {
