    ...
    openLinkOccurrence(occurrence, openToSide, fromUserGesture = false) {
        ...
        link.resolve(CancellationToken.None).then(uri => {
            ...
            if (messageOrError === 'invalid') {
                this.notificationService.warn(nls.localize(1439, null, link.url.toString()));
            }
            else if (messageOrError === 'missing') {
                this.notificationService.warn(nls.localize(1440, null));
            }
            else {
                onUnexpectedError(err);
            }
...
function getHoverMessage(link, useMetaKey) {
    ...
    const label = link.tooltip
        ? link.tooltip
        : executeCmd
            ? nls.localize(1441, null)
            : nls.localize(1442, null);
    const kb = useMetaKey
        ? platform.isMacintosh
            ? nls.localize(1443, null)
            : nls.localize(1444, null)
        : platform.isMacintosh
            ? nls.localize(1445, null)
            : nls.localize(1446, null);
    if (link.url) {
        let nativeLabel = '';
        if (/^command:/i.test(link.url.toString())) {
            ...
            if (match) {
                ...
                nativeLabel = nls.localize(1447, null, commandId);
            }
        }
...
class OpenLinkAction extends EditorAction {
    constructor() {
        super({
            ...
            label: nls.localize2(1448, "Open Link"),
