...
const accessibleDiffViewerInsertIcon = registerIcon('diff-review-insert', Codicon.add, localize(211, null));
const accessibleDiffViewerRemoveIcon = registerIcon('diff-review-remove', Codicon.remove, localize(212, null));
const accessibleDiffViewerCloseIcon = registerIcon('diff-review-close', Codicon.close, localize(213, null));
...
let View = class View extends Disposable {
    ...
    _render(store) {
        ...
        container.setAttribute('aria-label', localize(215, null));
        ...
        for (const viewItem of group?.lines || []) {
            ...
            if (viewItem.type === LineType.Header) {
                ...
                const getAriaLines = (lines) => lines === 0 ? localize(216, null)
                    : lines === 1 ? localize(217, null)
                        : localize(218, null, lines);
                ...
                header.setAttribute('aria-label', localize(219, null, (diffIndex + 1), diffsLength, r.original.startLineNumber, originalChangedLinesCntAria, r.modified.startLineNumber, modifiedChangedLinesCntAria));
    ...
    _createRow(item, lineHeight, width, originalOptions, originalModel, originalModelOpts, modifiedOptions, modifiedModel, modifiedModelOpts) {
        ...
        if (lineContent.length === 0) {
            lineContent = localize(220, null);
        }
        let ariaLabel = '';
        switch (item.type) {
            case LineType.Unchanged:
                if (item.originalLineNumber === item.modifiedLineNumber) {
                    ariaLabel = localize(221, null, lineContent, item.originalLineNumber);
                }
                else {
                    ariaLabel = localize(222, null, lineContent, item.originalLineNumber, item.modifiedLineNumber);
                }
                break;
            case LineType.Added:
                ariaLabel = localize(223, null, lineContent, item.modifiedLineNumber);
                break;
            case LineType.Deleted:
                ariaLabel = localize(224, null, lineContent, item.originalLineNumber);
                break;
        }
