...
export function ensureFileSystemProviderError(error) {
    if (!error) {
        return createFileSystemProviderError(localize(2078, null), FileSystemProviderErrorCode.Unknown); // https://github.com/microsoft/vscode/issues/72798
    }
    return error;
}
...
export class ByteSize {
    ...
    static formatSize(size) {
        ...
        if (size < ByteSize.MB) {
            return localize(2080, null, (size / ByteSize.KB).toFixed(2));
        }
        if (size < ByteSize.GB) {
            return localize(2081, null, (size / ByteSize.MB).toFixed(2));
        }
        if (size < ByteSize.TB) {
            return localize(2082, null, (size / ByteSize.GB).toFixed(2));
        }
        return localize(2083, null, (size / ByteSize.TB).toFixed(2));
    }
}
