...
let FileService = class FileService extends Disposable {
    ...
    async withProvider(resource) {
        ...
        if (!isAbsolutePath(resource)) {
            throw new FileOperationError(localize(2084, null, this.resourceForError(resource)), 8 /* FileOperationResult.FILE_INVALID_PATH */);
        }
        // Activate provider
        await this.activateProvider(resource.scheme);
        // Assert provider
    ...
    async resolve(resource, options) {
        try {
            ...
            if (toFileSystemProviderErrorCode(error) === FileSystemProviderErrorCode.FileNotFound) {
                throw new FileOperationError(localize(2086, null, this.resourceForError(resource)), 1 /* FileOperationResult.FILE_NOT_FOUND */);
            }
    ...
    async doValidateCreateFile(resource, options) {
        ...
        if (!options?.overwrite && await this.exists(resource)) {
            throw new FileOperationError(localize(2087, null, this.resourceForError(resource)), 3 /* FileOperationResult.FILE_MODIFIED_SINCE */, options);
        }
    }
    ...
    async validateWriteFile(provider, resource, bufferOrReadableOrStream, options) {
        ...
        if ((stat.type & FileType.Directory) !== 0) {
            throw new FileOperationError(localize(2093, null, this.resourceForError(resource)), 0 /* FileOperationResult.FILE_IS_DIRECTORY */, options);
        }
        ...
        if (typeof options?.mtime === 'number' && typeof options.etag === 'string' && options.etag !== ETAG_DISABLED &&
            ...
            throw new FileOperationError(localize(2094, null), 3 /* FileOperationResult.FILE_MODIFIED_SINCE */, options);
        }
        return { stat, buffer };
    }
    ...
    async validateReadFile(resource, options) {
        ...
        if (stat.isDirectory) {
            throw new FileOperationError(localize(2096, null, this.resourceForError(resource)), 0 /* FileOperationResult.FILE_IS_DIRECTORY */, options);
        }
    ...
    validateReadFileLimits(resource, size, options) {
        if (typeof options?.limits?.size === 'number' && size > options.limits.size) {
            throw new TooLargeFileOperationError(localize(2098, null, this.resourceForError(resource)), 7 /* FileOperationResult.FILE_TOO_LARGE */, size, options);
        }
    }
    //#endregion
    ...
    async doValidateMoveCopy(sourceProvider, source, targetProvider, target, mode, overwrite) {
        ...
        if (exists && !isSameResourceWithDifferentPathCase) {
            ...
            if (!overwrite) {
                throw new FileOperationError(localize(2101, null, this.resourceForError(source), this.resourceForError(target)), 4 /* FileOperationResult.FILE_MOVE_CONFLICT */);
            }
    ...
    async doValidateDelete(resource, options) {
        ...
        if (stat) {
            ...
            throw new FileOperationError(localize(2107, null, this.resourceForError(resource)), 1 /* FileOperationResult.FILE_NOT_FOUND */);
        }
        // Validate recursive
    ...
    throwIfFileSystemIsReadonly(provider, resource) {
        if (provider.capabilities & 2048 /* FileSystemProviderCapabilities.Readonly */) {
            throw new FileOperationError(localize(2109, null, this.resourceForError(resource)), 6 /* FileOperationResult.FILE_PERMISSION_DENIED */);
        }
        return provider;
    }
    throwIfFileIsReadonly(resource, stat) {
        if ((stat.permissions ?? 0) & FilePermission.Readonly) {
            throw new FileOperationError(localize(2110, null, this.resourceForError(resource)), 6 /* FileOperationResult.FILE_PERMISSION_DENIED */);
        }
    }
