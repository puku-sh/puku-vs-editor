...
let DialogMainService = class DialogMainService {
    ...
    pickFileFolder(options, window) {
        return this.doPick({ ...options, pickFolders: true, pickFiles: true, title: localize(1879, null) }, window);
    }
    pickFolder(options, window) {
        let optionsInternal = {
            ...options,
            pickFolders: true,
            title: localize(1880, null)
        };
        if (isWindows) {
            ...
            optionsInternal = {
                ...optionsInternal,
                buttonLabel: mnemonicButtonLabel(localize(1881, null)).withMnemonic
            };
        }
        return this.doPick(optionsInternal, window);
    }
    pickFile(options, window) {
        return this.doPick({ ...options, pickFiles: true, title: localize(1882, null) }, window);
    }
    pickWorkspace(options, window) {
        const title = localize(1883, null);
        const buttonLabel = mnemonicButtonLabel(localize(1884, null)).withMnemonic;
