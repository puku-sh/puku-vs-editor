...
let WindowsMainService = class WindowsMainService extends Disposable {
    ...
    async doExtractPathsFromAPI(openConfig) {
        ...
        const pathsToOpen = await Promise.all(coalesce(openConfig.urisToOpen || []).map(async (pathToOpen) => {
            ...
            this.dialogMainService.showMessageBox({
                type: 'info',
                buttons: [localize(2741, null)],
                message: uri.scheme === Schemas.file ? localize(2742, null) : localize(2743, null),
                detail: uri.scheme === Schemas.file ?
                    localize(2744, null, getPathLabel(uri, { os: OS, tildify: this.environmentMainService })) :
                    localize(2745, null, uri.toString(true))
    ...
    async onUNCHostNotAllowed(path, options) {
        ...
        const { response, checkboxChecked } = await this.dialogMainService.showMessageBox({
            ...
            message: localize(2749, null, uri.authority),
            detail: localize(2750, null, getPathLabel(uri, { os: OS, tildify: this.environmentMainService })),
            checkboxLabel: localize(2751, null, uri.authority),
            cancelId: 1
        });
        if (response === 0) {
