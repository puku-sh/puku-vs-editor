    ...
    renderElement(element, _index, data) {
        ...
        if (element.tooltip) {
            ...
            if (this._supportsPreview && element.canPreview) {
                data.container.title = localize(1821, null, actionTitle, previewTitle);
            }
            else {
                data.container.title = localize(1822, null, actionTitle);
            }
        }
        else {
            ...
            accessibilityProvider: {
                getAriaLabel: element => {
                    if (element.kind === "action" /* ActionListItemKind.Action */) {
                        ...
                        if (element.disabled) {
                            label = localize(1823, null, label, element.disabled);
                        }
                        return label;
                    }
                    return null;
                },
                getWidgetAriaLabel: () => localize(1824, null),
                getRole: (e) => {
                    switch (e.kind) {
