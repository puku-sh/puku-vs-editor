...
let WorkspacesHistoryMainService = class WorkspacesHistoryMainService extends Disposable {
    ...
    async clearRecentlyOpened(options) {
        if (options?.confirm) {
            const { response } = await this.dialogMainService.showMessageBox({
                ...
                message: localize(2755, null),
                detail: localize(2756, null),
                cancelId: 1
            });
            if (response !== 0) {
                return;
    ...
    async updateWindowsJumpList() {
        ...
        jumpList.push({
            ...
            items: [
                {
                    type: 'task',
                    title: localize(2757, null),
                    description: localize(2758, null),
        ...
        if ((await this.getRecentlyOpened()).workspaces.length > 0) {
            ...
            if (items.length > 0) {
                jumpList.push({
                    type: 'custom',
                    name: hasWorkspaces ? localize(2759, null) : localize(2760, null),
                    items
                });
            }
        }
    ...
    getWindowsJumpListLabel(workspace, recentLabel) {
        ...
        if (this.workspacesManagementMainService.isUntitledWorkspace(workspace)) {
            return { title: localize(2761, null), description: '' };
        }
        // Workspace: normal
        ...
        return { title: localize(2762, null, filename), description: this.renderJumpListPathDescription(workspace.configPath) };
    }
