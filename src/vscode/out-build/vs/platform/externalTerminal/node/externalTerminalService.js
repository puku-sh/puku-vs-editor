...
const TERMINAL_TITLE = nls.localize(2064, null);
                ...
                osa.on('exit', (code) => {
                    if (code === 0) { // OK
                        ...
                        if (stderr) {
                            ...
                            reject(new Error(nls.localize(2065, null, script, code)));
                        }
                    }
                });
            }
            else {
                reject(new Error(nls.localize(2066, null, terminalApp)));
            }
        });
    }
...
export class LinuxExternalTerminalService extends ExternalTerminalService {
    static { this.WAIT_MESSAGE = nls.localize(2067, null); }
    openTerminal(configuration, cwd) {
        return this.spawnTerminal(cp, configuration, cwd);
    }
    runInTerminal(title, dir, args, envVars, settings) {
                ...
                cmd.on('exit', (code) => {
                    if (code === 0) { // OK
                        ...
                        if (stderr) {
                            ...
                            reject(new Error(nls.localize(2068, null, exec, code)));
                        }
                    }
                });
            });
...
function improveError(err) {
    if (err.errno === 'ENOENT' && err.path) {
        return new Error(nls.localize(2069, null, err.path));
    }
    return err;
}
/**
