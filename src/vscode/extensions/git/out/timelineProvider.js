    ...
    return class GitTimelineProvider {
        ...
        async provideTimeline(uri, options, token) {
            ...
            for (let index = 0; index < commits.length; index++) {
                ...
                const commands = [
                    (0, historyProvider_1.getHistoryItemHoverCommitHashCommands)(uri, c.hash),
                    (0, historyProvider_1.processHistoryItemRemoteHoverCommands)(commitRemoteSourceCommands, c.hash)
                ];
                item.tooltip = (0, historyProvider_1.getHistoryItemHover)(avatars?.get(c.hash), c.authorName, c.authorEmail, date, messageWithLinks, c.shortStat, undefined, commands);
            ...
            if (options.cursor === undefined) {
                ...
                if (index) {
                    ...
                    item.iconPath = new vscode_1.ThemeIcon('git-commit');
                    item.description = '';
                    item.tooltip = (0, historyProvider_1.getHistoryItemHover)(undefined, you, undefined, date, repository_1.Resource.getStatusText(index.type), undefined, undefined, undefined);
                    const cmd = this.commands.resolveTimelineOpenDiffCommand(item, uri);
                    if (cmd) {
                        item.command = {
                            title: openComparison,
                ...
                if (showUncommitted) {
                    ...
                    if (working) {
                        ...
                        item.description = '';
                        item.tooltip = (0, historyProvider_1.getHistoryItemHover)(undefined, you, undefined, date, repository_1.Resource.getStatusText(working.type), undefined, undefined, undefined);
                        const cmd = this.commands.resolveTimelineOpenDiffCommand(item, uri);
                        if (cmd) {
                            item.command = {
                                title: openComparison,
