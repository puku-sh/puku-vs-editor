...
exports.AVATAR_SIZE = exports.GitHistoryProvider = void 0;
exports.getHistoryItemHoverCommitHashCommands = getHistoryItemHoverCommitHashCommands;
exports.processHistoryItemRemoteHoverCommands = processHistoryItemRemoteHoverCommands;
        ...
        async provideHistoryItems(options, token) {
            ...
            try {
                ...
                for (const commit of commits) {
                    ...
                    const commands = [
                        getHistoryItemHoverCommitHashCommands(vscode_1.Uri.file(this.repository.root), commit.hash),
...
function getHistoryItemHoverCommitHashCommands(documentUri, hash) {
    return [{
...
function processHistoryItemRemoteHoverCommands(commands, hash) {
    return commands.map(command => ({
        ...command,
        arguments: [...command.arguments ?? [], hash]
    }));
}
function getHistoryItemHover(authorAvatar, authorName, authorEmail, authorDate, message, shortStats, references, commands) {
    ...
    if (references && references.length > 0) {
        for (const reference of references) {
            const labelIconId = reference.icon instanceof vscode_1.ThemeIcon ? reference.icon.id : '';
