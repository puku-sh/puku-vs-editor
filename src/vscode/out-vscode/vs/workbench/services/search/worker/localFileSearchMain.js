...
function $rj(arg1, path, options) {
  if (!arg1 || typeof path !== "string") {
    return false;
  }
  return $sj(arg1, options)(path);
}
...
var $sHc = class _$sHc {
  ...
  static getChannel(workerServer) {
    return workerServer.getChannel(_$sHc.CHANNEL_NAME);
  }
  static setChannel(workerClient, obj) {
    workerClient.setChannel(_$sHc.CHANNEL_NAME, obj);
  }
};
...
`;
      ...
      const fileRange = new $HD(startLine, matchStartIndex - lineRanges[startLine].start, endLine, matchStartIndex + matchedText.length - lineRanges[endLine].start);
      const previewRange = new $HD(0, matchStartIndex - lineRanges[startLine].start - offset, endLine - startLine, matchStartIndex + matchedText.length - lineRanges[endLine].start - (endLine === startLine ? offset : 0));
      const match = {
        rangeLocations: [{
          source: fileRange,
...
var $6Rb = class {
  constructor(contents, b, c, d = false) {
    this.b = b;
    this.c = c;
    this.d = d;
    if (b[b.length - 1] === "\\") {
    ...
    this.a = this.f(contents, this.b, this.c);
  }
  /**
  ...
  e(lines, dirPath, trimForExclusions2) {
    const includeLines = lines.map((line) => this.g(line, dirPath));
    const includeExpression = /* @__PURE__ */ Object.create(null);
    for (const line of includeLines) {
      includeExpression[line] = true;
    }
    return $sj(includeExpression, { trimForExclusions: trimForExclusions2, ignoreCase: this.d });
  }
  f(ignoreContents, dirPath, parent) {
    ...
    const isFileIgnored = this.e(fileIgnoreLines, dirPath, true);
    const fileIncludeLines = fileLines.filter((line) => line.includes("!")).map((line) => line.replace(/!/g, ""));
    const isFileIncluded = this.e(fileIncludeLines, dirPath, false);
    const dirIgnoreLines = contentLines.filter((line) => !line.includes("!"));
    const isDirIgnored = this.e(dirIgnoreLines, dirPath, true);
    const dirIncludeLines = contentLines.filter((line) => line.includes("!")).map((line) => line.replace(/!/g, ""));
    const isDirIncluded = this.e(dirIncludeLines, dirPath, false);
    const isPathIgnored = (path, isDir) => {
      if (!(this.d ? $ig(path, dirPath) : path.startsWith(dirPath))) {
        return false;
      }
...
function $yYc(workerServer) {
  return new $zYc(workerServer);
}
var $zYc = class {
  constructor(workerServer) {
    this._requestHandlerBrand = void 0;
    this.cancellationTokens = /* @__PURE__ */ new Map();
    this.d = $sHc.getChannel(workerServer);
  }
  $cancelQuery(queryId) {
  ...
  async $listDirectory(handle, query, folderQuery, ignorePathCasing, queryId) {
    ...
    const extUri = new $ph(() => ignorePathCasing);
  ...
  async $searchDirectory(handle, query, folderQuery, ignorePathCasing, queryId) {
    ...
    return time("searchInFiles", async () => {
      ...
      const processFile = async (file) => {
        ...
        const fileResults = $xYc(bytes, pattern, {
  ...
  async h(handle, queryProps, folderQuery, extUri, onFile, token) {
    const folderExcludes = folderQuery.excludePattern?.map((excludePattern) => $sj(excludePattern.pattern ?? {}, { trimForExclusions: true }));
    ...
    const processDirectory = async (directory, prior, ignoreFile) => {
      if (!folderQuery.disregardIgnoreFiles) {
        ...
        await Promise.all(ignoreFiles.map(async (file) => {
          ...
          ignoreFile = new $6Rb(ignoreContents, prior, ignoreFile);
        }));
      }
...
function createSearchRegExp(options) {
  return $1f(options.pattern, !!options.isRegExp, {
...
function pathExcludedInQuery(queryProps, fsPath) {
  if (queryProps.excludePattern && $rj(queryProps.excludePattern, fsPath)) {
    return true;
  }
  return false;
}
function pathIncludedInQuery(queryProps, path, extUri) {
  if (queryProps.excludePattern && $rj(queryProps.excludePattern, path)) {
    return false;
  }
  if (queryProps.includePattern || queryProps.usingSearchPaths) {
    if (queryProps.includePattern && $rj(queryProps.includePattern, path)) {
      return true;
    }
    if (queryProps.usingSearchPaths) {
      return !!queryProps.folderQueries && queryProps.folderQueries.some((fq) => {
        ...
        if (extUri.isEqualOrParent(uri, searchPath)) {
          const relPath = $_(searchPath.path, uri.path);
          return !fq.includePattern || !!$rj(fq.includePattern, relPath);
        } else {
          return false;
        }
      });
