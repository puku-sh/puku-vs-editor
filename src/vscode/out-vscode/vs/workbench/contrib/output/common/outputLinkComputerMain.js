...
var $dwc = class _$dwc {
  constructor(workerServer) {
    ...
    this.a = new $4fb();
    this.b = /* @__PURE__ */ new Map();
    this.a.bindToServer(workerServer);
  }
  $setWorkspaceFolders(workspaceFolders) {
    this.c(workspaceFolders);
  }
  c(_workspaceFolders) {
    ...
    for (const workspaceFolder of workspaceFolders) {
      const patterns = _$dwc.createPatterns(workspaceFolder);
      this.b.set(workspaceFolder, patterns);
    }
  }
  d(uri) {
    return this.a.getModel(uri);
  }
  $computeLinks(uri) {
    ...
    const links = [];
    const lines = $4f(model.getValue());
    for (const [folderUri, folderPatterns] of this.b) {
      const resourceCreator = {
        toResource: (folderRelativePath) => {
          if (typeof folderRelativePath === "string") {
            return $Ah(folderUri, folderRelativePath);
          }
          return null;
        }
      };
      for (let i = 0, len = lines.length; i < len; i++) {
        links.push(..._$dwc.detectLinks(lines[i], i + 1, folderPatterns, resourceCreator));
      }
    }
    return links;
  }
  static createPatterns(workspaceFolder) {
    ...
    if ($m && workspaceFolder.scheme === Schemas.file) {
      workspaceFolderVariants.push($Wg(workspaceFolderPath));
    }
    for (const workspaceFolderVariant of workspaceFolderVariants) {
      ...
      patterns.push(new RegExp($Sf(workspaceFolderVariant) + `(${pathPattern}) on line ((\\d+)(, column (\\d+))?)`, "gi"));
      patterns.push(new RegExp($Sf(workspaceFolderVariant) + `(${pathPattern}):line ((\\d+)(, column (\\d+))?)`, "gi"));
      patterns.push(new RegExp($Sf(workspaceFolderVariant) + `(${pathPattern})(\\s?\\((\\d+)(,(\\d+))?)\\)`, "gi"));
      patterns.push(new RegExp($Sf(workspaceFolderVariant) + `(${strictPathPattern})(:(\\d+))?(:(\\d+))?`, "gi"));
    }
    return patterns;
  }
  /**
  ...
  static detectLinks(line, lineIndex, patterns, resourceCreator) {
    ...
    patterns.forEach((pattern) => {
      ...
      while ((match = pattern.exec(line)) !== null) {
        const folderRelativePath = $Yf(match[1], ".").replace(/\\/g, "/");
        let resourceString;
        try {
        ...
        if (match[3]) {
          ...
          if (match[5]) {
            ...
            resourceString = $Of("{0}#{1},{2}", resourceString, lineNumber, columnNumber);
          } else {
            resourceString = $Of("{0}#{1}", resourceString, lineNumber);
          }
        }
        const fullMatch = $Yf(match[0], ".");
        ...
        if (links.some((link) => $HD.areIntersectingOrTouching(link.range, linkRange))) {
          return;
        }
        links.push({
          range: linkRange,
...
function $ewc(workerServer) {
  return new $dwc(workerServer);
}
