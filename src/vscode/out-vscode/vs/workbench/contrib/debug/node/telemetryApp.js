...
var ChannelServer = class {
  ...
  onPromise(request) {
    const channel2 = this.channels.get(request.channelName);
    if (!channel2) {
    ...
    let promise;
    try {
      promise = channel2.call(this.ctx, request.name, request.arg, cancellationTokenSource.token);
    } catch (err) {
  ...
  onEventListen(request) {
    ...
    const id2 = request.id;
    const event = channel2.listen(this.ctx, request.name, request.arg);
