...
function $Ti(stream, listener, token) {
...
function $bj(stream) {
  return $Si(stream, (chunks) => $2i.concat(chunks));
}

// out-build/vs/base/common/objects.js
function $wp(destination, source, overwrite = true) {
  if (!$0c(destination)) {
    return source;
  }
  if ($0c(source)) {
    Object.keys(source).forEach((key) => {
      if (key in destination) {
        if (overwrite) {
          if ($0c(destination[key]) && $0c(source[key])) {
            $wp(destination[key], source[key], overwrite);
          } else {
...
function $yp(obj) {
  ...
  return JSON.stringify(obj, (key, value) => {
    if ($0c(value) || Array.isArray(value)) {
      if (seen.has(value)) {
        return "[Circular]";
      } else {
        seen.add(value);
...
var $1m = class {
  ...
  registerChannel(channelName, channel2) {
    this.b.set(channelName, channel2);
  ...
  s(request) {
    const channel2 = this.b.get(request.channelName);
    if (!channel2) {
      this.v(request);
      return;
    }
    ...
    let promise;
    try {
      promise = channel2.call(this.j, request.name, request.arg, cancellationTokenSource.token);
    } catch (err) {
  ...
  t(request) {
    ...
    const id2 = request.id;
    const event = channel2.listen(this.j, request.name, request.arg);
...
var $gp = $Fj("telemetryService");
var $ip = $Fj("customEndpointTelemetryService");
var TelemetryLevel;
...
var $7u = new $6u();
var $9u = "telemetry";
var $0u = { id: $9u, name: localize(2330, null) };
function $cv(data) {
  const properties = {};
  const measurements = {};
  const flat = {};
...
async function getClient(instrumentationKey, addInternalFlag, xhrOverride) {
  const oneDs = $s ? await $PK("@microsoft/1ds-core-js", "bundle/ms.core.min.js") : await import("@microsoft/1ds-core-js");
  const postPlugin = $s ? await $PK("@microsoft/1ds-post-js", "bundle/ms.post.min.js") : await import("@microsoft/1ds-post-js");
...
async function makeTelemetryRequest(options, requestService) {
  ...
  const responseData = (await $bj(response.stream)).toString();
...
var appender = new $d5(void 0, false, process.argv[2], JSON.parse(process.argv[3]), process.argv[4]);
process.once("exit", () => appender.flush());
var channel = new $Fy([appender]);
var server = new $ox("telemetry");
